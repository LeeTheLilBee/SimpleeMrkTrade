"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_510_TOWER_BROKER_SAFEGUARD_CHECKLIST_BATCH_CLOSE_READINESS_TESTS_RECURSION_SAFE
"""
...
def test_pack_510_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_batch_close_readiness_v510")
    payload = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()

    assert payload["pack"] == "510"
    assert payload["pack_number"] == 510
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-batch-close-readiness-v510.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "509"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "511"
    assert payload["cached"] is True
    assert payload["non_recursive"] is True
    assert payload["recursion_safe"] is True
    assert payload["simulation_only"] is True
    assert payload["preview_only"] is True
    assert payload["tower_broker_safeguard_checklist_preview_only"] is True
    ...
    assert payload["safe_to_continue_to_pack_511"] is True
    assert payload["prepare_pack_511_tower_manual_live_clearance_gate_index"]["pack"] == "511"


def test_pack_510_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_batch_close_readiness_v510")
    payload = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()
    summary = payload["tower_broker_safeguard_checklist_batch_close_readiness_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_batch_close_readiness_ready"] is True
...
def test_pack_510_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_batch_close_readiness_v510")
    payload = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()

    rows = payload["tower_broker_safeguard_checklist_batch_close_readiness_rows"]
    checks = payload["tower_broker_safeguard_checklist_batch_close_readiness_checks"]
    actions = payload["tower_broker_safeguard_checklist_batch_close_readiness_actions"]
    ...
    bridge = mod.build_pack_510_status_bridge()
    assert bridge["pack"] == "510"
    ...
    assert bridge["tower_broker_safeguard_checklist_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_511"] is True

    prep = mod.prepare_pack_511_tower_manual_live_clearance_gate_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "511"
    assert prep["source_pack"] == "510"

    first = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()
    second = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_batch_close_readiness_preview()
