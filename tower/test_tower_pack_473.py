"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_473_TOWER_KILL_SWITCH_BOARD_POLICY_MATRIX_TESTS
"""
...
def test_pack_473_contract_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_policy_matrix_v473")
    payload = mod.build_tower_kill_switch_board_policy_matrix_preview()

    assert payload["pack"] == "473"
    assert payload["pack_number"] == 473
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-kill-switch-board-policy-matrix-v473.json"
    ...
    assert payload["safe_to_continue_to_pack_474"] is True
    assert payload["prepare_pack_474_tower_kill_switch_board_detail_drawer"]["pack"] == "474"


def test_pack_473_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_policy_matrix_v473")
    payload = mod.build_tower_kill_switch_board_policy_matrix_preview()
    summary = payload["tower_kill_switch_board_policy_matrix_summary"]
    ...
    assert summary["tower_kill_switch_board_policy_matrix_ready"] is True
...
def test_pack_473_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_policy_matrix_v473")
    payload = mod.build_tower_kill_switch_board_policy_matrix_preview()

    rows = payload["tower_kill_switch_board_policy_matrix_rows"]
    checks = payload["tower_kill_switch_board_policy_matrix_checks"]
    actions = payload["tower_kill_switch_board_policy_matrix_actions"]
    ...
    bridge = mod.build_pack_473_status_bridge()
    assert bridge["pack"] == "473"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_kill_switch_board_policy_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_474"] is True

    prep = mod.prepare_pack_474_tower_kill_switch_board_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "474"
    assert prep["source_pack"] == "473"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-kill-switch-board-policy-matrix-v473.json" in rules

    response = app.test_client().get("/tower/tower-kill-switch-board-policy-matrix-v473.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "473"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_474"] is True

    first = mod.build_tower_kill_switch_board_policy_matrix_preview()
    second = mod.build_tower_kill_switch_board_policy_matrix_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_kill_switch_board_policy_matrix_preview()
