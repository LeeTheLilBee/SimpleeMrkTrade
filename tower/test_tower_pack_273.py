"""
SEARCHABLE LABEL: TOWER_PACK_273_HANDOFF_EVIDENCE_ROUTE_READINESS_NOTE_DRAFT_PREVIEW_TESTS
"""
...
def test_pack_273_note_draft_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()

    assert payload["pack"] == "273"
    assert payload["pack_number"] == 273
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Handoff Evidence Route Readiness Note Draft Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-draft-v273.json"
    ...
    assert payload["source_pack"] == "272"
    ...
    assert payload["safe_to_continue_to_pack_274"] is True
    assert payload["prepare_pack_274_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version"]["pack"] == "274"


def test_pack_273_note_draft_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()

    summary = payload["handoff_evidence_route_readiness_note_draft_summary"]

    assert summary["source_detail_drawer_count"] >= 14
    assert summary["draft_card_count"] >= 14
    assert summary["draft_field_count"] >= 140
    assert summary["draft_action_count"] >= 196
    ...
    assert summary["locked_field_count"] >= 56
    ...
    assert summary["evidence_route_readiness_note_draft_ready"] is True
...
def test_pack_273_cards_fields_actions_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()

    cards = payload["handoff_evidence_route_readiness_note_draft_cards"]
    fields = payload["handoff_evidence_route_readiness_note_draft_fields"]
    actions = payload["handoff_evidence_route_readiness_note_draft_actions"]
    checkpoints = payload["handoff_evidence_route_readiness_note_draft_checkpoints"]
...
def test_pack_273_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()
...
def test_pack_273_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_preview()

    assert third["status"] == "ready"


def test_pack_273_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_draft_v273")

    bridge = mod.build_pack_273_status_bridge()
    assert bridge["pack"] == "273"
    assert bridge["pack_number"] == 273
    ...
    assert bridge["source_pack"] == "272"
    assert bridge["source_closed_batch"] == "266-270"
    assert bridge["save_batch"] == "271-275"
    assert bridge["save_after_pack"] == 275
    assert bridge["next_pack"] == "274"
    assert bridge["draft_card_count"] >= 14
    assert bridge["draft_field_count"] >= 140
    assert bridge["draft_action_count"] >= 196
    assert bridge["evidence_route_readiness_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_274"] is True

    prep = mod.prepare_pack_274_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "274"
...
def test_pack_273_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-draft-v273.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-draft-v273.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "273"
