...
def test_pack_2508_human_login_ob_contract():
    payload = build_ir_cert_p2508_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Observatory Permission Gate'
    assert payload["observatory_permission_gate_ready"] is True
