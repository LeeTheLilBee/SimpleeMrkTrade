"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_519_TOWER_MANUAL_LIVE_CLEARANCE_GATE_OWNER_FOCUS_BRIDGE_TESTS_RECURSION_SAFE
"""
...
def test_pack_519_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_owner_focus_bridge_v519")
    payload = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()

    assert payload["pack"] == "519"
    assert payload["pack_number"] == 519
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-owner-focus-bridge-v519.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "518"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "520"
    ...
    assert payload["safe_to_continue_to_pack_520"] is True
    assert payload["prepare_pack_520_tower_manual_live_clearance_gate_batch_close_readiness"]["pack"] == "520"


def test_pack_519_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_owner_focus_bridge_v519")
    payload = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()
    summary = payload["tower_manual_live_clearance_gate_owner_focus_bridge_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_owner_focus_bridge_ready"] is True
...
def test_pack_519_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_owner_focus_bridge_v519")
    payload = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()

    rows = payload["tower_manual_live_clearance_gate_owner_focus_bridge_rows"]
    checks = payload["tower_manual_live_clearance_gate_owner_focus_bridge_checks"]
    actions = payload["tower_manual_live_clearance_gate_owner_focus_bridge_actions"]
    ...
    bridge = mod.build_pack_519_status_bridge()
    assert bridge["pack"] == "519"
    ...
    assert bridge["tower_manual_live_clearance_gate_owner_focus_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_520"] is True

    prep = mod.prepare_pack_520_tower_manual_live_clearance_gate_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "520"
    assert prep["source_pack"] == "519"

    first = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()
    second = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_owner_focus_bridge_preview()
