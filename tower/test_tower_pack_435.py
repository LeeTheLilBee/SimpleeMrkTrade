"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_435_SHARED_NERVOUS_SYSTEM_STANDARDS_NOTE_DRAFT_TESTS
"""
...
def test_pack_435_contract_ready():
    mod = importlib.import_module("tower.tower_shared_nervous_system_standards_note_draft_v435")
    payload = mod.build_shared_nervous_system_standards_note_draft_preview()

    assert payload["pack"] == "435"
    assert payload["pack_number"] == 435
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-nervous-system-standards-note-draft-v435.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Nervous System Standards layer"
    assert payload["source_pack"] == "434"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "436"
    ...
    assert payload["safe_to_continue_to_pack_436"] is True
    assert payload["prepare_pack_436_shared_nervous_system_standards_note_version"]["pack"] == "436"


def test_pack_435_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_nervous_system_standards_note_draft_v435")
    payload = mod.build_shared_nervous_system_standards_note_draft_preview()
    summary = payload["shared_nervous_system_standards_note_draft_summary"]
    ...
    assert summary["shared_nervous_system_standards_note_draft_ready"] is True
...
def test_pack_435_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_nervous_system_standards_note_draft_v435")
    payload = mod.build_shared_nervous_system_standards_note_draft_preview()

    rows = payload["shared_nervous_system_standards_note_draft_rows"]
    checks = payload["shared_nervous_system_standards_note_draft_checks"]
    actions = payload["shared_nervous_system_standards_note_draft_actions"]
    ...
    bridge = mod.build_pack_435_status_bridge()
    assert bridge["pack"] == "435"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_nervous_system_standards_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_436"] is True

    prep = mod.prepare_pack_436_shared_nervous_system_standards_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "436"
    assert prep["source_pack"] == "435"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-nervous-system-standards-note-draft-v435.json" in rules

    response = app.test_client().get("/tower/shared-nervous-system-standards-note-draft-v435.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "435"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_436"] is True

    first = mod.build_shared_nervous_system_standards_note_draft_preview()
    second = mod.build_shared_nervous_system_standards_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_nervous_system_standards_note_draft_preview()
