"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_533_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_SAFETY_MATRIX_TESTS_RECURSION_SAFE
"""
...
def test_pack_533_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_safety_matrix_v533")
    payload = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()

    assert payload["pack"] == "533"
    assert payload["pack_number"] == 533
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-safety-matrix-v533.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "532"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "534"
    ...
    assert payload["safe_to_continue_to_pack_534"] is True
    assert payload["prepare_pack_534_tower_owner_override_decision_trigger_detail_drawer"]["pack"] == "534"


def test_pack_533_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_safety_matrix_v533")
    payload = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()
    summary = payload["tower_owner_override_decision_trigger_safety_matrix_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_safety_matrix_ready"] is True
...
def test_pack_533_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_safety_matrix_v533")
    payload = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()

    rows = payload["tower_owner_override_decision_trigger_safety_matrix_rows"]
    checks = payload["tower_owner_override_decision_trigger_safety_matrix_checks"]
    actions = payload["tower_owner_override_decision_trigger_safety_matrix_actions"]
    ...
    bridge = mod.build_pack_533_status_bridge()
    assert bridge["pack"] == "533"
    ...
    assert bridge["tower_owner_override_decision_trigger_safety_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_534"] is True

    prep = mod.prepare_pack_534_tower_owner_override_decision_trigger_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "534"
    assert prep["source_pack"] == "533"

    first = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()
    second = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_safety_matrix_preview()
