...
def test_pack_2426_walkthrough_contract():
    payload = build_ir_cert_p2426_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Walkthrough Launch Review"
    assert payload["launch_review_ready"] is True
