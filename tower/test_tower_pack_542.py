"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_542_TOWER_BETA_BACKBONE_FINAL_READINESS_REGISTRY_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_542_contract_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_registry_contract_v542")
    payload = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()

    assert payload["pack"] == "542"
    assert payload["pack_number"] == 542
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-backbone-final-readiness-registry-contract-v542.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Tower Beta Backbone Final Readiness layer"
    assert payload["source_pack"] == "541"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "543"
    ...
    assert payload["safe_to_continue_to_pack_543"] is True
    assert payload["prepare_pack_543_tower_beta_backbone_final_readiness_safety_matrix"]["pack"] == "543"


def test_pack_542_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_registry_contract_v542")
    payload = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()
    summary = payload["tower_beta_backbone_final_readiness_registry_contract_summary"]
    ...
    assert summary["tower_beta_backbone_final_readiness_registry_contract_ready"] is True
...
def test_pack_542_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_registry_contract_v542")
    payload = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()

    rows = payload["tower_beta_backbone_final_readiness_registry_contract_rows"]
    checks = payload["tower_beta_backbone_final_readiness_registry_contract_checks"]
    actions = payload["tower_beta_backbone_final_readiness_registry_contract_actions"]
    ...
    bridge = mod.build_pack_542_status_bridge()
    assert bridge["pack"] == "542"
    ...
    assert bridge["tower_beta_backbone_final_readiness_registry_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_543"] is True

    prep = mod.prepare_pack_543_tower_beta_backbone_final_readiness_safety_matrix()
    assert prep["ready"] is True
    assert prep["next_pack"] == "543"
    assert prep["source_pack"] == "542"

    first = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()
    second = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_backbone_final_readiness_registry_contract_preview()
