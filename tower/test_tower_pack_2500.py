...
def test_pack_2500_activation_approval_contract():
    payload = build_ir_cert_p2500_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Approval Decision Receipt'
    assert payload["owner_approval_decision_ready"] is True
