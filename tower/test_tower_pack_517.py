"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_517_TOWER_MANUAL_LIVE_CLEARANCE_GATE_HANDOFF_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_517_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_handoff_contract_v517")
    payload = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()

    assert payload["pack"] == "517"
    assert payload["pack_number"] == 517
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-handoff-contract-v517.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "516"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "518"
    ...
    assert payload["safe_to_continue_to_pack_518"] is True
    assert payload["prepare_pack_518_tower_manual_live_clearance_gate_readiness_bridge"]["pack"] == "518"


def test_pack_517_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_handoff_contract_v517")
    payload = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()
    summary = payload["tower_manual_live_clearance_gate_handoff_contract_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_handoff_contract_ready"] is True
...
def test_pack_517_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_handoff_contract_v517")
    payload = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()

    rows = payload["tower_manual_live_clearance_gate_handoff_contract_rows"]
    checks = payload["tower_manual_live_clearance_gate_handoff_contract_checks"]
    actions = payload["tower_manual_live_clearance_gate_handoff_contract_actions"]
    ...
    bridge = mod.build_pack_517_status_bridge()
    assert bridge["pack"] == "517"
    ...
    assert bridge["tower_manual_live_clearance_gate_handoff_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_518"] is True

    prep = mod.prepare_pack_518_tower_manual_live_clearance_gate_readiness_bridge()
    assert prep["ready"] is True
    assert prep["next_pack"] == "518"
    assert prep["source_pack"] == "517"

    first = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()
    second = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_handoff_contract_preview()
