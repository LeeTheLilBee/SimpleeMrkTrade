"""
SEARCHABLE LABEL: TOWER_PACK_386_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_FINAL_CLOSEOUT_ARCHIVE_TESTS
"""
...
def test_pack_386_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_v386")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()

    assert payload["pack"] == "386"
    assert payload["pack_number"] == 386
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-archive-index-v386.json"
    assert payload["tower_sublayer"] == "Handoff Policy Route Enforcement Owner Acceptance Final Closeout Archive layer"
    assert payload["source_pack"] == "385"
    assert payload["source_closed_batch"] == "366-385"
    assert payload["save_batch"] == "386-390"
    assert payload["save_after_pack"] == 390
    assert payload["next_pack"] == "387"
    ...
    assert payload["safe_to_continue_to_pack_387"] is True
    assert payload["prepare_pack_387_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_detail_drawer"]["pack"] == "387"


def test_pack_386_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_v386")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()
    summary = payload["owner_acceptance_final_closeout_archive_index_summary"]
    ...
    assert summary["owner_acceptance_final_closeout_archive_index_ready"] is True
...
def test_pack_386_payload_shapes_endpoint_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_v386")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()

    rows = payload["owner_acceptance_final_closeout_archive_index_rows"]
    actions = payload["owner_acceptance_final_closeout_archive_index_actions"]
    checkpoints = payload["owner_acceptance_final_closeout_archive_index_checkpoints"]
    ...
    bridge = mod.build_pack_386_status_bridge()
    assert bridge["pack"] == "386"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["owner_acceptance_final_closeout_archive_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_387"] is True

    prep = mod.prepare_pack_387_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "387"
    assert prep["source_pack"] == "386"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-archive-index-v386.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-archive-index-v386.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "386"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["archive_preview_only"] is True
        assert data["safe_to_continue_to_pack_387"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_archive_index_preview()
