"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_427_SHARED_CONNECTIVE_TISSUE_HANDOFF_CONTRACT_TESTS
"""
...
def test_pack_427_contract_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_handoff_contract_v427")
    payload = mod.build_shared_connective_tissue_handoff_contract_preview()

    assert payload["pack"] == "427"
    assert payload["pack_number"] == 427
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-connective-tissue-handoff-contract-v427.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Connective Tissue layer"
    assert payload["source_pack"] == "426"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "428"
    ...
    assert payload["safe_to_continue_to_pack_428"] is True
    assert payload["prepare_pack_428_shared_connective_tissue_readiness_bridge"]["pack"] == "428"


def test_pack_427_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_handoff_contract_v427")
    payload = mod.build_shared_connective_tissue_handoff_contract_preview()
    summary = payload["shared_connective_tissue_handoff_contract_summary"]
    ...
    assert summary["shared_connective_tissue_handoff_contract_ready"] is True
...
def test_pack_427_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_handoff_contract_v427")
    payload = mod.build_shared_connective_tissue_handoff_contract_preview()

    rows = payload["shared_connective_tissue_handoff_contract_rows"]
    checks = payload["shared_connective_tissue_handoff_contract_checks"]
    actions = payload["shared_connective_tissue_handoff_contract_actions"]
    ...
    bridge = mod.build_pack_427_status_bridge()
    assert bridge["pack"] == "427"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_connective_tissue_handoff_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_428"] is True

    prep = mod.prepare_pack_428_shared_connective_tissue_readiness_bridge()
    assert prep["ready"] is True
    assert prep["next_pack"] == "428"
    assert prep["source_pack"] == "427"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-connective-tissue-handoff-contract-v427.json" in rules

    response = app.test_client().get("/tower/shared-connective-tissue-handoff-contract-v427.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "427"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_428"] is True

    first = mod.build_shared_connective_tissue_handoff_contract_preview()
    second = mod.build_shared_connective_tissue_handoff_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_connective_tissue_handoff_contract_preview()
