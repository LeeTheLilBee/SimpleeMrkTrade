"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_423_SHARED_CONNECTIVE_TISSUE_SNAPSHOT_CONTRACT_TESTS
"""
...
def test_pack_423_contract_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_snapshot_contract_v423")
    payload = mod.build_shared_connective_tissue_snapshot_contract_preview()

    assert payload["pack"] == "423"
    assert payload["pack_number"] == 423
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-connective-tissue-snapshot-contract-v423.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Connective Tissue layer"
    assert payload["source_pack"] == "422"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "424"
    ...
    assert payload["safe_to_continue_to_pack_424"] is True
    assert payload["prepare_pack_424_shared_connective_tissue_detail_drawer"]["pack"] == "424"


def test_pack_423_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_snapshot_contract_v423")
    payload = mod.build_shared_connective_tissue_snapshot_contract_preview()
    summary = payload["shared_connective_tissue_snapshot_contract_summary"]
    ...
    assert summary["shared_connective_tissue_snapshot_contract_ready"] is True
...
def test_pack_423_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_snapshot_contract_v423")
    payload = mod.build_shared_connective_tissue_snapshot_contract_preview()

    rows = payload["shared_connective_tissue_snapshot_contract_rows"]
    checks = payload["shared_connective_tissue_snapshot_contract_checks"]
    actions = payload["shared_connective_tissue_snapshot_contract_actions"]
    ...
    bridge = mod.build_pack_423_status_bridge()
    assert bridge["pack"] == "423"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_connective_tissue_snapshot_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_424"] is True

    prep = mod.prepare_pack_424_shared_connective_tissue_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "424"
    assert prep["source_pack"] == "423"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-connective-tissue-snapshot-contract-v423.json" in rules

    response = app.test_client().get("/tower/shared-connective-tissue-snapshot-contract-v423.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "423"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_424"] is True

    first = mod.build_shared_connective_tissue_snapshot_contract_preview()
    second = mod.build_shared_connective_tissue_snapshot_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_connective_tissue_snapshot_contract_preview()
