...
def test_pack_2444_guided_run_contract():
    payload = build_ir_cert_p2444_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Six-Room Sequence Guard'
    assert payload["six_room_sequence_guard_ready"] is True
