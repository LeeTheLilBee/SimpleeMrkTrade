...
def test_pack_2490_deployment_boundary():
    payload = build_ir_cert_p2490_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Operations Release Evidence Packet'
    assert payload["release_evidence_packet_ready"] is True
