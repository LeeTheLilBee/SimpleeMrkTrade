...
def test_pack_2493_activation_approval_contract():
    payload = build_ir_cert_p2493_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Activation Approval Request'
    assert payload["activation_approval_request_ready"] is True
