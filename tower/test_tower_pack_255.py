"""
SEARCHABLE LABEL: TOWER_PACK_255_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_CONTINUOUS_ASSURANCE_BATCH_CLOSE_READINESS_PREVIEW_TESTS
"""
...
def test_pack_255_batch_close_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()

    assert payload["pack"] == "255"
    assert payload["pack_number"] == 255
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Continuous Assurance Batch Close Readiness Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-batch-close-readiness-v255.json"
    ...
    assert payload["source_packs"]["251"]["pack"] == "251"
    assert payload["source_packs"]["252"]["pack"] == "252"
    assert payload["source_packs"]["253"]["pack"] == "253"
    assert payload["source_packs"]["254"]["pack"] == "254"

    assert payload["safe_to_continue_to_pack_256"] is True
    assert payload["prepare_pack_256_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_queue"]["pack"] == "256"


def test_pack_255_batch_close_summary_ready_to_continue():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()

    summary = payload["governance_continuous_assurance_batch_close_summary"]

    assert summary["save_batch"] == "251-260"
    assert summary["save_after_pack"] == 260
    assert summary["next_batch"] == "251-260"
    assert summary["pack_card_count"] >= 5
    assert summary["close_check_count"] >= 11
    assert summary["save_manifest_preview_count"] >= 11
    assert summary["transition_preview_count"] >= 2
    assert summary["future_commit_manifest_count"] >= 11
    ...
    assert summary["continuous_assurance_ready_to_continue"] is True

    assert summary["real_batch_close_write_enabled"] is False
    assert summary["real_continuous_assurance_write_enabled"] is False
    assert summary["real_continuous_assurance_batch_close_write_enabled"] is False
...
def test_pack_255_cards_checks_manifest_and_transitions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()

    cards = payload["governance_continuous_assurance_batch_pack_cards"]
    checks = payload["governance_continuous_assurance_batch_close_checks"]
    manifest = payload["governance_continuous_assurance_save_manifest_preview"]
    transitions = payload["governance_continuous_assurance_transition_previews"]

    packs = {card["pack"] for card in cards}
    assert {"251", "252", "253", "254", "255"}.issubset(packs)
    ...
    assert all(row["include_in_future_commit"] is True for row in manifest)
...
def test_pack_255_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()
    ...
    assert safety["no_real_continuous_assurance_batch_close_write"] is True
...
def test_pack_255_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_preview()

    assert third["status"] == "ready"


def test_pack_255_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_batch_close_readiness_v255")

    bridge = mod.build_pack_255_status_bridge()
    assert bridge["pack"] == "255"
    assert bridge["pack_number"] == 255
    ...
    assert bridge["close_check_count"] >= 11
    assert bridge["save_manifest_preview_count"] >= 11
    assert bridge["continuous_assurance_ready_to_continue"] is True
    assert bridge["safe_to_continue_to_pack_256"] is True

    prep = mod.prepare_pack_256_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_queue()
    assert prep["ready"] is True
    assert prep["next_pack"] == "256"
...
def test_pack_255_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-batch-close-readiness-v255.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-batch-close-readiness-v255.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "255"
