"""
SEARCHABLE LABEL: TOWER_PACK_367_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_FINAL_CLOSEOUT_TESTS
"""
...
def test_pack_367_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_v367")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()

    assert payload["pack"] == "367"
    assert payload["pack_number"] == 367
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-detail-drawer-v367.json"
    assert payload["tower_sublayer"] == "Handoff Policy Route Enforcement Owner Acceptance Final Closeout layer"
    assert payload["source_pack"] == "366"
    assert payload["source_closed_batch"] == "361-365"
    assert payload["save_batch"] == "366-370"
    assert payload["save_after_pack"] == 370
    assert payload["next_pack"] == "368"
    ...
    assert payload["safe_to_continue_to_pack_368"] is True
    assert payload["prepare_pack_368_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_note_draft"]["pack"] == "368"


def test_pack_367_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_v367")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()
    summary = payload["owner_acceptance_final_closeout_detail_drawer_summary"]
    ...
    assert summary["owner_acceptance_final_closeout_detail_drawer_ready"] is True
...
def test_pack_367_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_v367")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()

    cards = payload["owner_acceptance_final_closeout_detail_drawer_cards"]
    fields = payload["owner_acceptance_final_closeout_detail_drawer_fields"]
    actions = payload["owner_acceptance_final_closeout_detail_drawer_actions"]
    checkpoints = payload["owner_acceptance_final_closeout_detail_drawer_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-detail-drawer-v367.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-closeout-detail-drawer-v367.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "367"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["final_closeout_preview_only"] is True
        assert data["safe_to_continue_to_pack_368"] is True


def test_pack_367_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_v367")

    bridge = mod.build_pack_367_status_bridge()
    assert bridge["pack"] == "367"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_final_closeout_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_368"] is True

    prep = mod.prepare_pack_368_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "368"
    assert prep["source_pack"] == "367"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_closeout_detail_drawer_preview()
