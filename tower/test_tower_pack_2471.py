...
def test_pack_2471_operations_contract():
    payload = build_ir_cert_p2471_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Corruption Detection and Recovery'
    assert payload["corruption_recovery_ready"] is True
