...
def test_pack_2464_operations_contract():
    payload = build_ir_cert_p2464_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Schema Migration and Version Gate'
    assert payload["schema_version_gate_ready"] is True
