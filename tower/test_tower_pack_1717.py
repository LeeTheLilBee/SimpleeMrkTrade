"""
SEARCHABLE LABEL:
TOWER_PACK_1717_BETA_OPERATIONS_READINESS_TESTS
"""

import importlib


def test_pack_1717_ready():
    mod = importlib.import_module("tower.tower_tower_beta_operations_readiness_batch_close_readiness_v1717")
    payload = mod.build_tower_beta_operations_readiness_batch_close_readiness_preview()

    assert payload["pack"] == "1717"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-operations-readiness-batch-close-readiness-v1717.json"
    assert payload["source_pack"] == "1716"
    assert payload["current_packs"] == "1708-1757"
    assert payload["save_block"] == "1707-1757"
    assert payload["next_pack"] == "1718"
    ...
    assert payload["safe_to_continue_to_pack_1718"] is True


def test_pack_1717_protocol_authority_and_locks():
    mod = importlib.import_module("tower.tower_tower_beta_operations_readiness_batch_close_readiness_v1717")
    payload = mod.build_tower_beta_operations_readiness_batch_close_readiness_preview()
    ...
    assert summary["tower_beta_operations_readiness_batch_close_readiness_ready"] is True
...
def test_pack_1717_bridge_prep_copy():
    mod = importlib.import_module("tower.tower_tower_beta_operations_readiness_batch_close_readiness_v1717")

    bridge = mod.build_pack_1717_status_bridge()
    prep = mod.prepare_pack_1718_tower_beta_operations_readiness_route_review_index()

    assert bridge["pack"] == "1717"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["tower_beta_operations_readiness_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_1718"] is True

    assert prep["ready"] is True
    assert prep["next_pack"] == "1718"
    assert prep["source_pack"] == "1717"

    first = mod.build_tower_beta_operations_readiness_batch_close_readiness_preview()
    second = mod.build_tower_beta_operations_readiness_batch_close_readiness_preview()
    ...
    third = mod.build_tower_beta_operations_readiness_batch_close_readiness_preview()
