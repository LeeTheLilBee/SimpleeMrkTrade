...
def test_pack_2460_persistence_contract():
    payload = build_ir_cert_p2460_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Lost-Session Resume'
    assert payload["lost_session_resume_ready"] is True
