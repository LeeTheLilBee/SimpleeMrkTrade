from tower.tower_ir_cert_p2513 import (
    build_ir_cert_p2513_preview,
)


def test_pack_2513_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2513_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Tower UI v2 Theme Contract'
    assert payload["tower_ui_v2_theme_contract_ready"] is True
