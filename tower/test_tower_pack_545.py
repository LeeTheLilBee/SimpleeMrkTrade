"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_545_TOWER_BETA_BACKBONE_FINAL_READINESS_NOTE_DRAFT_TESTS_RECURSION_SAFE
"""
...
def test_pack_545_contract_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_note_draft_v545")
    payload = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()

    assert payload["pack"] == "545"
    assert payload["pack_number"] == 545
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-backbone-final-readiness-note-draft-v545.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Tower Beta Backbone Final Readiness layer"
    assert payload["source_pack"] == "544"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "546"
    ...
    assert payload["safe_to_continue_to_pack_546"] is True
    assert payload["prepare_pack_546_tower_beta_backbone_final_readiness_note_version"]["pack"] == "546"


def test_pack_545_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_note_draft_v545")
    payload = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()
    summary = payload["tower_beta_backbone_final_readiness_note_draft_summary"]
    ...
    assert summary["tower_beta_backbone_final_readiness_note_draft_ready"] is True
...
def test_pack_545_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_note_draft_v545")
    payload = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()

    rows = payload["tower_beta_backbone_final_readiness_note_draft_rows"]
    checks = payload["tower_beta_backbone_final_readiness_note_draft_checks"]
    actions = payload["tower_beta_backbone_final_readiness_note_draft_actions"]
    ...
    bridge = mod.build_pack_545_status_bridge()
    assert bridge["pack"] == "545"
    ...
    assert bridge["tower_beta_backbone_final_readiness_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_546"] is True

    prep = mod.prepare_pack_546_tower_beta_backbone_final_readiness_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "546"
    assert prep["source_pack"] == "545"

    first = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()
    second = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_backbone_final_readiness_note_draft_preview()
