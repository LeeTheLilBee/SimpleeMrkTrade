...
def test_pack_2507_human_login_ob_contract():
    payload = build_ir_cert_p2507_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Observatory Launch Card'
    assert payload["observatory_launch_card_ready"] is True
