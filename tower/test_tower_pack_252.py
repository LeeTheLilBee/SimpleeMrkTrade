"""
SEARCHABLE LABEL: TOWER_PACK_252_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_CONTINUOUS_ASSURANCE_DETAIL_DRAWER_PREVIEW_TESTS
"""
...
def test_pack_252_continuous_assurance_detail_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()

    assert payload["pack"] == "252"
    assert payload["pack_number"] == 252
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Continuous Assurance Detail Drawer Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-detail-drawer-v252.json"
    ...
    assert payload["source_pack"] == "251"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_253"] is True
    assert payload["prepare_pack_253_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft"]["pack"] == "253"


def test_pack_252_continuous_assurance_detail_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()

    summary = payload["governance_continuous_assurance_detail_summary"]

    assert summary["source_assurance_card_count"] >= 7
    assert summary["detail_drawer_count"] >= 7
    assert summary["detail_section_count"] >= 42
    assert summary["evidence_pointer_count"] >= 28
    assert summary["detail_action_count"] >= 70
    assert summary["detail_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 7
    assert summary["blocked_action_count"] >= 63
    assert summary["redacted_section_count"] >= 7
    assert summary["redacted_pointer_count"] >= 7
    ...
    assert summary["continuous_assurance_detail_ready"] is True

    assert summary["real_continuous_assurance_write_enabled"] is False
    assert summary["real_continuous_assurance_detail_write_enabled"] is False
    assert summary["real_continuous_assurance_drawer_state_write_enabled"] is False
...
def test_pack_252_detail_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()

    drawers = payload["governance_continuous_assurance_detail_drawers"]
    sections = payload["governance_continuous_assurance_detail_sections"]
    pointers = payload["governance_continuous_assurance_evidence_pointers"]
    actions = payload["governance_continuous_assurance_detail_actions"]
    checkpoints = payload["governance_continuous_assurance_detail_checkpoints"]
...
def test_pack_252_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()
    ...
    assert safety["no_real_continuous_assurance_detail_write"] is True
    assert safety["no_real_continuous_assurance_drawer_state_write"] is True
...
def test_pack_252_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_preview()

    assert third["status"] == "ready"


def test_pack_252_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_detail_drawer_v252")

    bridge = mod.build_pack_252_status_bridge()
    assert bridge["pack"] == "252"
    assert bridge["pack_number"] == 252
    ...
    assert bridge["source_pack"] == "251"
    assert bridge["source_status"] == "ready"
    assert bridge["detail_drawer_count"] >= 7
    assert bridge["detail_section_count"] >= 42
    assert bridge["evidence_pointer_count"] >= 28
    assert bridge["detail_action_count"] >= 70
    assert bridge["continuous_assurance_detail_ready"] is True
    assert bridge["safe_to_continue_to_pack_253"] is True

    prep = mod.prepare_pack_253_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "253"
...
def test_pack_252_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-detail-drawer-v252.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-detail-drawer-v252.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "252"
