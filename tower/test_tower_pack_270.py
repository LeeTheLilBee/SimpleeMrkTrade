"""
SEARCHABLE LABEL: TOWER_PACK_270_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_HANDOFF_BATCH_CLOSE_READINESS_PREVIEW_TESTS
"""
...
def test_pack_270_handoff_batch_close_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()

    assert payload["pack"] == "270"
    assert payload["pack_number"] == 270
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Handoff Batch Close Readiness Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-handoff-batch-close-readiness-v270.json"
    ...
    assert payload["safe_to_push_packs_266_to_270"] is True
    assert payload["safe_to_continue_to_pack_271"] is True
    assert payload["prepare_pack_271_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_index"]["pack"] == "271"


def test_pack_270_handoff_batch_close_summary_ready_to_push():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()

    summary = payload["governance_handoff_batch_close_summary"]

    assert summary["source_closed_batch"] == "261-265"
    assert summary["save_batch"] == "266-270"
    assert summary["save_after_pack"] == 270
    assert summary["next_batch"] == "271-275"
    assert summary["next_pack"] == "271"
    ...
    assert summary["governance_handoff_batch_ready_to_push"] is True
...
def test_pack_270_cards_checks_manifest_and_transitions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()

    cards = payload["governance_handoff_batch_pack_cards"]
    checks = payload["governance_handoff_batch_close_checks"]
    manifest = payload["governance_handoff_save_manifest_preview"]
    transitions = payload["governance_handoff_transition_previews"]

    packs = {card["pack"] for card in cards}
    assert {"266", "267", "268", "269", "270"}.issubset(packs)
    ...
    expected_paths = {
        ...
        "tower/receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270.py",
        "tower/test_tower_pack_270.py",
        "web/app.py",
    }
...
def test_pack_270_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()
...
def test_pack_270_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_preview()

    assert third["status"] == "ready"


def test_pack_270_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_handoff_batch_close_readiness_v270")

    bridge = mod.build_pack_270_status_bridge()
    assert bridge["pack"] == "270"
    assert bridge["pack_number"] == 270
    ...
    assert bridge["next_batch"] == "271-275"
    assert bridge["next_pack"] == "271"
    assert bridge["pack_card_count"] >= 5
    assert bridge["close_check_count"] >= 15
    assert bridge["save_manifest_preview_count"] >= 11
    assert bridge["governance_handoff_batch_ready_to_push"] is True
    assert bridge["safe_to_push_packs_266_to_270"] is True
    assert bridge["safe_to_continue_to_pack_271"] is True

    prep = mod.prepare_pack_271_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "271"
    ...
    assert prep["closed_batch"] == "266-270"
    assert prep["next_batch"] == "271-275"
    assert prep["save_after_pack"] == 275
    assert prep["safe_to_continue"] is True


def test_pack_270_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-handoff-batch-close-readiness-v270.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-handoff-batch-close-readiness-v270.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "270"
        ...
        assert data["next_batch"] == "271-275"
        assert data["next_pack"] == "271"
        assert data["safe_to_push_packs_266_to_270"] is True
        assert data["safe_to_continue_to_pack_271"] is True
