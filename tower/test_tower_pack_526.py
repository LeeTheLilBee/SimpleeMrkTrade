"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_526_TOWER_CAPITAL_DEPLOYMENT_GATE_NOTE_VERSION_TESTS_RECURSION_SAFE
"""
...
def test_pack_526_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_note_version_v526")
    payload = mod.build_tower_capital_deployment_gate_note_version_preview()

    assert payload["pack"] == "526"
    assert payload["pack_number"] == 526
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-note-version-v526.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "525"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "527"
    ...
    assert payload["safe_to_continue_to_pack_527"] is True
    assert payload["prepare_pack_527_tower_capital_deployment_gate_handoff_contract"]["pack"] == "527"


def test_pack_526_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_note_version_v526")
    payload = mod.build_tower_capital_deployment_gate_note_version_preview()
    summary = payload["tower_capital_deployment_gate_note_version_summary"]
    ...
    assert summary["tower_capital_deployment_gate_note_version_ready"] is True
...
def test_pack_526_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_note_version_v526")
    payload = mod.build_tower_capital_deployment_gate_note_version_preview()

    rows = payload["tower_capital_deployment_gate_note_version_rows"]
    checks = payload["tower_capital_deployment_gate_note_version_checks"]
    actions = payload["tower_capital_deployment_gate_note_version_actions"]
    ...
    bridge = mod.build_pack_526_status_bridge()
    assert bridge["pack"] == "526"
    ...
    assert bridge["tower_capital_deployment_gate_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_527"] is True

    prep = mod.prepare_pack_527_tower_capital_deployment_gate_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "527"
    assert prep["source_pack"] == "526"

    first = mod.build_tower_capital_deployment_gate_note_version_preview()
    second = mod.build_tower_capital_deployment_gate_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_note_version_preview()
