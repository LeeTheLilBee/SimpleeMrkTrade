...
"""
PACK 182 - Owner Note Saved View Preset Detail Edit History Version Detail / Compare View.

This module sits on top of Pack 181.

Pack 181 creates preview-only edit history timelines and version cards.
...
- rollback/restore/save blocked action metadata
- all real history/version/save/restore/rollback writes blocked
...
PACK_ID = "PACK_182"
VERSION_DETAIL_COMPARE_ENDPOINT = "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-detail-compare-view.json"
...
def _load_pack_181_payload(force_refresh: bool = False) -> Dict[str, Any]:
    try:
        ...
        fn = getattr(
            mod,
            "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload",
            None,
        )
        if callable(fn):
...
def _build_compare_row(timeline: Dict[str, Any], event: Dict[str, Any], sequence: int) -> Dict[str, Any]:
    ...
    row = {
        "version_compare_row_id": f"saved_view_preset_version_detail_compare_row_{_stable_hash(identity, 18)}",
        ...
        "left_version_label": "Original Preview Snapshot",
        "right_version_label": "Draft Preview Snapshot",
        ...
        "row_status": "saved_view_preset_version_detail_compare_row_preview_ready",
        "row_result_type": "owner_note_saved_view_preset_version_detail_compare_row_preview",
...
def _build_version_detail_drawer(timeline: Dict[str, Any], sequence: int) -> Dict[str, Any]:
    ...
    original_version = next((card for card in version_cards if card.get("version_role") == "original"), version_cards[0] if version_cards else {})
    draft_version = next((card for card in version_cards if card.get("version_role") == "draft_preview"), version_cards[1] if len(version_cards) > 1 else {})
    compare_version = next((card for card in version_cards if card.get("version_role") == "compare_preview"), version_cards[2] if len(version_cards) > 2 else {})
    ...
    detail_drawer_id = f"saved_view_preset_version_detail_compare_drawer_{_stable_hash(identity, 18)}"

    drawer = {
        "version_detail_drawer_id": detail_drawer_id,
        "edit_history_timeline_id": timeline_id,
        "detail_edit_drawer_id": drawer_id,
        "source_kind": source_kind,
        "source_id": source_id,
        "sequence": int(sequence),
        "drawer_status": "saved_view_preset_version_detail_compare_drawer_preview_ready",
        "drawer_result_type": "owner_note_saved_view_preset_version_detail_compare_drawer_preview",
        "original_version_card_id": original_version.get("version_card_id"),
        "draft_version_card_id": draft_version.get("version_card_id"),
        "compare_version_card_id": compare_version.get("version_card_id"),
        ...
        "rollback_action_preview": {
            "action_id": f"saved_view_preset_version_detail_rollback_action_{_stable_hash((detail_drawer_id, 'rollback'), 18)}",
            "action_label": "Rollback to original preview",
            "target_version_card_id": original_version.get("version_card_id"),
        ...
        "restore_action_preview": {
            "action_id": f"saved_view_preset_version_detail_restore_action_{_stable_hash((detail_drawer_id, 'restore'), 18)}",
            "action_label": "Restore draft preview",
            "target_version_card_id": draft_version.get("version_card_id"),
            "allowed_in_preview": True,
            "allowed_now": False,
            "real_restore_executed": False,
            "safe_preview_only": True,
        ...
        "save_action_preview": {
            "action_id": f"saved_view_preset_version_detail_save_action_{_stable_hash((detail_drawer_id, 'save'), 18)}",
            "action_label": "Save version preview",
            "target_version_card_id": draft_version.get("version_card_id"),
            "allowed_in_preview": True,
            "allowed_now": False,
            "real_version_saved": False,
...
def _build_indexes(drawers: List[Dict[str, Any]]) -> Dict[str, Any]:
    indexes = {
    ...
    for drawer in drawers:
        ...
        indexes["version_detail_drawers_by_id"][version_detail_drawer_id] = drawer
        ...
        for row in drawer.get("compare_rows", []):
            ...
            indexes["compare_rows_by_drawer_id"].setdefault(version_detail_drawer_id, []).append(row)
            indexes["compare_rows_by_source_kind"].setdefault(source_kind, []).append(row)

            if row.get("changed") is True:
                indexes["changed_compare_rows_by_drawer_id"].setdefault(version_detail_drawer_id, []).append(row)
            else:
                indexes["unchanged_compare_rows_by_drawer_id"].setdefault(version_detail_drawer_id, []).append(row)

    return indexes


@lru_cache(maxsize=1)
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload_cached() -> Dict[str, Any]:
    pack_181 = _load_pack_181_payload(force_refresh=False)

    timelines = pack_181.get("edit_history_timelines", [])
    ...
    changed_rows = [row for row in all_compare_rows if row.get("changed") is True]
    unchanged_rows = [row for row in all_compare_rows if row.get("changed") is False]
    ...
    readiness_checks = {
        "pack_181_ready": pack_181.get("status") == "ready",
        ...
        "all_version_detail_drawers_ready": all(drawer.get("drawer_status") == "saved_view_preset_version_detail_compare_drawer_preview_ready" for drawer in version_detail_drawers),
        ...
        "all_compare_rows_ready": all(row.get("row_status") == "saved_view_preset_version_detail_compare_row_preview_ready" for row in all_compare_rows),
    ...
    return {
        ...
        **_base_flags(),
        "source_pack_181": {
            "status": pack_181.get("status"),
            "readiness_score": pack_181.get("summary", {}).get("readiness_score"),
            "edit_history_timeline_count": pack_181.get("summary", {}).get("edit_history_timeline_count"),
            "version_card_count": pack_181.get("summary", {}).get("version_card_count"),
            "field_change_event_count": pack_181.get("summary", {}).get("field_change_event_count"),
            "changed_field_event_count": pack_181.get("summary", {}).get("changed_field_event_count"),
            "unchanged_field_event_count": pack_181.get("summary", {}).get("unchanged_field_event_count"),
        },
        "summary": {
            ...
            "readiness_label": "Owner note saved view preset detail edit history version detail/compare view ready" if readiness_score == 100 else "Owner note saved view preset detail edit history version detail/compare view needs review",
        },
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload(force_refresh: bool = False) -> Dict[str, Any]:
    if force_refresh:
        build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload_cached.cache_clear()
    return copy.deepcopy(build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload_cached())


def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload(force_refresh: bool = False) -> Dict[str, Any]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge(force_refresh: bool = False) -> Dict[str, Any]:
    payload = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload(force_refresh=force_refresh)
...
def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge(force_refresh: bool = False) -> Dict[str, Any]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_quick_action() -> Dict[str, Any]:
    bridge = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge()
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_unified_owner_section() -> Dict[str, Any]:
    ...
    section = {
        ...
        "subtitle": "Preview version detail drawers, side-by-side compare rows, changed/unchanged grouping, and blocked rollback/restore/save actions.",
        "status": payload.get("status"),
        "href": VERSION_DETAIL_COMPARE_ENDPOINT,
        "cards": [
            {
                "id": "version_detail_readiness",
                "title": "Version detail readiness",
            ...
            },
            {
                ...
                "value": summary.get("version_detail_drawer_count"),
                "label": "Version detail drawers",
            },
            {
                "id": "comparison_rows",
                "title": "Compare rows",
                "value": summary.get("comparison_row_count"),
                "label": "Side-by-side compare rows",
            },
            {
                "id": "changed_rows",
                "title": "Changed rows",
                "value": summary.get("changed_compare_row_count"),
                "label": "Changed field rows",
            },
            {
                "id": "version_cards",
                "title": "Version cards",
                "value": summary.get("version_card_count"),
                "label": "Preview version cards",
            },
            {
                "id": "blocked_actions",
                "title": "Actions",
                "value": "Blocked" if checks.get("all_save_actions_blocked") and checks.get("all_rollback_actions_blocked") else "Review",
                "label": "No real save/rollback/restore",
            },
        ],
    }
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_html_section() -> str:
    section = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_unified_owner_section()
    ...
    for card in cards:
        card_html.append(
            f"""
            <article class="tower-card policy-change-approval-receipt-owner-note-saved-view-version-detail-compare-card">
    ...
    return f"""
    <section class="tower-section policy-change-approval-receipt-owner-note-saved-view-version-detail-compare-section" id="policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-detail-compare-view">
        <div class="tower-section-heading">
            <p class="tower-kicker">Pack 182</p>
            <h2>{section.get('title', 'Owner Note Saved View Version Compare')}</h2>
            <p>{section.get('subtitle', '')}</p>
            <a class="tower-link-pill" href="{VERSION_DETAIL_COMPARE_ENDPOINT}">Open saved view preset version detail/compare JSON</a>
        </div>
...
__all__ = [
    ...
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_payload",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge",
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_status_bridge",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_quick_action",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_unified_owner_section",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_detail_compare_view_html_section",
]
