...
def test_pack_2509_human_login_ob_contract():
    payload = build_ir_cert_p2509_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Observatory Step-Up Gate'
    assert payload["observatory_step_up_gate_ready"] is True
