...
def test_pack_2451_guided_run_contract():
    payload = build_ir_cert_p2451_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Walkthrough Reset and New-Run Contract'
    assert payload["walkthrough_reset_ready"] is True
