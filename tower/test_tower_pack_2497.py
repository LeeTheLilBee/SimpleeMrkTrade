...
def test_pack_2497_activation_approval_contract():
    payload = build_ir_cert_p2497_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Activation Window Preview'
    assert payload["activation_window_preview_ready"] is True
