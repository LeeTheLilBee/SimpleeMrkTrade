"""
SEARCHABLE LABEL: TOWER_PACK_288_HANDOFF_POLICY_ROUTE_ENFORCEMENT_AUDIT_EVIDENCE_NOTE_DRAFT_PREVIEW_TESTS
"""
...
def test_pack_288_evidence_note_draft_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()

    assert payload["pack"] == "288"
    assert payload["pack_number"] == 288
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-evidence-note-draft-v288.json"
    ...
    assert payload["safe_to_continue_to_pack_289"] is True
    assert payload["prepare_pack_289_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_version"]["pack"] == "289"


def test_pack_288_evidence_note_draft_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()

    summary = payload["handoff_policy_route_audit_evidence_note_draft_summary"]

    assert summary["source_detail_drawer_count"] >= 11
    assert summary["draft_card_count"] >= 11
    assert summary["draft_field_count"] >= 121
    assert summary["draft_action_count"] >= 187
    ...
    assert summary["evidence_note_draft_ready"] is True
...
def test_pack_288_cards_fields_actions_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()

    cards = payload["handoff_policy_route_audit_evidence_note_draft_cards"]
    fields = payload["handoff_policy_route_audit_evidence_note_draft_fields"]
    actions = payload["handoff_policy_route_audit_evidence_note_draft_actions"]
    checkpoints = payload["handoff_policy_route_audit_evidence_note_draft_checkpoints"]
...
def test_pack_288_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()
...
def test_pack_288_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_preview()

    assert third["status"] == "ready"


def test_pack_288_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_draft_v288")

    bridge = mod.build_pack_288_status_bridge()
    assert bridge["pack"] == "288"
    assert bridge["pack_number"] == 288
    ...
    assert bridge["source_pack"] == "287"
    assert bridge["source_closed_batch"] == "281-285"
    assert bridge["save_batch"] == "286-290"
    assert bridge["save_after_pack"] == 290
    assert bridge["next_pack"] == "289"
    assert bridge["draft_card_count"] >= 11
    assert bridge["draft_field_count"] >= 121
    assert bridge["draft_action_count"] >= 187
    assert bridge["evidence_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_289"] is True

    prep = mod.prepare_pack_289_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "289"
...
def test_pack_288_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-evidence-note-draft-v288.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-evidence-note-draft-v288.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "288"
