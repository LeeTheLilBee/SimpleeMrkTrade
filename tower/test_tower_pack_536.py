"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_536_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_NOTE_VERSION_TESTS_RECURSION_SAFE
"""
...
def test_pack_536_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_note_version_v536")
    payload = mod.build_tower_owner_override_decision_trigger_note_version_preview()

    assert payload["pack"] == "536"
    assert payload["pack_number"] == 536
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-note-version-v536.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "535"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "537"
    ...
    assert payload["safe_to_continue_to_pack_537"] is True
    assert payload["prepare_pack_537_tower_owner_override_decision_trigger_handoff_contract"]["pack"] == "537"


def test_pack_536_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_note_version_v536")
    payload = mod.build_tower_owner_override_decision_trigger_note_version_preview()
    summary = payload["tower_owner_override_decision_trigger_note_version_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_note_version_ready"] is True
...
def test_pack_536_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_note_version_v536")
    payload = mod.build_tower_owner_override_decision_trigger_note_version_preview()

    rows = payload["tower_owner_override_decision_trigger_note_version_rows"]
    checks = payload["tower_owner_override_decision_trigger_note_version_checks"]
    actions = payload["tower_owner_override_decision_trigger_note_version_actions"]
    ...
    bridge = mod.build_pack_536_status_bridge()
    assert bridge["pack"] == "536"
    ...
    assert bridge["tower_owner_override_decision_trigger_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_537"] is True

    prep = mod.prepare_pack_537_tower_owner_override_decision_trigger_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "537"
    assert prep["source_pack"] == "536"

    first = mod.build_tower_owner_override_decision_trigger_note_version_preview()
    second = mod.build_tower_owner_override_decision_trigger_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_note_version_preview()
