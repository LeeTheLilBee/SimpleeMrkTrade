"""
SEARCHABLE LABEL: TOWER_PACK_259_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_CONTINUOUS_ASSURANCE_ESCALATION_NOTE_VERSION_PREVIEW_TESTS
"""
...
def test_pack_259_escalation_note_version_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()

    assert payload["pack"] == "259"
    assert payload["pack_number"] == 259
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Continuous Assurance Escalation Note Version Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-version-v259.json"
    ...
    assert payload["source_pack"] == "258"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_260"] is True
    assert payload["prepare_pack_260_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_batch_close_readiness"]["pack"] == "260"


def test_pack_259_escalation_note_version_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()

    summary = payload["governance_continuous_assurance_escalation_note_version_summary"]

    assert summary["source_draft_card_count"] >= 8
    assert summary["version_card_count"] >= 16
    assert summary["compare_row_count"] >= 160
    assert summary["version_action_count"] >= 240
    assert summary["version_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 16
    assert summary["blocked_action_count"] >= 224
    ...
    assert summary["escalation_note_version_ready"] is True

    assert summary["real_escalation_write_enabled"] is False
    assert summary["real_escalation_note_write_enabled"] is False
    assert summary["real_escalation_note_save_enabled"] is False
    assert summary["real_escalation_note_submit_enabled"] is False
    assert summary["real_escalation_note_delete_enabled"] is False
    assert summary["real_escalation_note_version_write_enabled"] is False
    assert summary["real_escalation_note_version_restore_enabled"] is False
    assert summary["real_escalation_note_version_apply_enabled"] is False
    assert summary["real_escalation_note_version_delete_enabled"] is False
...
def test_pack_259_note_version_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()

    cards = payload["governance_continuous_assurance_escalation_note_version_cards"]
    rows = payload["governance_continuous_assurance_escalation_note_version_compare_rows"]
    actions = payload["governance_continuous_assurance_escalation_note_version_actions"]
    checkpoints = payload["governance_continuous_assurance_escalation_note_version_checkpoints"]
    ...
    assert len(blocked_actions) >= 224
...
def test_pack_259_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()
    ...
    assert safety["no_real_escalation_note_save"] is True
    assert safety["no_real_escalation_note_submit"] is True
    assert safety["no_real_escalation_note_delete"] is True
    assert safety["no_real_escalation_note_version_write"] is True
    assert safety["no_real_escalation_note_version_restore"] is True
    assert safety["no_real_escalation_note_version_apply"] is True
    assert safety["no_real_escalation_note_version_delete"] is True
...
def test_pack_259_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_preview()

    assert third["status"] == "ready"


def test_pack_259_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version_v259")

    bridge = mod.build_pack_259_status_bridge()
    assert bridge["pack"] == "259"
    assert bridge["pack_number"] == 259
    ...
    assert bridge["source_pack"] == "258"
    assert bridge["source_status"] == "ready"
    assert bridge["version_card_count"] >= 16
    assert bridge["compare_row_count"] >= 160
    assert bridge["version_action_count"] >= 240
    assert bridge["escalation_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_260"] is True

    prep = mod.prepare_pack_260_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "260"
...
def test_pack_259_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-version-v259.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-version-v259.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "259"
