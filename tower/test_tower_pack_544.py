"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_544_TOWER_BETA_BACKBONE_FINAL_READINESS_DETAIL_DRAWER_TESTS_RECURSION_SAFE
"""
...
def test_pack_544_contract_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_detail_drawer_v544")
    payload = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()

    assert payload["pack"] == "544"
    assert payload["pack_number"] == 544
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-backbone-final-readiness-detail-drawer-v544.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Tower Beta Backbone Final Readiness layer"
    assert payload["source_pack"] == "543"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "545"
    ...
    assert payload["safe_to_continue_to_pack_545"] is True
    assert payload["prepare_pack_545_tower_beta_backbone_final_readiness_note_draft"]["pack"] == "545"


def test_pack_544_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_detail_drawer_v544")
    payload = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()
    summary = payload["tower_beta_backbone_final_readiness_detail_drawer_summary"]
    ...
    assert summary["tower_beta_backbone_final_readiness_detail_drawer_ready"] is True
...
def test_pack_544_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_detail_drawer_v544")
    payload = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()

    rows = payload["tower_beta_backbone_final_readiness_detail_drawer_rows"]
    checks = payload["tower_beta_backbone_final_readiness_detail_drawer_checks"]
    actions = payload["tower_beta_backbone_final_readiness_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_544_status_bridge()
    assert bridge["pack"] == "544"
    ...
    assert bridge["tower_beta_backbone_final_readiness_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_545"] is True

    prep = mod.prepare_pack_545_tower_beta_backbone_final_readiness_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "545"
    assert prep["source_pack"] == "544"

    first = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()
    second = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_backbone_final_readiness_detail_drawer_preview()
