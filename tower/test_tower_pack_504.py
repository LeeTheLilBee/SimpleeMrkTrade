"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_504_TOWER_BROKER_SAFEGUARD_CHECKLIST_DETAIL_DRAWER_TESTS_RECURSION_SAFE
"""
...
def test_pack_504_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_detail_drawer_v504")
    payload = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()

    assert payload["pack"] == "504"
    assert payload["pack_number"] == 504
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-detail-drawer-v504.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "503"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "505"
    ...
    assert payload["safe_to_continue_to_pack_505"] is True
    assert payload["prepare_pack_505_tower_broker_safeguard_checklist_note_draft"]["pack"] == "505"


def test_pack_504_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_detail_drawer_v504")
    payload = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()
    summary = payload["tower_broker_safeguard_checklist_detail_drawer_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_detail_drawer_ready"] is True
...
def test_pack_504_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_detail_drawer_v504")
    payload = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()

    rows = payload["tower_broker_safeguard_checklist_detail_drawer_rows"]
    checks = payload["tower_broker_safeguard_checklist_detail_drawer_checks"]
    actions = payload["tower_broker_safeguard_checklist_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_504_status_bridge()
    assert bridge["pack"] == "504"
    ...
    assert bridge["tower_broker_safeguard_checklist_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_505"] is True

    prep = mod.prepare_pack_505_tower_broker_safeguard_checklist_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "505"
    assert prep["source_pack"] == "504"

    first = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()
    second = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_detail_drawer_preview()
