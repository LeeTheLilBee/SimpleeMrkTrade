...
def test_pack_2488_deployment_boundary():
    payload = build_ir_cert_p2488_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Retention Owner Decision Receipt'
    assert payload["retention_owner_decision_ready"] is True
