...
def test_pack_2467_operations_contract():
    payload = build_ir_cert_p2467_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Backup Verification'
    assert payload["backup_verification_ready"] is True
