"""
SEARCHABLE LABEL: TOWER_PACK_274_HANDOFF_EVIDENCE_ROUTE_READINESS_NOTE_VERSION_PREVIEW_TESTS
"""
...
def test_pack_274_note_version_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()

    assert payload["pack"] == "274"
    assert payload["pack_number"] == 274
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Handoff Evidence Route Readiness Note Version Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-version-v274.json"
    ...
    assert payload["source_pack"] == "273"
    ...
    assert payload["safe_to_continue_to_pack_275"] is True
    assert payload["prepare_pack_275_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_batch_close_readiness"]["pack"] == "275"


def test_pack_274_note_version_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()

    summary = payload["handoff_evidence_route_readiness_note_version_summary"]

    assert summary["source_draft_card_count"] >= 14
    assert summary["version_card_count"] >= 28
    assert summary["compare_row_count"] >= 280
    assert summary["version_action_count"] >= 448
    assert summary["version_checkpoint_count"] >= 8

    assert summary["enabled_action_count"] >= 28
    assert summary["blocked_action_count"] >= 420
    ...
    assert summary["evidence_route_readiness_note_version_ready"] is True
...
def test_pack_274_cards_rows_actions_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()

    cards = payload["handoff_evidence_route_readiness_note_version_cards"]
    rows = payload["handoff_evidence_route_readiness_note_version_compare_rows"]
    actions = payload["handoff_evidence_route_readiness_note_version_actions"]
    checkpoints = payload["handoff_evidence_route_readiness_note_version_checkpoints"]
    ...
    assert len(blocked_actions) >= 420
...
def test_pack_274_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()
...
def test_pack_274_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_preview()

    assert third["status"] == "ready"


def test_pack_274_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_note_version_v274")

    bridge = mod.build_pack_274_status_bridge()
    assert bridge["pack"] == "274"
    assert bridge["pack_number"] == 274
    ...
    assert bridge["source_pack"] == "273"
    assert bridge["source_closed_batch"] == "266-270"
    assert bridge["save_batch"] == "271-275"
    assert bridge["save_after_pack"] == 275
    assert bridge["next_pack"] == "275"
    assert bridge["version_card_count"] >= 28
    assert bridge["compare_row_count"] >= 280
    assert bridge["version_action_count"] >= 448
    assert bridge["evidence_route_readiness_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_275"] is True

    prep = mod.prepare_pack_275_receipt_chain_saved_view_owner_review_handoff_evidence_route_readiness_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "275"
...
def test_pack_274_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-version-v274.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-handoff-evidence-route-readiness-note-version-v274.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "274"
