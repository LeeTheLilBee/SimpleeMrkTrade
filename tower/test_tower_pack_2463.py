...
def test_pack_2463_operations_contract():
    payload = build_ir_cert_p2463_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Persistence Configuration Contract'
    assert payload["hosted_configuration_ready"] is True
