...
def test_pack_2473_hosted_assurance_contract():
    payload = build_ir_cert_p2473_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Runtime Environment Gate'
    assert payload["hosted_runtime_gate_ready"] is True
