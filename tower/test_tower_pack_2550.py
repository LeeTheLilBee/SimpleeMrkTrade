from tower.tower_ir_cert_p2550 import (
    build_ir_cert_p2550_preview,
)


def test_pack_2550_tower_ob_handoff_contract():
    payload = build_ir_cert_p2550_preview()

    assert payload["pack"] == "2550"
    assert payload["pack_name"] == 'Integration Evidence Drawers'
    ...
    assert payload["integration_evidence_drawers_ready"] is True
    assert payload[
