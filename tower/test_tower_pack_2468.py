...
def test_pack_2468_operations_contract():
    payload = build_ir_cert_p2468_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Controlled Restore Preview'
    assert payload["controlled_restore_ready"] is True
