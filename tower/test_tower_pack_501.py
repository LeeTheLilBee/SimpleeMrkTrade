"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_501_TOWER_BROKER_SAFEGUARD_CHECKLIST_INDEX_TESTS_RECURSION_SAFE
"""
...
def test_pack_501_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_index_v501")
    payload = mod.build_tower_broker_safeguard_checklist_index_preview()

    assert payload["pack"] == "501"
    assert payload["pack_number"] == 501
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-index-v501.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "500"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "502"
    ...
    assert payload["safe_to_continue_to_pack_502"] is True
    assert payload["prepare_pack_502_tower_broker_safeguard_checklist_registry_contract"]["pack"] == "502"


def test_pack_501_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_index_v501")
    payload = mod.build_tower_broker_safeguard_checklist_index_preview()
    summary = payload["tower_broker_safeguard_checklist_index_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_index_ready"] is True
...
def test_pack_501_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_index_v501")
    payload = mod.build_tower_broker_safeguard_checklist_index_preview()

    rows = payload["tower_broker_safeguard_checklist_index_rows"]
    checks = payload["tower_broker_safeguard_checklist_index_checks"]
    actions = payload["tower_broker_safeguard_checklist_index_actions"]
    ...
    bridge = mod.build_pack_501_status_bridge()
    assert bridge["pack"] == "501"
    ...
    assert bridge["tower_broker_safeguard_checklist_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_502"] is True

    prep = mod.prepare_pack_502_tower_broker_safeguard_checklist_registry_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "502"
    assert prep["source_pack"] == "501"

    first = mod.build_tower_broker_safeguard_checklist_index_preview()
    second = mod.build_tower_broker_safeguard_checklist_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_index_preview()
