...
def test_pack_2453_persistence_contract():
    payload = build_ir_cert_p2453_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Walkthrough Persistence Boundary'
    assert payload["persistence_boundary_ready"] is True
