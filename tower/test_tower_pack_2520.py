from tower.tower_ir_cert_p2520 import (
    build_ir_cert_p2520_preview,
)


def test_pack_2520_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2520_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Quick Launch Panel'
    assert payload["quick_launch_panel_ready"] is True
