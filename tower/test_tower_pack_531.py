"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_531_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_INDEX_TESTS_RECURSION_SAFE
"""
...
def test_pack_531_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_index_v531")
    payload = mod.build_tower_owner_override_decision_trigger_index_preview()

    assert payload["pack"] == "531"
    assert payload["pack_number"] == 531
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-index-v531.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "530"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "532"
    ...
    assert payload["safe_to_continue_to_pack_532"] is True
    assert payload["prepare_pack_532_tower_owner_override_decision_trigger_registry_contract"]["pack"] == "532"


def test_pack_531_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_index_v531")
    payload = mod.build_tower_owner_override_decision_trigger_index_preview()
    summary = payload["tower_owner_override_decision_trigger_index_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_index_ready"] is True
...
def test_pack_531_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_index_v531")
    payload = mod.build_tower_owner_override_decision_trigger_index_preview()

    rows = payload["tower_owner_override_decision_trigger_index_rows"]
    checks = payload["tower_owner_override_decision_trigger_index_checks"]
    actions = payload["tower_owner_override_decision_trigger_index_actions"]
    ...
    bridge = mod.build_pack_531_status_bridge()
    assert bridge["pack"] == "531"
    ...
    assert bridge["tower_owner_override_decision_trigger_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_532"] is True

    prep = mod.prepare_pack_532_tower_owner_override_decision_trigger_registry_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "532"
    assert prep["source_pack"] == "531"

    first = mod.build_tower_owner_override_decision_trigger_index_preview()
    second = mod.build_tower_owner_override_decision_trigger_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_index_preview()
