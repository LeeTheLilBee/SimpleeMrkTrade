from tower.tower_ir_cert_p2516 import (
    build_ir_cert_p2516_preview,
)


def test_pack_2516_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2516_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Observatory Launch Door'
    assert payload["observatory_launch_door_ready"] is True
