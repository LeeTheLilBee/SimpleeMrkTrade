...
def test_pack_2492_deployment_boundary():
    payload = build_ir_cert_p2492_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Deployment Boundary Checkpoint'
    assert payload["deployment_boundary_checkpoint_ready"] is True
