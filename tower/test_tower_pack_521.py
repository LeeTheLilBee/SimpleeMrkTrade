"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_521_TOWER_CAPITAL_DEPLOYMENT_GATE_INDEX_TESTS_RECURSION_SAFE
"""
...
def test_pack_521_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_index_v521")
    payload = mod.build_tower_capital_deployment_gate_index_preview()

    assert payload["pack"] == "521"
    assert payload["pack_number"] == 521
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-index-v521.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "520"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "522"
    ...
    assert payload["safe_to_continue_to_pack_522"] is True
    assert payload["prepare_pack_522_tower_capital_deployment_gate_registry_contract"]["pack"] == "522"


def test_pack_521_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_index_v521")
    payload = mod.build_tower_capital_deployment_gate_index_preview()
    summary = payload["tower_capital_deployment_gate_index_summary"]
    ...
    assert summary["tower_capital_deployment_gate_index_ready"] is True
...
def test_pack_521_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_index_v521")
    payload = mod.build_tower_capital_deployment_gate_index_preview()

    rows = payload["tower_capital_deployment_gate_index_rows"]
    checks = payload["tower_capital_deployment_gate_index_checks"]
    actions = payload["tower_capital_deployment_gate_index_actions"]
    ...
    bridge = mod.build_pack_521_status_bridge()
    assert bridge["pack"] == "521"
    ...
    assert bridge["tower_capital_deployment_gate_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_522"] is True

    prep = mod.prepare_pack_522_tower_capital_deployment_gate_registry_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "522"
    assert prep["source_pack"] == "521"

    first = mod.build_tower_capital_deployment_gate_index_preview()
    second = mod.build_tower_capital_deployment_gate_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_index_preview()
