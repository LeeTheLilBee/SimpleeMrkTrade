"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_513_TOWER_MANUAL_LIVE_CLEARANCE_GATE_SAFETY_MATRIX_TESTS_RECURSION_SAFE
"""
...
def test_pack_513_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_safety_matrix_v513")
    payload = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()

    assert payload["pack"] == "513"
    assert payload["pack_number"] == 513
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-safety-matrix-v513.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "512"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "514"
    ...
    assert payload["safe_to_continue_to_pack_514"] is True
    assert payload["prepare_pack_514_tower_manual_live_clearance_gate_detail_drawer"]["pack"] == "514"


def test_pack_513_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_safety_matrix_v513")
    payload = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()
    summary = payload["tower_manual_live_clearance_gate_safety_matrix_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_safety_matrix_ready"] is True
...
def test_pack_513_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_safety_matrix_v513")
    payload = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()

    rows = payload["tower_manual_live_clearance_gate_safety_matrix_rows"]
    checks = payload["tower_manual_live_clearance_gate_safety_matrix_checks"]
    actions = payload["tower_manual_live_clearance_gate_safety_matrix_actions"]
    ...
    bridge = mod.build_pack_513_status_bridge()
    assert bridge["pack"] == "513"
    ...
    assert bridge["tower_manual_live_clearance_gate_safety_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_514"] is True

    prep = mod.prepare_pack_514_tower_manual_live_clearance_gate_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "514"
    assert prep["source_pack"] == "513"

    first = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()
    second = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_safety_matrix_preview()
