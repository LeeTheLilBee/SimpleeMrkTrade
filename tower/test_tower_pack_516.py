"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_516_TOWER_MANUAL_LIVE_CLEARANCE_GATE_NOTE_VERSION_TESTS_RECURSION_SAFE
"""
...
def test_pack_516_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_version_v516")
    payload = mod.build_tower_manual_live_clearance_gate_note_version_preview()

    assert payload["pack"] == "516"
    assert payload["pack_number"] == 516
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-note-version-v516.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "515"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "517"
    ...
    assert payload["safe_to_continue_to_pack_517"] is True
    assert payload["prepare_pack_517_tower_manual_live_clearance_gate_handoff_contract"]["pack"] == "517"


def test_pack_516_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_version_v516")
    payload = mod.build_tower_manual_live_clearance_gate_note_version_preview()
    summary = payload["tower_manual_live_clearance_gate_note_version_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_note_version_ready"] is True
...
def test_pack_516_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_version_v516")
    payload = mod.build_tower_manual_live_clearance_gate_note_version_preview()

    rows = payload["tower_manual_live_clearance_gate_note_version_rows"]
    checks = payload["tower_manual_live_clearance_gate_note_version_checks"]
    actions = payload["tower_manual_live_clearance_gate_note_version_actions"]
    ...
    bridge = mod.build_pack_516_status_bridge()
    assert bridge["pack"] == "516"
    ...
    assert bridge["tower_manual_live_clearance_gate_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_517"] is True

    prep = mod.prepare_pack_517_tower_manual_live_clearance_gate_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "517"
    assert prep["source_pack"] == "516"

    first = mod.build_tower_manual_live_clearance_gate_note_version_preview()
    second = mod.build_tower_manual_live_clearance_gate_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_note_version_preview()
