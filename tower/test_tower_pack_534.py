"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_534_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_DETAIL_DRAWER_TESTS_RECURSION_SAFE
"""
...
def test_pack_534_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_detail_drawer_v534")
    payload = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()

    assert payload["pack"] == "534"
    assert payload["pack_number"] == 534
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-detail-drawer-v534.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "533"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "535"
    ...
    assert payload["safe_to_continue_to_pack_535"] is True
    assert payload["prepare_pack_535_tower_owner_override_decision_trigger_note_draft"]["pack"] == "535"


def test_pack_534_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_detail_drawer_v534")
    payload = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()
    summary = payload["tower_owner_override_decision_trigger_detail_drawer_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_detail_drawer_ready"] is True
...
def test_pack_534_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_detail_drawer_v534")
    payload = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()

    rows = payload["tower_owner_override_decision_trigger_detail_drawer_rows"]
    checks = payload["tower_owner_override_decision_trigger_detail_drawer_checks"]
    actions = payload["tower_owner_override_decision_trigger_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_534_status_bridge()
    assert bridge["pack"] == "534"
    ...
    assert bridge["tower_owner_override_decision_trigger_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_535"] is True

    prep = mod.prepare_pack_535_tower_owner_override_decision_trigger_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "535"
    assert prep["source_pack"] == "534"

    first = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()
    second = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_detail_drawer_preview()
