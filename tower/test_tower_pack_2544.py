from tower.tower_ir_cert_p2544 import (
    build_ir_cert_p2544_preview,
)


def test_pack_2544_tower_ob_handoff_contract():
    payload = build_ir_cert_p2544_preview()

    assert payload["pack"] == "2544"
    assert payload["pack_name"] == 'OB Six-Room Acceptance Package Recognition'
    ...
    assert payload["ob_six_room_acceptance_package_recognition_ready"] is True
    assert payload[
