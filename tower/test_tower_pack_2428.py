...
def test_pack_2428_walkthrough_contract():
    payload = build_ir_cert_p2428_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Walkthrough Receipt View"
    assert payload["receipt_view_ready"] is True
