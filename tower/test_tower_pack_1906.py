"""
SEARCHABLE LABEL:
TOWER_PACK_1906_INCIDENT_RESPONSE_OPERATIONS_TESTS
"""

import importlib


def test_pack_1906_ready():
    mod = importlib.import_module(
        "tower.tower_tower_beta_incident_response_operations_closeout_note_draft_v1906"
    )

    payload = mod.build_tower_beta_incident_response_operations_closeout_note_draft_preview()

    assert payload["pack"] == "1906"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-incident-response-operations-closeout-note-draft-v1906.json"
    assert payload["source_pack"] == "1905"
    assert payload["current_packs"] == "1861-1910"
    assert payload["save_block"] == "1860-1910"
    assert payload["next_pack"] == "1907"
    ...
    assert payload["safe_to_continue_to_pack_1907"] is True


def test_pack_1906_locks():
    mod = importlib.import_module(
        "tower.tower_tower_beta_incident_response_operations_closeout_note_draft_v1906"
    )
    ...
    assert summary["tower_beta_incident_response_operations_closeout_note_draft_ready"] is True
...
def test_pack_1906_bridge_prep_copy():
    mod = importlib.import_module(
        "tower.tower_tower_beta_incident_response_operations_closeout_note_draft_v1906"
    )

    bridge = mod.build_pack_1906_status_bridge()
    prep = mod.prepare_pack_1907_tower_beta_incident_response_operations_closeout_note_version()

    assert bridge["pack"] == "1906"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["tower_beta_incident_response_operations_closeout_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_1907"] is True

    assert prep["ready"] is True
    assert prep["next_pack"] == "1907"
    assert prep["source_pack"] == "1906"

    first = mod.build_tower_beta_incident_response_operations_closeout_note_draft_preview()
    second = mod.build_tower_beta_incident_response_operations_closeout_note_draft_preview()
    ...
    third = mod.build_tower_beta_incident_response_operations_closeout_note_draft_preview()
