"""
SEARCHABLE LABEL: TOWER_PACK_312_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_HANDOFF_TESTS
"""
...
def test_pack_312_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_v312")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()

    assert payload["pack"] == "312"
    assert payload["pack_number"] == 312
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-handoff-detail-drawer-v312.json"
    ...
    assert payload["safe_to_continue_to_pack_313"] is True
    assert payload["prepare_pack_313_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_note_draft"]["pack"] == "313"


def test_pack_312_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_v312")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()
    summary = payload["owner_acceptance_handoff_detail_summary"]
    ...
    assert summary["owner_acceptance_handoff_detail_ready"] is True
...
def test_pack_312_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_v312")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()

    cards = payload["owner_acceptance_handoff_detail_drawers"]
    fields = payload["owner_acceptance_handoff_detail_sections"]
    actions = payload["owner_acceptance_handoff_detail_actions"]
    checkpoints = payload["owner_acceptance_handoff_detail_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-handoff-detail-drawer-v312.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-handoff-detail-drawer-v312.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "312"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_313"] is True


def test_pack_312_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_v312")

    bridge = mod.build_pack_312_status_bridge()
    assert bridge["pack"] == "312"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_handoff_detail_ready"] is True
    assert bridge["safe_to_continue_to_pack_313"] is True

    prep = mod.prepare_pack_313_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "313"
    assert prep["source_pack"] == "312"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_handoff_detail_drawer_preview()
