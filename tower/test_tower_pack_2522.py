from tower.tower_ir_cert_p2522 import (
    build_ir_cert_p2522_preview,
)


def test_pack_2522_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2522_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Simplee Front Door Checkpoint'
    assert payload["simplee_front_door_checkpoint_ready"] is True
