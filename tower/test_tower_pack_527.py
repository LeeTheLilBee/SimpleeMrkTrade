"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_527_TOWER_CAPITAL_DEPLOYMENT_GATE_HANDOFF_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_527_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_handoff_contract_v527")
    payload = mod.build_tower_capital_deployment_gate_handoff_contract_preview()

    assert payload["pack"] == "527"
    assert payload["pack_number"] == 527
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-handoff-contract-v527.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "526"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "528"
    ...
    assert payload["safe_to_continue_to_pack_528"] is True
    assert payload["prepare_pack_528_tower_capital_deployment_gate_readiness_bridge"]["pack"] == "528"


def test_pack_527_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_handoff_contract_v527")
    payload = mod.build_tower_capital_deployment_gate_handoff_contract_preview()
    summary = payload["tower_capital_deployment_gate_handoff_contract_summary"]
    ...
    assert summary["tower_capital_deployment_gate_handoff_contract_ready"] is True
...
def test_pack_527_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_handoff_contract_v527")
    payload = mod.build_tower_capital_deployment_gate_handoff_contract_preview()

    rows = payload["tower_capital_deployment_gate_handoff_contract_rows"]
    checks = payload["tower_capital_deployment_gate_handoff_contract_checks"]
    actions = payload["tower_capital_deployment_gate_handoff_contract_actions"]
    ...
    bridge = mod.build_pack_527_status_bridge()
    assert bridge["pack"] == "527"
    ...
    assert bridge["tower_capital_deployment_gate_handoff_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_528"] is True

    prep = mod.prepare_pack_528_tower_capital_deployment_gate_readiness_bridge()
    assert prep["ready"] is True
    assert prep["next_pack"] == "528"
    assert prep["source_pack"] == "527"

    first = mod.build_tower_capital_deployment_gate_handoff_contract_preview()
    second = mod.build_tower_capital_deployment_gate_handoff_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_handoff_contract_preview()
