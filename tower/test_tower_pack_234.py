"""
SEARCHABLE LABEL: TOWER_PACK_234_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_CONTINUITY_NOTE_VERSION_PREVIEW_TESTS
"""
...
def test_pack_234_continuity_note_version_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()

    assert payload["pack"] == "234"
    assert payload["pack_number"] == 234
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Continuity Note Version Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-continuity-note-version-v234.json"
    ...
    assert payload["source_pack"] == "233"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_235"] is True
    assert payload["prepare_pack_235_receipt_chain_saved_view_owner_review_continuity_batch_close_readiness"]["pack"] == "235"


def test_pack_234_continuity_note_version_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()

    summary = payload["continuity_note_version_summary"]

    assert summary["source_draft_card_count"] >= 8
    assert summary["version_card_count"] >= 16
    assert summary["compare_row_count"] >= 96
    assert summary["version_action_count"] >= 128
    assert summary["checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 16
    assert summary["blocked_action_count"] >= 112
...
def test_pack_234_continuity_note_version_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()

    cards = payload["continuity_note_version_cards"]
    rows = payload["continuity_note_version_compare_rows"]
    actions = payload["continuity_note_version_actions"]
    checkpoints = payload["continuity_note_version_checkpoints"]
...
def test_pack_234_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()
    ...
    assert safety["no_real_continuity_note_version_compare_apply"] is True
...
def test_pack_234_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")

    first = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_continuity_note_version_preview()

    assert third["status"] == "ready"


def test_pack_234_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_note_version_v234")

    bridge = mod.build_pack_234_status_bridge()
    assert bridge["pack"] == "234"
    assert bridge["pack_number"] == 234
    ...
    assert bridge["source_pack"] == "233"
    assert bridge["source_status"] == "ready"
    assert bridge["version_card_count"] >= 16
    assert bridge["compare_row_count"] >= 96
    assert bridge["version_action_count"] >= 128
    assert bridge["continuity_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_235"] is True

    prep = mod.prepare_pack_235_receipt_chain_saved_view_owner_review_continuity_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "235"
...
def test_pack_234_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-continuity-note-version-v234.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-continuity-note-version-v234.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "234"
