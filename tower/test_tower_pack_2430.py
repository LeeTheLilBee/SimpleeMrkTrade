...
def test_pack_2430_walkthrough_contract():
    payload = build_ir_cert_p2430_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Walkthrough Failure and Denial View"
    assert payload["denial_view_ready"] is True
