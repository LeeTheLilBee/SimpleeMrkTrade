"""
SEARCHABLE LABEL: TOWER_PACK_361_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_RELEASE_CLOSEOUT_TESTS
"""
...
def test_pack_361_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_v361")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()

    assert payload["pack"] == "361"
    assert payload["pack_number"] == 361
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-release-closeout-index-v361.json"
    assert payload["tower_sublayer"] == "Handoff Policy Route Enforcement Owner Acceptance Release Closeout layer"
    assert payload["source_pack"] == "360"
    assert payload["source_closed_batch"] == "356-360"
    assert payload["save_batch"] == "361-365"
    assert payload["save_after_pack"] == 365
    assert payload["next_pack"] == "362"
    ...
    assert payload["safe_to_continue_to_pack_362"] is True
    assert payload["prepare_pack_362_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_detail_drawer"]["pack"] == "362"


def test_pack_361_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_v361")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()
    summary = payload["owner_acceptance_release_closeout_index_summary"]
    ...
    assert summary["owner_acceptance_release_closeout_index_ready"] is True
...
def test_pack_361_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_v361")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()

    cards = payload["owner_acceptance_release_closeout_index_cards"]
    fields = payload["owner_acceptance_release_closeout_index_fields"]
    actions = payload["owner_acceptance_release_closeout_index_actions"]
    checkpoints = payload["owner_acceptance_release_closeout_index_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-release-closeout-index-v361.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-release-closeout-index-v361.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "361"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["closeout_preview_only"] is True
        assert data["safe_to_continue_to_pack_362"] is True


def test_pack_361_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_v361")

    bridge = mod.build_pack_361_status_bridge()
    assert bridge["pack"] == "361"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_release_closeout_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_362"] is True

    prep = mod.prepare_pack_362_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "362"
    assert prep["source_pack"] == "361"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_release_closeout_index_preview()
