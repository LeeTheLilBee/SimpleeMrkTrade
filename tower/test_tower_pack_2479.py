...
def test_pack_2479_hosted_assurance_contract():
    payload = build_ir_cert_p2479_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Export Evidence Review'
    assert payload["export_evidence_review_ready"] is True
