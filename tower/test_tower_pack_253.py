"""
SEARCHABLE LABEL: TOWER_PACK_253_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_CONTINUOUS_ASSURANCE_NOTE_DRAFT_PREVIEW_TESTS
"""
...
def test_pack_253_continuous_assurance_note_draft_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()

    assert payload["pack"] == "253"
    assert payload["pack_number"] == 253
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Continuous Assurance Note Draft Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-note-draft-v253.json"
    ...
    assert payload["source_pack"] == "252"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_254"] is True
    assert payload["prepare_pack_254_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_version"]["pack"] == "254"


def test_pack_253_continuous_assurance_note_draft_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()

    summary = payload["governance_continuous_assurance_note_draft_summary"]

    assert summary["source_detail_drawer_count"] >= 7
    assert summary["draft_card_count"] >= 7
    assert summary["draft_field_count"] >= 63
    assert summary["draft_action_count"] >= 84
    assert summary["draft_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 7
    assert summary["blocked_action_count"] >= 77
    assert summary["editable_field_count"] >= 28
    assert summary["locked_field_count"] >= 35
    ...
    assert summary["continuous_assurance_note_draft_ready"] is True
...
def test_pack_253_note_draft_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()

    cards = payload["governance_continuous_assurance_note_draft_cards"]
    fields = payload["governance_continuous_assurance_note_draft_fields"]
    actions = payload["governance_continuous_assurance_note_draft_actions"]
    checkpoints = payload["governance_continuous_assurance_note_draft_checkpoints"]
...
def test_pack_253_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()
...
def test_pack_253_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_preview()

    assert third["status"] == "ready"


def test_pack_253_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_draft_v253")

    bridge = mod.build_pack_253_status_bridge()
    assert bridge["pack"] == "253"
    assert bridge["pack_number"] == 253
    ...
    assert bridge["source_pack"] == "252"
    assert bridge["source_status"] == "ready"
    assert bridge["draft_card_count"] >= 7
    assert bridge["draft_field_count"] >= 63
    assert bridge["draft_action_count"] >= 84
    assert bridge["continuous_assurance_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_254"] is True

    prep = mod.prepare_pack_254_receipt_chain_saved_view_owner_review_governance_continuous_assurance_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "254"
...
def test_pack_253_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-note-draft-v253.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-note-draft-v253.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "253"
