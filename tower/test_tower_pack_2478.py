...
def test_pack_2478_hosted_assurance_contract():
    payload = build_ir_cert_p2478_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Retention Approval Queue'
    assert payload["retention_approval_queue_ready"] is True
