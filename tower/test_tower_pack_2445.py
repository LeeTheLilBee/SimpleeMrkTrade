...
def test_pack_2445_guided_run_contract():
    payload = build_ir_cert_p2445_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Per-Room Completion Receipt'
    assert payload["room_completion_receipt_ready"] is True
