...
"""
PACK 186 fast test - Owner Note Saved View Preset Detail Edit History Version Compare
...
def test_pack_186_payload_ready_and_preview_only():
    ...
    payload = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_payload(force_refresh=True)

    assert payload["pack_number"] == 186
    assert payload["status"] == "ready"
    assert payload["endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert payload["source_endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-preview.json"
...
def test_pack_186_timelines_version_cards_and_field_events():
    mod = importlib.import_module(
        "tower.policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview"
    )
    payload = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_payload(force_refresh=True)
    ...
    for timeline in timelines:
        assert timeline["edit_history_timeline_id"].startswith("version_compare_saved_view_filter_detail_edit_history_timeline_")
        ...
        assert timeline["timeline_status"] == "version_compare_saved_view_filter_detail_edit_history_timeline_preview_ready"
        assert timeline["timeline_result_type"] == "owner_note_version_compare_saved_view_filter_detail_edit_history_timeline_preview"
        assert timeline["version_card_count"] == 3
        assert timeline["field_change_event_count"] == 5
        assert timeline["changed_field_count"] >= 0
        assert timeline["changed_field_count"] + timeline["unchanged_field_count"] == 5
        assert timeline["active_version_card_id"]
        assert timeline["original_version_card_id"]
        assert timeline["compare_version_card_id"]

        rollback = timeline["rollback_preview"]
        restore = timeline["restore_preview"]
        compare = timeline["compare_preview"]
        ...
        assert restore["restore_allowed_in_preview"] is True
        assert restore["restore_allowed_now"] is False
        ...
        assert timeline["saved_view_preview_only"] is True
        assert timeline["filter_preset_preview_only"] is True
        ...
        assert timeline["real_saved_view_written"] is False
        assert timeline["real_user_preference_written"] is False
        assert timeline["real_filter_preference_saved"] is False
        ...
        for card in timeline["version_cards"]:
            assert card["version_card_id"].startswith("version_compare_saved_view_filter_detail_edit_version_card_")
            ...
            assert card["version_status"] == "version_compare_saved_view_filter_detail_edit_version_card_preview_ready"
            assert card["version_result_type"] == "owner_note_version_compare_saved_view_filter_detail_edit_version_card_preview"
        ...
        for event in timeline["field_change_events"]:
            assert event["field_change_event_id"].startswith("version_compare_saved_view_filter_detail_edit_field_change_event_")
            ...
            assert event["field_type"]
            assert isinstance(event["changed"], bool)
            assert event["change_status"] in {"changed_preview", "unchanged_preview"}
            assert event["event_status"] == "version_compare_saved_view_filter_detail_edit_field_change_event_preview_ready"
            assert event["event_result_type"] == "owner_note_version_compare_saved_view_filter_detail_edit_field_change_event_preview"
            ...
            assert event["detail_edit_preview_only"] is True
            ...
            assert event["real_filter_preference_saved"] is False


def test_pack_186_indexes_bridge_integrations_route_and_no_secrets():
    mod = importlib.import_module(
        "tower.policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview"
    )
    payload = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_payload(force_refresh=True)
    ...
    bridge = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_status_bridge(force_refresh=True)
    assert bridge["pack_number"] == 186
    assert bridge["status"] == "ready"
    assert bridge["readiness_score"] == 100
    assert bridge["endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json"
    ...
    action = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_quick_action()
    assert action["id"] == "policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview"
    assert action["href"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert action["status"] == "ready"

    section = mod.build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_unified_owner_section()
    assert section["section_id"] == "policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview"
    assert section["status"] == "ready"
    assert section["href"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert len(section["cards"]) >= 6

    status = importlib.import_module("tower.tower_status")
    assert hasattr(status, "build_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_status_bridge")
    tower_bridge = status.build_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_status_bridge()
    ...
    assert hasattr(qa, "build_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_quick_action")
    assert hasattr(qa, "append_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_quick_action")
    actions = qa.append_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_quick_action([])
    assert any(item.get("id") == "policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview" for item in actions)

    unified = importlib.import_module("tower.security_command_unified_owner_page")
    assert hasattr(unified, "build_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_unified_section")
    assert hasattr(unified, "append_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_section")
    sections = unified.append_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_section([])
    assert any(item.get("section_id") == "policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview" for item in sections)

    app_text = Path("web/app.py").read_text(encoding="utf-8")
    assert "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json" in app_text
    assert "tower_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_json" in app_text
    assert "_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_route_guard" in app_text

    route_text = Path("tower/ob_route_coverage_report.py").read_text(encoding="utf-8")
    assert "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-preview.json" in route_text
    assert "_pack_186_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_preview_route_guard" in route_text
