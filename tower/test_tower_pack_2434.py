...
def test_pack_2434_real_surface_contract():
    payload = build_ir_cert_p2434_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["tower_entry_navigation_ready"] is True
