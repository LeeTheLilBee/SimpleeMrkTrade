"""
SEARCHABLE LABEL: TOWER_PACK_297_HANDOFF_POLICY_ROUTE_ENFORCEMENT_FINAL_AUDIT_TESTS
"""
...
def test_pack_297_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_v297")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()

    assert payload["pack"] == "297"
    assert payload["pack_number"] == 297
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-detail-drawer-v297.json"
    ...
    assert payload["safe_to_continue_to_pack_298"] is True
    assert payload["prepare_pack_298_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_note_draft"]["pack"] == "298"


def test_pack_297_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_v297")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()
    summary = payload["final_audit_detail_summary"]
    ...
    assert summary["final_audit_detail_ready"] is True
...
def test_pack_297_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_v297")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()

    cards = payload["final_audit_detail_drawers"]
    fields = payload["final_audit_detail_sections"]
    actions = payload["final_audit_detail_actions"]
    checkpoints = payload["final_audit_detail_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-detail-drawer-v297.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-detail-drawer-v297.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "297"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_298"] is True


def test_pack_297_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_v297")

    bridge = mod.build_pack_297_status_bridge()
    assert bridge["pack"] == "297"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 13
    assert bridge["final_audit_detail_ready"] is True
    assert bridge["safe_to_continue_to_pack_298"] is True

    prep = mod.prepare_pack_298_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "298"
    assert prep["source_pack"] == "297"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_detail_drawer_preview()
