"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_505_TOWER_BROKER_SAFEGUARD_CHECKLIST_NOTE_DRAFT_TESTS_RECURSION_SAFE
"""
...
def test_pack_505_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_draft_v505")
    payload = mod.build_tower_broker_safeguard_checklist_note_draft_preview()

    assert payload["pack"] == "505"
    assert payload["pack_number"] == 505
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-note-draft-v505.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "504"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "506"
    ...
    assert payload["safe_to_continue_to_pack_506"] is True
    assert payload["prepare_pack_506_tower_broker_safeguard_checklist_note_version"]["pack"] == "506"


def test_pack_505_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_draft_v505")
    payload = mod.build_tower_broker_safeguard_checklist_note_draft_preview()
    summary = payload["tower_broker_safeguard_checklist_note_draft_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_note_draft_ready"] is True
...
def test_pack_505_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_draft_v505")
    payload = mod.build_tower_broker_safeguard_checklist_note_draft_preview()

    rows = payload["tower_broker_safeguard_checklist_note_draft_rows"]
    checks = payload["tower_broker_safeguard_checklist_note_draft_checks"]
    actions = payload["tower_broker_safeguard_checklist_note_draft_actions"]
    ...
    bridge = mod.build_pack_505_status_bridge()
    assert bridge["pack"] == "505"
    ...
    assert bridge["tower_broker_safeguard_checklist_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_506"] is True

    prep = mod.prepare_pack_506_tower_broker_safeguard_checklist_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "506"
    assert prep["source_pack"] == "505"

    first = mod.build_tower_broker_safeguard_checklist_note_draft_preview()
    second = mod.build_tower_broker_safeguard_checklist_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_note_draft_preview()
