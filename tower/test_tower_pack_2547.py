from tower.tower_ir_cert_p2547 import (
    build_ir_cert_p2547_preview,
)


def test_pack_2547_tower_ob_handoff_contract():
    payload = build_ir_cert_p2547_preview()

    assert payload["pack"] == "2547"
    assert payload["pack_name"] == 'Six-Room Walkthrough Integration Surface'
    ...
    assert payload["six_room_walkthrough_integration_surface_ready"] is True
    assert payload[
