"""
SEARCHABLE LABEL: TOWER_PACK_242_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_DETAIL_DRAWER_PREVIEW_TESTS
"""
...
def test_pack_242_governance_detail_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()

    assert payload["pack"] == "242"
    assert payload["pack_number"] == 242
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Detail Drawer Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-detail-drawer-v242.json"
    ...
    assert payload["source_pack"] == "241"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_243"] is True
    assert payload["prepare_pack_243_receipt_chain_saved_view_owner_review_governance_note_draft"]["pack"] == "243"


def test_pack_242_governance_detail_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()

    summary = payload["governance_detail_summary"]

    assert summary["source_governance_card_count"] >= 8
    assert summary["detail_drawer_count"] >= 8
    assert summary["detail_section_count"] >= 40
    assert summary["policy_pointer_count"] >= 24
    assert summary["detail_action_count"] >= 48
    ...
    assert summary["all_headers_no_writes"] is True
    ...
    assert summary["all_pointers_no_mutation"] is True
    ...
    assert summary["governance_detail_ready"] is True

    assert summary["real_governance_detail_state_write_enabled"] is False
...
def test_pack_242_governance_detail_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()

    headers = payload["governance_detail_headers"]
    sections = payload["governance_detail_sections"]
    pointers = payload["governance_detail_policy_pointers"]
    actions = payload["governance_detail_actions"]
    checkpoints = payload["governance_detail_checkpoints"]
    ...
    assert all(header["writes_state"] is False for header in headers)
    ...
    assert all(pointer["mutation_allowed"] is False for pointer in pointers)
...
def test_pack_242_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()
...
def test_pack_242_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_detail_drawer_preview()

    assert third["status"] == "ready"


def test_pack_242_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_detail_drawer_v242")

    bridge = mod.build_pack_242_status_bridge()
    assert bridge["pack"] == "242"
    assert bridge["pack_number"] == 242
    ...
    assert bridge["source_pack"] == "241"
    assert bridge["source_status"] == "ready"
    assert bridge["detail_drawer_count"] >= 8
    assert bridge["detail_section_count"] >= 40
    assert bridge["policy_pointer_count"] >= 24
    assert bridge["detail_action_count"] >= 48
    assert bridge["governance_detail_ready"] is True
    assert bridge["safe_to_continue_to_pack_243"] is True

    prep = mod.prepare_pack_243_receipt_chain_saved_view_owner_review_governance_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "243"
...
def test_pack_242_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-detail-drawer-v242.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-detail-drawer-v242.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "242"
