...
"""
PACK 181 - Owner Note Saved View Preset Detail Edit History / Version Preview.

This module sits on top of Pack 180.

Pack 180 creates preview-only detail/edit drawers for saved view/filter presets.
Pack 181 creates preview-only edit history/version scaffolding:
...
PACK_ID = "PACK_181"
EDIT_HISTORY_VERSION_ENDPOINT = "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-preview.json"
...
def _load_pack_180_payload(force_refresh: bool = False) -> Dict[str, Any]:
    try:
...
def _value_raw(value_payload: Any) -> Any:
...
def _build_field_change_event(drawer: Dict[str, Any], row: Dict[str, Any], sequence: int) -> Dict[str, Any]:
    ...
    event = {
        "field_change_event_id": f"saved_view_preset_detail_edit_field_change_event_{_stable_hash(identity, 18)}",
        ...
        "event_status": "saved_view_preset_detail_edit_field_change_event_preview_ready",
        "event_result_type": "owner_note_saved_view_preset_detail_edit_field_change_event_preview",
...
def _build_version_card(drawer: Dict[str, Any], version_number: int, changed_events: List[Dict[str, Any]]) -> Dict[str, Any]:
    drawer_id = _safe_text(drawer.get("detail_edit_drawer_id"))
    source_kind = _safe_text(drawer.get("source_kind"))
    source_id = _safe_text(drawer.get("source_id"))
    ...
    changed_count = len([event for event in changed_events if event.get("changed") is True])
    unchanged_count = len([event for event in changed_events if event.get("changed") is False])

    if version_number == 1:
        label = "Original Preview Snapshot"
        version_role = "original"
        active = False
        field_change_ids = []
    elif version_number == 2:
        label = "Draft Preview Snapshot"
        version_role = "draft_preview"
        active = True
        field_change_ids = [event.get("field_change_event_id") for event in changed_events if event.get("changed") is True]
    else:
        label = "Compare Preview Snapshot"
        version_role = "compare_preview"
        active = False
        field_change_ids = [event.get("field_change_event_id") for event in changed_events]

    card = {
        "version_card_id": f"saved_view_preset_detail_edit_version_card_{_stable_hash(identity, 18)}",
        "detail_edit_drawer_id": drawer_id,
        "source_kind": source_kind,
        "source_id": source_id,
        ...
        "version_status": "saved_view_preset_detail_edit_version_card_preview_ready",
        "version_result_type": "owner_note_saved_view_preset_detail_edit_version_card_preview",
...
def _build_edit_history_timeline(drawer: Dict[str, Any], sequence: int) -> Dict[str, Any]:
    ...
    version_cards = [
        _build_version_card(drawer, version_number=1, changed_events=field_events),
        _build_version_card(drawer, version_number=2, changed_events=field_events),
        _build_version_card(drawer, version_number=3, changed_events=field_events),
    ]
    ...
    timeline = {
        "edit_history_timeline_id": f"saved_view_preset_detail_edit_history_timeline_{_stable_hash(identity, 18)}",
        "detail_edit_drawer_id": drawer_id,
        "source_kind": source_kind,
        "source_id": source_id,
        "sequence": int(sequence),
        "timeline_status": "saved_view_preset_detail_edit_history_timeline_preview_ready",
        "timeline_result_type": "owner_note_saved_view_preset_detail_edit_history_timeline_preview",
        ...
        "rollback_preview": {
            "rollback_preview_id": f"saved_view_preset_detail_edit_rollback_preview_{_stable_hash((drawer_id, source_id, 'rollback'), 18)}",
            "detail_edit_drawer_id": drawer_id,
            "source_kind": source_kind,
            "source_id": source_id,
        ...
        "restore_preview": {
            "restore_preview_id": f"saved_view_preset_detail_edit_restore_preview_{_stable_hash((drawer_id, source_id, 'restore'), 18)}",
            "detail_edit_drawer_id": drawer_id,
            "source_kind": source_kind,
            "source_id": source_id,
        ...
        "compare_preview": {
            "compare_preview_id": f"saved_view_preset_detail_edit_compare_preview_{_stable_hash((drawer_id, source_id, 'compare'), 18)}",
            "detail_edit_drawer_id": drawer_id,
            "source_kind": source_kind,
            "source_id": source_id,
...
def _build_indexes(timelines: List[Dict[str, Any]]) -> Dict[str, Any]:
    indexes = {
        "timelines_by_id": {},
...
@lru_cache(maxsize=1)
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload_cached() -> Dict[str, Any]:
    pack_180 = _load_pack_180_payload(force_refresh=False)

    saved_view_drawers = pack_180.get("saved_view_detail_edit_drawers", [])
    if not isinstance(saved_view_drawers, list):
        saved_view_drawers = []

    filter_preset_drawers = pack_180.get("filter_preset_detail_edit_drawers", [])
    ...
    readiness_checks = {
        "pack_180_ready": pack_180.get("status") == "ready",
        ...
        "all_timelines_ready": all(timeline.get("timeline_status") == "saved_view_preset_detail_edit_history_timeline_preview_ready" for timeline in timelines),
        ...
        "all_version_cards_ready": all(card.get("version_status") == "saved_view_preset_detail_edit_version_card_preview_ready" for card in all_version_cards),
        "all_field_change_events_ready": all(event.get("event_status") == "saved_view_preset_detail_edit_field_change_event_preview_ready" for event in all_events),
    ...
    return {
        ...
        **_base_flags(),
        "source_pack_180": {
            "status": pack_180.get("status"),
            "readiness_score": pack_180.get("summary", {}).get("readiness_score"),
            "total_detail_drawer_count": pack_180.get("summary", {}).get("total_detail_drawer_count"),
            "total_editable_row_count": pack_180.get("summary", {}).get("total_editable_row_count"),
            "changed_field_count": pack_180.get("summary", {}).get("changed_field_count"),
            "unchanged_field_count": pack_180.get("summary", {}).get("unchanged_field_count"),
        },
        "summary": {
            ...
            "readiness_label": "Owner note saved view preset detail edit history/version preview ready" if readiness_score == 100 else "Owner note saved view preset detail edit history/version preview needs review",
        },
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload(force_refresh: bool = False) -> Dict[str, Any]:
    if force_refresh:
        build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload_cached.cache_clear()
    return copy.deepcopy(build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload_cached())


def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload(force_refresh: bool = False) -> Dict[str, Any]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge(force_refresh: bool = False) -> Dict[str, Any]:
    payload = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload(force_refresh=force_refresh)
...
def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge(force_refresh: bool = False) -> Dict[str, Any]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_quick_action() -> Dict[str, Any]:
    bridge = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge()
    ...
    action = {
        ...
        "endpoint": EDIT_HISTORY_VERSION_ENDPOINT,
        "description": "Preview edit history timelines, version cards, field change events, and rollback/restore metadata with all real writes blocked.",
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_unified_owner_section() -> Dict[str, Any]:
    ...
    section = {
        ...
        "subtitle": "Preview saved view/filter preset edit history timelines, versions, change events, compare, rollback, and restore metadata.",
        "status": payload.get("status"),
        "href": EDIT_HISTORY_VERSION_ENDPOINT,
        "cards": [
            {
            ...
            },
            {
                "id": "timelines",
                "title": "Timelines",
                "value": summary.get("edit_history_timeline_count"),
                "label": "Edit history timelines",
            },
            {
                "id": "version_cards",
                "title": "Version cards",
            ...
            },
            {
                "id": "field_events",
                "title": "Field events",
                "value": summary.get("field_change_event_count"),
                "label": "Preview field change events",
            },
            {
                "id": "changed_events",
                "title": "Changed events",
                "value": summary.get("changed_field_event_count"),
                "label": "Preview changes",
            },
            {
                "id": "history_writes",
                "title": "History writes",
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_html_section() -> str:
    section = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_unified_owner_section()
    ...
    for card in cards:
        card_html.append(
            f"""
            <article class="tower-card policy-change-approval-receipt-owner-note-saved-view-history-version-card">
    ...
    return f"""
    <section class="tower-section policy-change-approval-receipt-owner-note-saved-view-history-version-section" id="policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-preview">
        <div class="tower-section-heading">
            <p class="tower-kicker">Pack 181</p>
            <h2>{section.get('title', 'Owner Note Saved View History')}</h2>
            <p>{section.get('subtitle', '')}</p>
            <a class="tower-link-pill" href="{EDIT_HISTORY_VERSION_ENDPOINT}">Open saved view preset history/version JSON</a>
        </div>
...
__all__ = [
    ...
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_payload",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge",
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_status_bridge",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_quick_action",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_unified_owner_section",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_preview_html_section",
]
