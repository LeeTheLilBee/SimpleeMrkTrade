"""
SEARCHABLE LABEL:
TOWER_PACK_1707_BETA_OPERATIONS_READINESS_INDEX_TESTS
"""

from tower.tower_beta_operations_readiness_index_v1707 import (
    build_tower_beta_operations_readiness_index_preview,
    build_pack_1707_status_bridge,
    prepare_pack_1708_tower_beta_operations_readiness_index,
)


def test_pack_1707_ready():
    payload = build_tower_beta_operations_readiness_index_preview()

    assert payload["pack"] == "1707"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["source_block"] == "1656-1706"
    assert payload["source_pack"] == "1706"
    assert payload["next_pack"] == "1708"
    assert payload["cached"] is True
    assert payload["non_recursive"] is True
    assert payload["recursion_safe"] is True
    assert payload["preview_only"] is True
    assert payload["contract_only"] is True
    assert payload["safe_to_continue_to_pack_1708"] is True


def test_pack_1707_protocol_authority():
    payload = build_tower_beta_operations_readiness_index_preview()
    ...
    assert summary["tower_beta_operations_readiness_index_ready"] is True
...
def test_pack_1707_bridge_prep_copy():
    bridge = build_pack_1707_status_bridge()
    prep = prepare_pack_1708_tower_beta_operations_readiness_index()

    assert bridge["pack"] == "1707"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["tower_beta_operations_readiness_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_1708"] is True

    assert prep["ready"] is True
    assert prep["next_pack"] == "1708"
    assert prep["source_pack"] == "1707"

    first = build_tower_beta_operations_readiness_index_preview()
    second = build_tower_beta_operations_readiness_index_preview()
    ...
    third = build_tower_beta_operations_readiness_index_preview()
