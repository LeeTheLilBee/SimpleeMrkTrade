"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_428_SHARED_CONNECTIVE_TISSUE_READINESS_BRIDGE_TESTS
"""
...
def test_pack_428_contract_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_readiness_bridge_v428")
    payload = mod.build_shared_connective_tissue_readiness_bridge_preview()

    assert payload["pack"] == "428"
    assert payload["pack_number"] == 428
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-connective-tissue-readiness-bridge-v428.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Connective Tissue layer"
    assert payload["source_pack"] == "427"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "429"
    ...
    assert payload["safe_to_continue_to_pack_429"] is True
    assert payload["prepare_pack_429_shared_connective_tissue_owner_focus_bridge"]["pack"] == "429"


def test_pack_428_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_readiness_bridge_v428")
    payload = mod.build_shared_connective_tissue_readiness_bridge_preview()
    summary = payload["shared_connective_tissue_readiness_bridge_summary"]
    ...
    assert summary["shared_connective_tissue_readiness_bridge_ready"] is True
...
def test_pack_428_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_readiness_bridge_v428")
    payload = mod.build_shared_connective_tissue_readiness_bridge_preview()

    rows = payload["shared_connective_tissue_readiness_bridge_rows"]
    checks = payload["shared_connective_tissue_readiness_bridge_checks"]
    actions = payload["shared_connective_tissue_readiness_bridge_actions"]
    ...
    bridge = mod.build_pack_428_status_bridge()
    assert bridge["pack"] == "428"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_connective_tissue_readiness_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_429"] is True

    prep = mod.prepare_pack_429_shared_connective_tissue_owner_focus_bridge()
    assert prep["ready"] is True
    assert prep["next_pack"] == "429"
    assert prep["source_pack"] == "428"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-connective-tissue-readiness-bridge-v428.json" in rules

    response = app.test_client().get("/tower/shared-connective-tissue-readiness-bridge-v428.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "428"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_429"] is True

    first = mod.build_shared_connective_tissue_readiness_bridge_preview()
    second = mod.build_shared_connective_tissue_readiness_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_connective_tissue_readiness_bridge_preview()
