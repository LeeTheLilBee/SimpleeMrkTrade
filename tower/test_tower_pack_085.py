
from __future__ import annotations

import json
import os
import sys
from pathlib import Path

PROJECT_ROOT = Path("/content/SimpleeMrkTrade_REAL_CLONE")
os.chdir(PROJECT_ROOT)

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

for name in list(sys.modules.keys()):
    if name == "tower" or name.startswith("tower.") or name == "web.app":
        sys.modules.pop(name, None)

from tower.deny_path_exposure_transition_checkpoint import build_deny_path_exposure_transition_checkpoint


def _print(title, payload=None):
    print()
    print("=" * 80)
    print(title)
    print("=" * 80)
    if payload is not None:
        print(json.dumps(payload, indent=2, sort_keys=True, default=str))


def run_tests():
    checkpoint = build_deny_path_exposure_transition_checkpoint()

    _print("DENY-PATH / EXPOSURE TRANSITION CHECKPOINT", {
        "ok": checkpoint.get("ok"),
        "readiness_score": checkpoint.get("readiness_score"),
        "readiness_label": checkpoint.get("readiness_label"),
        "proof": checkpoint.get("proof"),
        "deny_path_replacement_summary": checkpoint.get("deny_path_replacement_summary"),
        "exposure_mapping_summary": checkpoint.get("exposure_mapping_summary"),
        "next_block": checkpoint.get("next_block"),
        "current_boundary": checkpoint.get("current_boundary"),
    })

    assert checkpoint.get("ok") is True
    assert checkpoint.get("readiness_score") == 100
    assert checkpoint.get("readiness_label") == "Ready to surface deny-path and exposure mapping panels"
    assert checkpoint.get("no_secret_leakage") is True

    proof = checkpoint.get("proof", {})
    for key, value in proof.items():
        assert value is True, key

    deny = checkpoint.get("deny_path_replacement_summary", {})
    assert deny.get("ok") is True
    assert deny.get("total", 0) >= 1
    assert deny.get("verified", 0) >= 1
    assert deny.get("by_route", {}).get("/no-access", 0) >= 1

    mapping = checkpoint.get("exposure_mapping_summary", {})
    assert mapping.get("ok") is True
    assert mapping.get("total", 0) >= 1
    assert isinstance(mapping.get("counts"), dict)
    assert sum(mapping.get("counts", {}).values()) == mapping.get("total")

    no_access = checkpoint.get("no_access_proof", {})
    assert no_access.get("status_code") == 403
    for key, value in no_access.items():
        if key != "status_code":
            assert value is True, key

    private = checkpoint.get("observatory_private_proof", {})
    assert private.get("status_code") == 403
    for key, value in private.items():
        if key != "status_code":
            assert value is True, key

    built = json.dumps(checkpoint.get("built_packs", []), sort_keys=True)
    for pack in ["081", "082", "083", "084", "085"]:
        assert pack in built

    next_block = json.dumps(checkpoint.get("next_block", []), sort_keys=True)
    for pack in ["086", "087", "088", "089", "090"]:
        assert pack in next_block

    boundary = json.dumps(checkpoint.get("current_boundary", {}), sort_keys=True)
    assert "Most older deny paths are still intentionally untouched" in boundary
    assert "small verified pieces" in boundary

    serialized = json.dumps(checkpoint, sort_keys=True, default=str)
    assert "tower_keycard=" not in serialized
    assert "SHOULD_NOT_SURVIVE" not in serialized
    assert "raw_token=" not in serialized
    assert '"raw_token":' not in serialized
    assert "Bearer SHOULD_NOT_SURVIVE" not in serialized
    assert "Soulaana:" in checkpoint.get("soulaana_translation", "")

    final = {
        "pack": "085",
        "status": "passed",
        "human_reason": "Deny-path replacement receipts and exposure mapping pass are checkpointed.",
    }
    _print("PACK 085 RESULT", final)
    return final


if __name__ == "__main__":
    run_tests()
