...
def test_pack_2450_guided_run_contract():
    payload = build_ir_cert_p2450_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Completion Summary View'
    assert payload["owner_completion_summary_ready"] is True
