...
def test_pack_2511_human_login_ob_contract():
    payload = build_ir_cert_p2511_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Logout and Session Revocation'
    assert payload["logout_session_revocation_ready"] is True
