...
def test_pack_2477_hosted_assurance_contract():
    payload = build_ir_cert_p2477_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Controlled Restore Drill'
    assert payload["controlled_restore_drill_ready"] is True
