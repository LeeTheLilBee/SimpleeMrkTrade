"""
SEARCHABLE LABEL: TOWER_PACK_232_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_CONTINUITY_DETAIL_DRAWER_PREVIEW_TESTS
"""
...
def test_pack_232_continuity_detail_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()

    assert payload["pack"] == "232"
    assert payload["pack_number"] == 232
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Continuity Detail Drawer Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-continuity-detail-drawer-v232.json"
    ...
    assert payload["source_pack"] == "231"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_233"] is True
    assert payload["prepare_pack_233_receipt_chain_saved_view_owner_review_continuity_note_draft"]["pack"] == "233"


def test_pack_232_continuity_detail_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()

    summary = payload["continuity_detail_summary"]

    assert summary["source_continuity_item_count"] >= 8
    assert summary["continuity_detail_drawer_count"] >= 8
    assert summary["continuity_detail_section_count"] >= 40
    assert summary["continuity_detail_action_count"] >= 56
    assert summary["continuity_detail_evidence_pointer_count"] >= 32
    assert summary["continuity_detail_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 8
    assert summary["blocked_action_count"] >= 48
    ...
    assert summary["continuity_detail_drawer_ready"] is True
...
def test_pack_232_continuity_detail_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()

    headers = payload["continuity_detail_headers"]
    sections = payload["continuity_detail_sections"]
    actions = payload["continuity_detail_actions"]
    pointers = payload["continuity_detail_evidence_pointers"]
    checkpoints = payload["continuity_detail_checkpoints"]
    ...
    assert all(header["continuity_mode"] == "preview_only" for header in headers)
...
def test_pack_232_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")
    payload = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()
...
def test_pack_232_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")

    first = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_continuity_detail_drawer_preview()

    assert third["status"] == "ready"


def test_pack_232_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_continuity_detail_drawer_v232")

    bridge = mod.build_pack_232_status_bridge()
    assert bridge["pack"] == "232"
    assert bridge["pack_number"] == 232
    ...
    assert bridge["source_pack"] == "231"
    assert bridge["source_status"] == "ready"
    assert bridge["continuity_detail_drawer_count"] >= 8
    assert bridge["continuity_detail_section_count"] >= 40
    assert bridge["continuity_detail_action_count"] >= 56
    assert bridge["continuity_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_233"] is True

    prep = mod.prepare_pack_233_receipt_chain_saved_view_owner_review_continuity_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "233"
...
def test_pack_232_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-continuity-detail-drawer-v232.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-continuity-detail-drawer-v232.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "232"
