from tower.tower_ir_cert_p2521 import (
    build_ir_cert_p2521_preview,
)


def test_pack_2521_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2521_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hidden Evidence Drawers'
    assert payload["hidden_evidence_drawers_ready"] is True
