...
def test_pack_2481_hosted_assurance_contract():
    payload = build_ir_cert_p2481_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Operations Readiness Board'
    assert payload["operations_readiness_board_ready"] is True
