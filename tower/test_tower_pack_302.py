"""
SEARCHABLE LABEL: TOWER_PACK_302_HANDOFF_POLICY_ROUTE_ENFORCEMENT_FINAL_HANDOFF_TESTS
"""
...
def test_pack_302_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_v302")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()

    assert payload["pack"] == "302"
    assert payload["pack_number"] == 302
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-detail-drawer-v302.json"
    ...
    assert payload["safe_to_continue_to_pack_303"] is True
    assert payload["prepare_pack_303_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_draft"]["pack"] == "303"


def test_pack_302_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_v302")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()
    summary = payload["final_handoff_detail_summary"]
    ...
    assert summary["final_handoff_detail_ready"] is True
...
def test_pack_302_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_v302")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()

    cards = payload["final_handoff_detail_drawers"]
    fields = payload["final_handoff_detail_sections"]
    actions = payload["final_handoff_detail_actions"]
    checkpoints = payload["final_handoff_detail_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-detail-drawer-v302.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-detail-drawer-v302.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "302"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_303"] is True


def test_pack_302_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_v302")

    bridge = mod.build_pack_302_status_bridge()
    assert bridge["pack"] == "302"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 14
    assert bridge["final_handoff_detail_ready"] is True
    assert bridge["safe_to_continue_to_pack_303"] is True

    prep = mod.prepare_pack_303_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "303"
    assert prep["source_pack"] == "302"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_detail_drawer_preview()
