...
def test_pack_2489_deployment_boundary():
    payload = build_ir_cert_p2489_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Storage Incident Review Queue'
    assert payload["storage_incident_review_ready"] is True
