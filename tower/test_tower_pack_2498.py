...
def test_pack_2498_activation_approval_contract():
    payload = build_ir_cert_p2498_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Rollback Readiness Receipt'
    assert payload["rollback_readiness_receipt_ready"] is True
