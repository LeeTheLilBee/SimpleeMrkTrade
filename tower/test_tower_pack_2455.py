...
def test_pack_2455_persistence_contract():
    payload = build_ir_cert_p2455_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Progress Checkpoint Persistence'
    assert payload["progress_checkpoint_ready"] is True
