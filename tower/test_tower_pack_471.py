"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_471_TOWER_KILL_SWITCH_BOARD_INDEX_TESTS
"""
...
def test_pack_471_contract_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_index_v471")
    payload = mod.build_tower_kill_switch_board_index_preview()

    assert payload["pack"] == "471"
    assert payload["pack_number"] == 471
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-kill-switch-board-index-v471.json"
    ...
    assert payload["safe_to_continue_to_pack_472"] is True
    assert payload["prepare_pack_472_tower_kill_switch_board_registry_contract"]["pack"] == "472"


def test_pack_471_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_index_v471")
    payload = mod.build_tower_kill_switch_board_index_preview()
    summary = payload["tower_kill_switch_board_index_summary"]
    ...
    assert summary["tower_kill_switch_board_index_ready"] is True
...
def test_pack_471_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_index_v471")
    payload = mod.build_tower_kill_switch_board_index_preview()

    rows = payload["tower_kill_switch_board_index_rows"]
    checks = payload["tower_kill_switch_board_index_checks"]
    actions = payload["tower_kill_switch_board_index_actions"]
    ...
    bridge = mod.build_pack_471_status_bridge()
    assert bridge["pack"] == "471"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_kill_switch_board_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_472"] is True

    prep = mod.prepare_pack_472_tower_kill_switch_board_registry_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "472"
    assert prep["source_pack"] == "471"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-kill-switch-board-index-v471.json" in rules

    response = app.test_client().get("/tower/tower-kill-switch-board-index-v471.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "471"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_472"] is True

    first = mod.build_tower_kill_switch_board_index_preview()
    second = mod.build_tower_kill_switch_board_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_kill_switch_board_index_preview()
