"""
SEARCHABLE LABEL: TOWER_PACK_321_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_FINAL_LOCK_TESTS
"""
...
def test_pack_321_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_v321")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()

    assert payload["pack"] == "321"
    assert payload["pack_number"] == 321
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-lock-index-v321.json"
    ...
    assert payload["source_pack"] == "320"
    ...
    assert payload["safe_to_continue_to_pack_322"] is True
    assert payload["prepare_pack_322_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_detail_drawer"]["pack"] == "322"


def test_pack_321_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_v321")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()
    summary = payload["owner_acceptance_final_lock_index_summary"]
    ...
    assert summary["owner_acceptance_final_lock_index_ready"] is True
...
def test_pack_321_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_v321")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()

    cards = payload["owner_acceptance_final_lock_index_cards"]
    fields = payload["owner_acceptance_final_lock_index_fields"]
    actions = payload["owner_acceptance_final_lock_index_actions"]
    checkpoints = payload["owner_acceptance_final_lock_index_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-lock-index-v321.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-final-lock-index-v321.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "321"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_322"] is True


def test_pack_321_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_v321")

    bridge = mod.build_pack_321_status_bridge()
    assert bridge["pack"] == "321"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["save_now"] is True
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_final_lock_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_322"] is True

    prep = mod.prepare_pack_322_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "322"
    assert prep["source_pack"] == "321"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_final_lock_index_preview()
