"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_451_TOWER_MISSION_ACCOUNT_POLICY_REGISTRY_INDEX_TESTS
"""
...
def test_pack_451_contract_ready():
    mod = importlib.import_module("tower.tower_tower_mission_account_policy_registry_index_v451")
    payload = mod.build_tower_mission_account_policy_registry_index_preview()

    assert payload["pack"] == "451"
    assert payload["pack_number"] == 451
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-mission-account-policy-registry-index-v451.json"
    assert payload["tower_layer"] == "Giant Push 2"
    assert payload["tower_sublayer"] == "Mission Account Policy Registry layer"
    assert payload["source_pack"] == "450"
    assert payload["source_closed_batch"] == "396-450"
    assert payload["giant_push"] == "451-500"
    assert payload["next_batch"] == "501-550"
    assert payload["next_pack"] == "452"
    ...
    assert payload["safe_to_continue_to_pack_452"] is True
    assert payload["prepare_pack_452_tower_mission_account_policy_registry_registry_contract"]["pack"] == "452"


def test_pack_451_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_mission_account_policy_registry_index_v451")
    payload = mod.build_tower_mission_account_policy_registry_index_preview()
    summary = payload["tower_mission_account_policy_registry_index_summary"]
    ...
    assert summary["tower_mission_account_policy_registry_index_ready"] is True
...
def test_pack_451_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_mission_account_policy_registry_index_v451")
    payload = mod.build_tower_mission_account_policy_registry_index_preview()

    rows = payload["tower_mission_account_policy_registry_index_rows"]
    checks = payload["tower_mission_account_policy_registry_index_checks"]
    actions = payload["tower_mission_account_policy_registry_index_actions"]
    ...
    bridge = mod.build_pack_451_status_bridge()
    assert bridge["pack"] == "451"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_mission_account_policy_registry_index_ready"] is True
    assert bridge["safe_to_continue_to_pack_452"] is True

    prep = mod.prepare_pack_452_tower_mission_account_policy_registry_registry_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "452"
    assert prep["source_pack"] == "451"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-mission-account-policy-registry-index-v451.json" in rules

    response = app.test_client().get("/tower/tower-mission-account-policy-registry-index-v451.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "451"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_452"] is True

    first = mod.build_tower_mission_account_policy_registry_index_preview()
    second = mod.build_tower_mission_account_policy_registry_index_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_mission_account_policy_registry_index_preview()
