...
def test_pack_2446_guided_run_contract():
    payload = build_ir_cert_p2446_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Walkthrough Resume Contract'
    assert payload["walkthrough_resume_ready"] is True
