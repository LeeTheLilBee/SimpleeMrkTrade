...
def test_pack_2487_deployment_boundary():
    payload = build_ir_cert_p2487_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Restore Drill Evidence Ledger'
    assert payload["restore_drill_evidence_ready"] is True
