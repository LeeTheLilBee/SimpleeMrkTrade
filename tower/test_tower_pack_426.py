"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_426_SHARED_CONNECTIVE_TISSUE_NOTE_VERSION_TESTS
"""
...
def test_pack_426_contract_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_note_version_v426")
    payload = mod.build_shared_connective_tissue_note_version_preview()

    assert payload["pack"] == "426"
    assert payload["pack_number"] == 426
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-connective-tissue-note-version-v426.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Connective Tissue layer"
    assert payload["source_pack"] == "425"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "427"
    ...
    assert payload["safe_to_continue_to_pack_427"] is True
    assert payload["prepare_pack_427_shared_connective_tissue_handoff_contract"]["pack"] == "427"


def test_pack_426_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_note_version_v426")
    payload = mod.build_shared_connective_tissue_note_version_preview()
    summary = payload["shared_connective_tissue_note_version_summary"]
    ...
    assert summary["shared_connective_tissue_note_version_ready"] is True
...
def test_pack_426_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_note_version_v426")
    payload = mod.build_shared_connective_tissue_note_version_preview()

    rows = payload["shared_connective_tissue_note_version_rows"]
    checks = payload["shared_connective_tissue_note_version_checks"]
    actions = payload["shared_connective_tissue_note_version_actions"]
    ...
    bridge = mod.build_pack_426_status_bridge()
    assert bridge["pack"] == "426"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_connective_tissue_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_427"] is True

    prep = mod.prepare_pack_427_shared_connective_tissue_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "427"
    assert prep["source_pack"] == "426"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-connective-tissue-note-version-v426.json" in rules

    response = app.test_client().get("/tower/shared-connective-tissue-note-version-v426.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "426"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_427"] is True

    first = mod.build_shared_connective_tissue_note_version_preview()
    second = mod.build_shared_connective_tissue_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_connective_tissue_note_version_preview()
