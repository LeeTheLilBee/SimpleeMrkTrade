...
def test_pack_2512_human_login_ob_contract():
    payload = build_ir_cert_p2512_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Human Login-to-OB Checkpoint'
    assert payload["human_login_ob_checkpoint_ready"] is True
