"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_503_TOWER_BROKER_SAFEGUARD_CHECKLIST_SAFETY_MATRIX_TESTS_RECURSION_SAFE
"""
...
def test_pack_503_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_safety_matrix_v503")
    payload = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()

    assert payload["pack"] == "503"
    assert payload["pack_number"] == 503
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-safety-matrix-v503.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "502"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "504"
    ...
    assert payload["safe_to_continue_to_pack_504"] is True
    assert payload["prepare_pack_504_tower_broker_safeguard_checklist_detail_drawer"]["pack"] == "504"


def test_pack_503_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_safety_matrix_v503")
    payload = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()
    summary = payload["tower_broker_safeguard_checklist_safety_matrix_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_safety_matrix_ready"] is True
...
def test_pack_503_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_safety_matrix_v503")
    payload = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()

    rows = payload["tower_broker_safeguard_checklist_safety_matrix_rows"]
    checks = payload["tower_broker_safeguard_checklist_safety_matrix_checks"]
    actions = payload["tower_broker_safeguard_checklist_safety_matrix_actions"]
    ...
    bridge = mod.build_pack_503_status_bridge()
    assert bridge["pack"] == "503"
    ...
    assert bridge["tower_broker_safeguard_checklist_safety_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_504"] is True

    prep = mod.prepare_pack_504_tower_broker_safeguard_checklist_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "504"
    assert prep["source_pack"] == "503"

    first = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()
    second = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_safety_matrix_preview()
