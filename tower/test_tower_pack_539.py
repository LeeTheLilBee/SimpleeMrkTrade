"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_539_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_OWNER_FOCUS_BRIDGE_TESTS_RECURSION_SAFE
"""
...
def test_pack_539_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_owner_focus_bridge_v539")
    payload = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()

    assert payload["pack"] == "539"
    assert payload["pack_number"] == 539
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-owner-focus-bridge-v539.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "538"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "540"
    ...
    assert payload["safe_to_continue_to_pack_540"] is True
    assert payload["prepare_pack_540_tower_owner_override_decision_trigger_batch_close_readiness"]["pack"] == "540"


def test_pack_539_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_owner_focus_bridge_v539")
    payload = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()
    summary = payload["tower_owner_override_decision_trigger_owner_focus_bridge_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_owner_focus_bridge_ready"] is True
...
def test_pack_539_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_owner_focus_bridge_v539")
    payload = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()

    rows = payload["tower_owner_override_decision_trigger_owner_focus_bridge_rows"]
    checks = payload["tower_owner_override_decision_trigger_owner_focus_bridge_checks"]
    actions = payload["tower_owner_override_decision_trigger_owner_focus_bridge_actions"]
    ...
    bridge = mod.build_pack_539_status_bridge()
    assert bridge["pack"] == "539"
    ...
    assert bridge["tower_owner_override_decision_trigger_owner_focus_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_540"] is True

    prep = mod.prepare_pack_540_tower_owner_override_decision_trigger_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "540"
    assert prep["source_pack"] == "539"

    first = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()
    second = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_owner_focus_bridge_preview()
