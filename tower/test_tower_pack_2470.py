...
def test_pack_2470_operations_contract():
    payload = build_ir_cert_p2470_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Export Preview'
    assert payload["owner_export_preview_ready"] is True
