"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_512_TOWER_MANUAL_LIVE_CLEARANCE_GATE_REGISTRY_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_512_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_registry_contract_v512")
    payload = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()

    assert payload["pack"] == "512"
    assert payload["pack_number"] == 512
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-registry-contract-v512.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "511"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "513"
    ...
    assert payload["safe_to_continue_to_pack_513"] is True
    assert payload["prepare_pack_513_tower_manual_live_clearance_gate_safety_matrix"]["pack"] == "513"


def test_pack_512_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_registry_contract_v512")
    payload = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()
    summary = payload["tower_manual_live_clearance_gate_registry_contract_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_registry_contract_ready"] is True
...
def test_pack_512_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_registry_contract_v512")
    payload = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()

    rows = payload["tower_manual_live_clearance_gate_registry_contract_rows"]
    checks = payload["tower_manual_live_clearance_gate_registry_contract_checks"]
    actions = payload["tower_manual_live_clearance_gate_registry_contract_actions"]
    ...
    bridge = mod.build_pack_512_status_bridge()
    assert bridge["pack"] == "512"
    ...
    assert bridge["tower_manual_live_clearance_gate_registry_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_513"] is True

    prep = mod.prepare_pack_513_tower_manual_live_clearance_gate_safety_matrix()
    assert prep["ready"] is True
    assert prep["next_pack"] == "513"
    assert prep["source_pack"] == "512"

    first = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()
    second = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_registry_contract_preview()
