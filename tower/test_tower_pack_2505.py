...
def test_pack_2505_human_login_ob_contract():
    payload = build_ir_cert_p2505_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Tower Owner Session'
    assert payload["tower_owner_session_ready"] is True
