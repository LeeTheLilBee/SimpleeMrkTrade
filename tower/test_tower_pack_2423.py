...
def test_pack_2423_walkthrough_contract():
    payload = build_ir_cert_p2423_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Owner Walkthrough Entry Contract"
    assert payload["owner_walkthrough_entry_ready"] is True
