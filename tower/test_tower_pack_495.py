"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_495_TOWER_CAPITAL_SAFETY_ENFORCEMENT_READINESS_NOTE_DRAFT_TESTS
"""
...
def test_pack_495_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_note_draft_v495")
    payload = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()

    assert payload["pack"] == "495"
    assert payload["pack_number"] == 495
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-safety-enforcement-readiness-note-draft-v495.json"
    assert payload["tower_layer"] == "Giant Push 2"
    assert payload["tower_sublayer"] == "Capital Safety Enforcement Readiness layer"
    assert payload["source_pack"] == "494"
    assert payload["source_closed_batch"] == "396-450"
    assert payload["giant_push"] == "451-500"
    assert payload["next_batch"] == "501-550"
    assert payload["next_pack"] == "496"
    ...
    assert payload["safe_to_continue_to_pack_496"] is True
    assert payload["prepare_pack_496_tower_capital_safety_enforcement_readiness_note_version"]["pack"] == "496"


def test_pack_495_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_note_draft_v495")
    payload = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()
    summary = payload["tower_capital_safety_enforcement_readiness_note_draft_summary"]
    ...
    assert summary["tower_capital_safety_enforcement_readiness_note_draft_ready"] is True
...
def test_pack_495_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_note_draft_v495")
    payload = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()

    rows = payload["tower_capital_safety_enforcement_readiness_note_draft_rows"]
    checks = payload["tower_capital_safety_enforcement_readiness_note_draft_checks"]
    actions = payload["tower_capital_safety_enforcement_readiness_note_draft_actions"]
    ...
    bridge = mod.build_pack_495_status_bridge()
    assert bridge["pack"] == "495"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_capital_safety_enforcement_readiness_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_496"] is True

    prep = mod.prepare_pack_496_tower_capital_safety_enforcement_readiness_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "496"
    assert prep["source_pack"] == "495"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-capital-safety-enforcement-readiness-note-draft-v495.json" in rules

    response = app.test_client().get("/tower/tower-capital-safety-enforcement-readiness-note-draft-v495.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "495"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_496"] is True

    first = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()
    second = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_safety_enforcement_readiness_note_draft_preview()
