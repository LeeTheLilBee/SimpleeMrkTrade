"""
SEARCHABLE LABEL: TOWER_PACK_308_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_TESTS
"""
...
def test_pack_308_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_v308")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()

    assert payload["pack"] == "308"
    assert payload["pack_number"] == 308
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-note-draft-v308.json"
    ...
    assert payload["safe_to_continue_to_pack_309"] is True
    assert payload["prepare_pack_309_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_version"]["pack"] == "309"


def test_pack_308_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_v308")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()
    summary = payload["owner_acceptance_note_draft_summary"]
    ...
    assert summary["owner_acceptance_note_draft_ready"] is True
...
def test_pack_308_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_v308")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()

    cards = payload["owner_acceptance_note_draft_cards"]
    fields = payload["owner_acceptance_note_draft_fields"]
    actions = payload["owner_acceptance_note_draft_actions"]
    checkpoints = payload["owner_acceptance_note_draft_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-note-draft-v308.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-note-draft-v308.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "308"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_309"] is True


def test_pack_308_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_v308")

    bridge = mod.build_pack_308_status_bridge()
    assert bridge["pack"] == "308"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 14
    assert bridge["owner_acceptance_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_309"] is True

    prep = mod.prepare_pack_309_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "309"
    assert prep["source_pack"] == "308"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_note_draft_preview()
