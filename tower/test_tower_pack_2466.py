...
def test_pack_2466_operations_contract():
    payload = build_ir_cert_p2466_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Safe Backup Snapshot'
    assert payload["safe_backup_snapshot_ready"] is True
