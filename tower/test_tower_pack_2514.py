from tower.tower_ir_cert_p2514 import (
    build_ir_cert_p2514_preview,
)


def test_pack_2514_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2514_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Session Status Strip'
    assert payload["owner_session_status_strip_ready"] is True
