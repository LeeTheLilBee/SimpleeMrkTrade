...
"""
PACK 188 - Owner Note Saved View Preset Detail Edit History Version Compare
Saved View / Filter Preset Detail Edit History Version Compare Filter / Drawer Navigation Preview.

This module sits on top of Pack 187.

Pack 187 creates preview-only version detail/compare drawers.
Pack 188 creates preview-only navigation/filter scaffolding for those drawers:
- navigation items
- filter lanes
- quick filters
- grouped navigation
...
PACK_ID = "PACK_188"
COMPARE_FILTER_NAVIGATION_ENDPOINT = "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-filter-navigation.json"
...
def _load_pack_187_payload(force_refresh: bool = False) -> Dict[str, object]:
    try:
...
def _build_navigation_item(drawer: Dict[str, object], position: int, total: int, ordered: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    item = {
        "navigation_item_id": f"version_compare_saved_view_filter_detail_history_nav_item_{_stable_hash(identity, 18)}",
        ...
        "changed_compare_row_count": int(drawer.get("changed_field_count") or drawer.get("changed_compare_row_count") or 0),
        "unchanged_compare_row_count": int(drawer.get("unchanged_field_count") or drawer.get("unchanged_compare_row_count") or 0),
        ...
        "navigation_status": "version_compare_saved_view_filter_detail_history_navigation_item_preview_ready",
        "navigation_result_type": "owner_note_version_compare_saved_view_filter_detail_history_navigation_item_preview",
...
def _matches_lane(drawer: Dict[str, object], lane: Dict[str, object]) -> bool:
    ...
    if filter_type == "change_state":
        if lane.get("change_state") == "changed":
            return int(drawer.get("changed_field_count") or drawer.get("changed_compare_row_count") or 0) > 0
        if lane.get("change_state") == "unchanged":
            return int(drawer.get("unchanged_field_count") or drawer.get("unchanged_compare_row_count") or 0) > 0
...
def _build_filter_lane(lane_def: Dict[str, object], drawers: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    lane = {
        "filter_lane_preview_id": f"version_compare_saved_view_filter_detail_history_filter_lane_{_stable_hash((lane_def.get('filter_lane_id'), len(matched)), 18)}",
        ...
        "lane_status": "version_compare_saved_view_filter_detail_history_filter_lane_preview_ready",
        "lane_result_type": "owner_note_version_compare_saved_view_filter_detail_history_filter_lane_preview",
...
def _build_filter_lanes(drawers: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        "filter_lanes_preview_id": f"version_compare_saved_view_filter_detail_history_filter_lanes_{_stable_hash([lane['filter_lane_id'] for lane in lanes], 18)}",
        "filter_lanes": lanes,
        ...
        "filter_status": "version_compare_saved_view_filter_detail_history_filter_lanes_preview_ready",
        "filter_result_type": "owner_note_version_compare_saved_view_filter_detail_history_filter_lanes_preview",
        "filter_allowed_in_preview": True,
        **_base_flags(),
    }
...
def _build_quick_filter_chips(filter_lanes_payload: Dict[str, object]) -> Dict[str, object]:
    ...
    for order, lane_id in enumerate(QUICK_FILTER_IDS, start=1):
        ...
        if not isinstance(lane, dict):
            continue
        chip = {
            "quick_filter_chip_id": f"version_compare_saved_view_filter_detail_history_quick_filter_chip_{_stable_hash((lane_id, order), 18)}",
            "filter_lane_id": lane_id,
            "label": lane.get("label"),
            "order": int(order),
            "matched_drawer_count": lane.get("matched_drawer_count", 0),
            "chip_status": "version_compare_saved_view_filter_detail_history_quick_filter_chip_preview_ready",
            "chip_result_type": "owner_note_version_compare_saved_view_filter_detail_history_quick_filter_chip_preview",
            "filter_allowed_in_preview": True,
            "selection_allowed_in_preview": True,
            **_base_flags(),
        }
    ...
    return {
        "quick_filter_chips_preview_id": f"version_compare_saved_view_filter_detail_history_quick_filter_chips_{_stable_hash([chip['filter_lane_id'] for chip in chips], 18)}",
        "quick_filter_chips": chips,
        "quick_filter_chip_count": len(chips),
        "quick_filter_ids": [chip["filter_lane_id"] for chip in chips],
        "quick_filter_status": "version_compare_saved_view_filter_detail_history_quick_filter_chips_preview_ready",
        "quick_filter_result_type": "owner_note_version_compare_saved_view_filter_detail_history_quick_filter_chips_preview",
        **_base_flags(),
    }
...
def _build_grouped_navigation(nav_items: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    for item in nav_items:
        ...
        groups.setdefault(
            key,
            {
                ...
                "group_status": "version_compare_saved_view_filter_detail_history_grouped_navigation_preview_ready",
            },
        )
    ...
    return {
        "grouped_navigation_preview_id": f"version_compare_saved_view_filter_detail_history_grouped_navigation_{_stable_hash(list(groups.keys()), 18)}",
        "groups": groups,
        "group_keys": list(groups.keys()),
        "group_count": len(groups),
        "grouped_navigation_status": "version_compare_saved_view_filter_detail_history_grouped_navigation_preview_ready",
        "grouped_navigation_result_type": "owner_note_version_compare_saved_view_filter_detail_history_grouped_navigation_preview",
        **_base_flags(),
    }
...
def _build_selected_drawer_preview(nav_items: List[Dict[str, object]], drawers_by_id: Dict[str, Dict[str, object]]) -> Dict[str, object]:
    ...
    selected_payload = {
        "selected_drawer_preview_id": f"version_compare_saved_view_filter_detail_history_selected_drawer_{_stable_hash((drawer_id, selected.get('source_id')), 18)}",
        ...
        "selection_status": "version_compare_saved_view_filter_detail_history_selected_drawer_preview_ready",
        "selection_result_type": "owner_note_version_compare_saved_view_filter_detail_history_selected_drawer_preview",
        "open_allowed_in_preview": True,
        "selection_allowed_in_preview": True,
        **_base_flags(),
    }
...
@lru_cache(maxsize=1)
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload_cached() -> Dict[str, object]:
    pack_187 = _load_pack_187_payload(force_refresh=False)
    drawers = pack_187.get("version_detail_drawers", [])
    ...
    readiness_checks = {
        "pack_187_ready": pack_187.get("status") == "ready",
        ...
        "all_navigation_items_ready": all(item.get("navigation_status") == "version_compare_saved_view_filter_detail_history_navigation_item_preview_ready" for item in navigation_items),
        "filter_lanes_ready": filter_lanes_payload.get("filter_status") == "version_compare_saved_view_filter_detail_history_filter_lanes_preview_ready",
        "quick_filter_chips_ready": quick_filter_payload.get("quick_filter_status") == "version_compare_saved_view_filter_detail_history_quick_filter_chips_preview_ready",
        "grouped_navigation_ready": grouped_payload.get("grouped_navigation_status") == "version_compare_saved_view_filter_detail_history_grouped_navigation_preview_ready",
        "selected_drawer_preview_ready": selected_payload.get("selection_status") == "version_compare_saved_view_filter_detail_history_selected_drawer_preview_ready",
    ...
    return {
        ...
        **_base_flags(),
        "source_pack_187": {
            "status": pack_187.get("status"),
            "readiness_score": pack_187.get("summary", {}).get("readiness_score"),
            "version_detail_drawer_count": pack_187.get("summary", {}).get("version_detail_drawer_count"),
            "version_card_count": pack_187.get("summary", {}).get("version_card_count"),
            "comparison_row_count": pack_187.get("summary", {}).get("comparison_row_count"),
            "changed_compare_row_count": pack_187.get("summary", {}).get("changed_compare_row_count"),
            "unchanged_compare_row_count": pack_187.get("summary", {}).get("unchanged_compare_row_count"),
        },
        "summary": {
            ...
            "readiness_label": "Owner note version compare saved view/filter preset detail history filter/navigation preview ready" if readiness_score == 100 else "Owner note version compare saved view/filter preset detail history filter/navigation preview needs review",
        },
        "readiness_checks": readiness_checks,
        "filter_lane_definitions": FILTER_LANE_DEFS,
        "quick_filter_ids": QUICK_FILTER_IDS,
        "navigation_preview": {
            "navigation_preview_id": f"version_compare_saved_view_filter_detail_history_navigation_{_stable_hash([item.get('navigation_item_id') for item in navigation_items], 18)}",
            ...
            "navigation_status": "version_compare_saved_view_filter_detail_history_drawer_navigation_preview_ready",
            "navigation_result_type": "owner_note_version_compare_saved_view_filter_detail_history_drawer_navigation_preview",
            **_base_flags(),
        },
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload(force_refresh: bool = False) -> Dict[str, object]:
    if force_refresh:
        build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload_cached.cache_clear()
    return copy.deepcopy(build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload_cached())


def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload(force_refresh: bool = False) -> Dict[str, object]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge(force_refresh: bool = False) -> Dict[str, object]:
    payload = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload(force_refresh=force_refresh)
...
def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge(force_refresh: bool = False) -> Dict[str, object]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_quick_action() -> Dict[str, object]:
    bridge = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge()
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_unified_owner_section() -> Dict[str, object]:
    ...
    section = {
        ...
        "subtitle": "Preview filter lanes, drawer navigation, quick chips, grouped movement, and blocked navigation persistence for version detail compare drawers.",
        "status": payload.get("status"),
        "href": COMPARE_FILTER_NAVIGATION_ENDPOINT,
        "cards": [
            {"id": "navigation_readiness", "title": "Navigation readiness", "value": summary.get("readiness_score"), "label": summary.get("readiness_label")},
            {"id": "navigation_items", "title": "Navigation items", "value": summary.get("navigation_item_count"), "label": "Version detail drawer links"},
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_html_section() -> str:
    section = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_unified_owner_section()
    ...
    for card in cards:
        card_html.append(
            f"""
            <article class="tower-card policy-change-approval-receipt-owner-note-version-compare-filter-navigation-card">
    ...
    return f"""
    <section class="tower-section policy-change-approval-receipt-owner-note-version-compare-filter-navigation-section" id="policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-filter-navigation">
        <div class="tower-section-heading">
            <p class="tower-kicker">Pack 188</p>
            <h2>{section.get('title', 'Owner Note Version Compare Filter Navigation')}</h2>
            <p>{section.get('subtitle', '')}</p>
            <a class="tower-link-pill" href="{COMPARE_FILTER_NAVIGATION_ENDPOINT}">Open version compare filter/navigation JSON</a>
        </div>
...
__all__ = [
    ...
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_payload",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge",
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_status_bridge",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_quick_action",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_unified_owner_section",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_history_version_compare_filter_navigation_html_section",
]
