...
"""
PACK 191 fast test - Owner Note Version Compare Navigation Saved View /
Filter Preset Detail Edit History / Version Preview.
...
def test_pack_191_payload_ready_and_preview_only():
    mod = importlib.import_module("tower.owner_note_vc_nav_detail_history_v191")
    payload = mod.build_owner_note_vc_nav_detail_history_v191_payload(force_refresh=True)

    assert payload["pack_number"] == 191
    assert payload["status"] == "ready"
    assert payload["endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert payload["source_endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-preview.json"
...
def test_pack_191_timelines_version_cards_and_field_events():
    mod = importlib.import_module("tower.owner_note_vc_nav_detail_history_v191")
    payload = mod.build_owner_note_vc_nav_detail_history_v191_payload(force_refresh=True)
    ...
    saved_view_timelines = [item for item in timelines if item["source_kind"] == "saved_view_preset"]
    filter_preset_timelines = [item for item in timelines if item["source_kind"] == "filter_preset"]

    assert len(saved_view_timelines) == 6
    assert len(filter_preset_timelines) >= 9

    for timeline in timelines:
        assert timeline["edit_history_timeline_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_timeline_")
        ...
        assert timeline["timeline_status"] == "version_compare_navigation_saved_view_filter_detail_edit_history_timeline_preview_ready"
        assert timeline["timeline_result_type"] == "owner_note_version_compare_navigation_saved_view_filter_detail_edit_history_timeline_preview"
        assert timeline["version_card_count"] == 3
        assert len(timeline["version_cards"]) == 3
        assert timeline["field_change_event_count"] == 5
        assert len(timeline["field_change_events"]) == 5
        assert timeline["changed_field_count"] >= 0
        assert timeline["unchanged_field_count"] >= 0
        ...
        assert timeline["rollback_preview"]["rollback_preview_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_rollback_")
        assert timeline["rollback_preview"]["target_version_card_id"]
        assert timeline["rollback_preview"]["rollback_allowed_in_preview"] is True
        ...
        assert timeline["restore_preview"]["restore_preview_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_restore_")
        assert timeline["restore_preview"]["target_version_card_id"]
        assert timeline["restore_preview"]["restore_allowed_in_preview"] is True
        ...
        assert timeline["compare_preview"]["compare_preview_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_compare_")
        assert timeline["compare_preview"]["left_version_card_id"]
        assert timeline["compare_preview"]["right_version_card_id"]
        assert timeline["compare_preview"]["compare_allowed_in_preview"] is True
        assert timeline["compare_preview"]["changed_field_count"] == timeline["changed_field_count"]
        assert timeline["compare_preview"]["unchanged_field_count"] == timeline["unchanged_field_count"]
        ...
        assert version_roles == {"original", "draft_preview", "compare_preview"}

        version_numbers = {card["version_number"] for card in timeline["version_cards"]}
        assert version_numbers == {1, 2, 3}

        for card in timeline["version_cards"]:
            assert card["version_card_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_version_card_")
            ...
            assert card["version_label"]
            assert isinstance(card["version_snapshot"], dict)
            assert card["version_status"] == "version_compare_navigation_saved_view_filter_detail_edit_history_version_card_preview_ready"
            assert card["version_result_type"] == "owner_note_version_compare_navigation_saved_view_filter_detail_edit_history_version_card_preview"
        ...
        for event in timeline["field_change_events"]:
            assert event["field_change_event_id"].startswith("version_compare_navigation_saved_view_filter_detail_edit_history_field_change_event_")
            ...
            assert event["event_status"] == "version_compare_navigation_saved_view_filter_detail_edit_history_field_change_event_preview_ready"
            assert event["event_result_type"] == "owner_note_version_compare_navigation_saved_view_filter_detail_edit_history_field_change_event_preview"
...
def test_pack_191_indexes_bridge_integrations_route_and_no_secrets():
    mod = importlib.import_module("tower.owner_note_vc_nav_detail_history_v191")
    payload = mod.build_owner_note_vc_nav_detail_history_v191_payload(force_refresh=True)
    ...
    assert indexes["unchanged_events_by_drawer_id"]
    ...
    bridge = mod.build_owner_note_vc_nav_detail_history_v191_status_bridge(force_refresh=True)
    assert bridge["pack_number"] == 191
    assert bridge["status"] == "ready"
    assert bridge["readiness_score"] == 100
    assert bridge["endpoint"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json"
    ...
    assert bridge["unchanged_field_event_count"] >= 1
    assert bridge["rollback_preview_count"] >= 15
    assert bridge["restore_preview_count"] >= 15
    assert bridge["compare_preview_count"] >= 15

    action = mod.build_owner_note_vc_nav_detail_history_v191_quick_action()
    assert action["id"] == "owner_note_vc_nav_detail_history_v191"
    assert action["href"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert action["status"] == "ready"

    section = mod.build_owner_note_vc_nav_detail_history_v191_unified_owner_section()
    assert section["section_id"] == "owner_note_vc_nav_detail_history_v191"
    assert section["status"] == "ready"
    assert section["href"] == "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json"
    assert len(section["cards"]) >= 6

    status = importlib.import_module("tower.tower_status")
    assert hasattr(status, "build_pack_191_owner_note_vc_nav_detail_history_v191_status_bridge")
    tower_bridge = status.build_pack_191_owner_note_vc_nav_detail_history_v191_status_bridge()
    ...
    assert hasattr(qa, "build_pack_191_owner_note_vc_nav_detail_history_v191_quick_action")
    assert hasattr(qa, "append_pack_191_owner_note_vc_nav_detail_history_v191_quick_action")
    actions = qa.append_pack_191_owner_note_vc_nav_detail_history_v191_quick_action([])
    assert any(item.get("id") == "owner_note_vc_nav_detail_history_v191" for item in actions)

    unified = importlib.import_module("tower.security_command_unified_owner_page")
    assert hasattr(unified, "build_pack_191_owner_note_vc_nav_detail_history_v191_unified_section")
    assert hasattr(unified, "append_pack_191_owner_note_vc_nav_detail_history_v191_section")
    sections = unified.append_pack_191_owner_note_vc_nav_detail_history_v191_section([])
    assert any(item.get("section_id") == "owner_note_vc_nav_detail_history_v191" for item in sections)

    app_text = Path("web/app.py").read_text(encoding="utf-8")
    assert "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json" in app_text
    assert "tower_owner_note_vc_nav_detail_history_v191_json" in app_text
    assert "_pack_191_owner_note_vc_nav_detail_history_v191_route_guard" in app_text

    route_text = Path("tower/ob_route_coverage_report.py").read_text(encoding="utf-8")
    assert "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-history-version-compare-navigation-saved-view-filter-preset-detail-edit-history-version-preview.json" in route_text
    assert "_pack_191_owner_note_vc_nav_detail_history_v191_route_guard" in route_text
