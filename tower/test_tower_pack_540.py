"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_540_TOWER_OWNER_OVERRIDE_DECISION_TRIGGER_BATCH_CLOSE_READINESS_TESTS_RECURSION_SAFE
"""
...
def test_pack_540_contract_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_batch_close_readiness_v540")
    payload = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()

    assert payload["pack"] == "540"
    assert payload["pack_number"] == 540
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-owner-override-decision-trigger-batch-close-readiness-v540.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Owner Override, Decision Desk, and Trigger layer"
    assert payload["source_pack"] == "539"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "541"
    ...
    assert payload["safe_to_continue_to_pack_541"] is True
    assert payload["prepare_pack_541_tower_beta_backbone_final_readiness_index"]["pack"] == "541"


def test_pack_540_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_batch_close_readiness_v540")
    payload = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()
    summary = payload["tower_owner_override_decision_trigger_batch_close_readiness_summary"]
    ...
    assert summary["tower_owner_override_decision_trigger_batch_close_readiness_ready"] is True
...
def test_pack_540_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_owner_override_decision_trigger_batch_close_readiness_v540")
    payload = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()

    rows = payload["tower_owner_override_decision_trigger_batch_close_readiness_rows"]
    checks = payload["tower_owner_override_decision_trigger_batch_close_readiness_checks"]
    actions = payload["tower_owner_override_decision_trigger_batch_close_readiness_actions"]
    ...
    bridge = mod.build_pack_540_status_bridge()
    assert bridge["pack"] == "540"
    ...
    assert bridge["tower_owner_override_decision_trigger_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_541"] is True

    prep = mod.prepare_pack_541_tower_beta_backbone_final_readiness_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "541"
    assert prep["source_pack"] == "540"

    first = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()
    second = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_owner_override_decision_trigger_batch_close_readiness_preview()
