from tower.tower_ir_cert_p2551 import (
    build_ir_cert_p2551_preview,
)


def test_pack_2551_tower_ob_handoff_contract():
    payload = build_ir_cert_p2551_preview()

    assert payload["pack"] == "2551"
    assert payload["pack_name"] == 'Integration Cert Routes'
    ...
    assert payload["integration_cert_routes_ready"] is True
    assert payload[
