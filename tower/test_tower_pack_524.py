"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_524_TOWER_CAPITAL_DEPLOYMENT_GATE_DETAIL_DRAWER_TESTS_RECURSION_SAFE
"""
...
def test_pack_524_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_detail_drawer_v524")
    payload = mod.build_tower_capital_deployment_gate_detail_drawer_preview()

    assert payload["pack"] == "524"
    assert payload["pack_number"] == 524
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-detail-drawer-v524.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "523"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "525"
    ...
    assert payload["safe_to_continue_to_pack_525"] is True
    assert payload["prepare_pack_525_tower_capital_deployment_gate_note_draft"]["pack"] == "525"


def test_pack_524_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_detail_drawer_v524")
    payload = mod.build_tower_capital_deployment_gate_detail_drawer_preview()
    summary = payload["tower_capital_deployment_gate_detail_drawer_summary"]
    ...
    assert summary["tower_capital_deployment_gate_detail_drawer_ready"] is True
...
def test_pack_524_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_detail_drawer_v524")
    payload = mod.build_tower_capital_deployment_gate_detail_drawer_preview()

    rows = payload["tower_capital_deployment_gate_detail_drawer_rows"]
    checks = payload["tower_capital_deployment_gate_detail_drawer_checks"]
    actions = payload["tower_capital_deployment_gate_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_524_status_bridge()
    assert bridge["pack"] == "524"
    ...
    assert bridge["tower_capital_deployment_gate_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_525"] is True

    prep = mod.prepare_pack_525_tower_capital_deployment_gate_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "525"
    assert prep["source_pack"] == "524"

    first = mod.build_tower_capital_deployment_gate_detail_drawer_preview()
    second = mod.build_tower_capital_deployment_gate_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_detail_drawer_preview()
