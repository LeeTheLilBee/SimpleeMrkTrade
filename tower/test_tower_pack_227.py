"""
SEARCHABLE LABEL: TOWER_PACK_227_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_FOLLOWUP_DETAIL_DRAWER_PREVIEW_TESTS
"""
...
def test_pack_227_followup_detail_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")
    payload = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()

    assert payload["pack"] == "227"
    assert payload["pack_number"] == 227
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Follow-up Detail Drawer Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-followup-detail-drawer-v227.json"
    ...
    assert payload["source_pack"] == "226"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_228"] is True
    assert payload["prepare_pack_228_receipt_chain_saved_view_owner_review_followup_note_draft"]["pack"] == "228"


def test_pack_227_followup_detail_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")
    payload = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()

    summary = payload["followup_detail_summary"]

    assert summary["source_followup_item_count"] >= 7
    assert summary["followup_detail_drawer_count"] >= 7
    assert summary["followup_detail_section_count"] >= 28
    assert summary["followup_detail_action_count"] >= 42
    assert summary["followup_detail_evidence_pointer_count"] >= 21
    assert summary["followup_detail_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 7
    assert summary["blocked_action_count"] >= 35
    ...
    assert summary["followup_detail_drawer_ready"] is True
...
def test_pack_227_followup_detail_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")
    payload = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()

    headers = payload["followup_detail_headers"]
    sections = payload["followup_detail_sections"]
    actions = payload["followup_detail_actions"]
    pointers = payload["followup_detail_evidence_pointers"]
    checkpoints = payload["followup_detail_checkpoints"]
...
def test_pack_227_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")
    payload = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()
...
def test_pack_227_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")

    first = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_followup_detail_drawer_preview()

    assert third["status"] == "ready"


def test_pack_227_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_followup_detail_drawer_v227")

    bridge = mod.build_pack_227_status_bridge()
    assert bridge["pack"] == "227"
    assert bridge["pack_number"] == 227
    ...
    assert bridge["source_pack"] == "226"
    assert bridge["source_status"] == "ready"
    assert bridge["followup_detail_drawer_count"] >= 7
    assert bridge["followup_detail_section_count"] >= 28
    assert bridge["followup_detail_action_count"] >= 42
    assert bridge["followup_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_228"] is True

    prep = mod.prepare_pack_228_receipt_chain_saved_view_owner_review_followup_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "228"
...
def test_pack_227_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-followup-detail-drawer-v227.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-followup-detail-drawer-v227.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "227"
