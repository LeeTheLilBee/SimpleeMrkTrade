...
def test_pack_2452_guided_run_contract():
    payload = build_ir_cert_p2452_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Full Real-Surface Run Rehearsal'
    assert payload["full_real_surface_rehearsal_ready"] is True
