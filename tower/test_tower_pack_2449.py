...
def test_pack_2449_guided_run_contract():
    payload = build_ir_cert_p2449_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Six-Room Final Completion Receipt'
    assert payload["final_completion_receipt_ready"] is True
