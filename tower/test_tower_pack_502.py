"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_502_TOWER_BROKER_SAFEGUARD_CHECKLIST_REGISTRY_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_502_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_registry_contract_v502")
    payload = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()

    assert payload["pack"] == "502"
    assert payload["pack_number"] == 502
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-registry-contract-v502.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "501"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "503"
    ...
    assert payload["safe_to_continue_to_pack_503"] is True
    assert payload["prepare_pack_503_tower_broker_safeguard_checklist_safety_matrix"]["pack"] == "503"


def test_pack_502_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_registry_contract_v502")
    payload = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()
    summary = payload["tower_broker_safeguard_checklist_registry_contract_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_registry_contract_ready"] is True
...
def test_pack_502_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_registry_contract_v502")
    payload = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()

    rows = payload["tower_broker_safeguard_checklist_registry_contract_rows"]
    checks = payload["tower_broker_safeguard_checklist_registry_contract_checks"]
    actions = payload["tower_broker_safeguard_checklist_registry_contract_actions"]
    ...
    bridge = mod.build_pack_502_status_bridge()
    assert bridge["pack"] == "502"
    ...
    assert bridge["tower_broker_safeguard_checklist_registry_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_503"] is True

    prep = mod.prepare_pack_503_tower_broker_safeguard_checklist_safety_matrix()
    assert prep["ready"] is True
    assert prep["next_pack"] == "503"
    assert prep["source_pack"] == "502"

    first = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()
    second = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_registry_contract_preview()
