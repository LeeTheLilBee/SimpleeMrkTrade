...
def test_pack_2495_activation_approval_contract():
    payload = build_ir_cert_p2495_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Approval Challenge Receipt'
    assert payload["approval_challenge_receipt_ready"] is True
