"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_547_TOWER_BETA_BACKBONE_FINAL_READINESS_HANDOFF_CONTRACT_TESTS_RECURSION_SAFE
"""
...
def test_pack_547_contract_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_handoff_contract_v547")
    payload = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()

    assert payload["pack"] == "547"
    assert payload["pack_number"] == 547
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-backbone-final-readiness-handoff-contract-v547.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Tower Beta Backbone Final Readiness layer"
    assert payload["source_pack"] == "546"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "548"
    ...
    assert payload["safe_to_continue_to_pack_548"] is True
    assert payload["prepare_pack_548_tower_beta_backbone_final_readiness_readiness_bridge"]["pack"] == "548"


def test_pack_547_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_handoff_contract_v547")
    payload = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()
    summary = payload["tower_beta_backbone_final_readiness_handoff_contract_summary"]
    ...
    assert summary["tower_beta_backbone_final_readiness_handoff_contract_ready"] is True
...
def test_pack_547_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_handoff_contract_v547")
    payload = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()

    rows = payload["tower_beta_backbone_final_readiness_handoff_contract_rows"]
    checks = payload["tower_beta_backbone_final_readiness_handoff_contract_checks"]
    actions = payload["tower_beta_backbone_final_readiness_handoff_contract_actions"]
    ...
    bridge = mod.build_pack_547_status_bridge()
    assert bridge["pack"] == "547"
    ...
    assert bridge["tower_beta_backbone_final_readiness_handoff_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_548"] is True

    prep = mod.prepare_pack_548_tower_beta_backbone_final_readiness_readiness_bridge()
    assert prep["ready"] is True
    assert prep["next_pack"] == "548"
    assert prep["source_pack"] == "547"

    first = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()
    second = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_backbone_final_readiness_handoff_contract_preview()
