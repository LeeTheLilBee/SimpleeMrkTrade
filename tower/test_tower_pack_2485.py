...
def test_pack_2485_deployment_boundary():
    payload = build_ir_cert_p2485_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Startup Gate Wiring'
    assert payload["startup_gate_wiring_ready"] is True
