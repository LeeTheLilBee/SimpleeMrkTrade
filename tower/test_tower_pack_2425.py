...
def test_pack_2425_walkthrough_contract():
    payload = build_ir_cert_p2425_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Six-Room Walkthrough Manifest"
    assert payload["walkthrough_manifest_ready"] is True
