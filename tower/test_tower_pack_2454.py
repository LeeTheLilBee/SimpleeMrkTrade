...
def test_pack_2454_persistence_contract():
    payload = build_ir_cert_p2454_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Durable Guided-Run Ledger'
    assert payload["durable_run_ledger_ready"] is True
