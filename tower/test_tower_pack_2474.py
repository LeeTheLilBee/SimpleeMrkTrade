...
def test_pack_2474_hosted_assurance_contract():
    payload = build_ir_cert_p2474_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Persistence Startup Fail-Closed Contract'
    assert payload["startup_fail_closed_ready"] is True
