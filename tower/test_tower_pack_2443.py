...
def test_pack_2443_guided_run_contract():
    payload = build_ir_cert_p2443_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Guided Run Progress Contract'
    assert payload["guided_run_progress_ready"] is True
