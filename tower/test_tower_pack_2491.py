...
def test_pack_2491_deployment_boundary():
    payload = build_ir_cert_p2491_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Activation Recommendation'
    assert payload["activation_recommendation_ready"] is True
