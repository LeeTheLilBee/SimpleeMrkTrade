...
def test_pack_2469_operations_contract():
    payload = build_ir_cert_p2469_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Retention and Cleanup Preview'
    assert payload["retention_cleanup_ready"] is True
