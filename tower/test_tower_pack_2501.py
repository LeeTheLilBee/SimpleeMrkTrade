...
def test_pack_2501_activation_approval_contract():
    payload = build_ir_cert_p2501_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Activation Execution Hold'
    assert payload["activation_execution_hold_ready"] is True
