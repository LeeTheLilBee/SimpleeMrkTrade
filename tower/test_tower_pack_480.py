"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_480_TOWER_KILL_SWITCH_BOARD_BATCH_CLOSE_READINESS_TESTS
"""
...
def test_pack_480_contract_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_batch_close_readiness_v480")
    payload = mod.build_tower_kill_switch_board_batch_close_readiness_preview()

    assert payload["pack"] == "480"
    assert payload["pack_number"] == 480
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-kill-switch-board-batch-close-readiness-v480.json"
    assert payload["tower_layer"] == "Giant Push 2"
    assert payload["tower_sublayer"] == "Kill Switch Board and Emergency Halt Authority layer"
    assert payload["source_pack"] == "479"
    assert payload["source_closed_batch"] == "396-450"
    assert payload["giant_push"] == "451-500"
    assert payload["next_batch"] == "501-550"
    assert payload["next_pack"] == "481"
    assert payload["cached"] is True
    assert payload["non_recursive"] is True
    assert payload["simulation_only"] is True
    assert payload["preview_only"] is True
    assert payload["tower_kill_switch_board_preview_only"] is True
    assert payload["contract_only"] is True
    assert payload["capital_safety_preview_only"] is True
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True
    assert payload["safe_to_continue_to_pack_481"] is True
    assert payload["prepare_pack_481_tower_safety_notification_snapshot_index"]["pack"] == "481"


def test_pack_480_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_batch_close_readiness_v480")
    payload = mod.build_tower_kill_switch_board_batch_close_readiness_preview()
    summary = payload["tower_kill_switch_board_batch_close_readiness_summary"]
    ...
    assert summary["tower_kill_switch_board_batch_close_readiness_ready"] is True
...
def test_pack_480_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_batch_close_readiness_v480")
    payload = mod.build_tower_kill_switch_board_batch_close_readiness_preview()

    rows = payload["tower_kill_switch_board_batch_close_readiness_rows"]
    checks = payload["tower_kill_switch_board_batch_close_readiness_checks"]
    actions = payload["tower_kill_switch_board_batch_close_readiness_actions"]
    ...
    bridge = mod.build_pack_480_status_bridge()
    assert bridge["pack"] == "480"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_kill_switch_board_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_481"] is True

    prep = mod.prepare_pack_481_tower_safety_notification_snapshot_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "481"
    assert prep["source_pack"] == "480"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-kill-switch-board-batch-close-readiness-v480.json" in rules

    response = app.test_client().get("/tower/tower-kill-switch-board-batch-close-readiness-v480.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "480"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_481"] is True

    first = mod.build_tower_kill_switch_board_batch_close_readiness_preview()
    second = mod.build_tower_kill_switch_board_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_kill_switch_board_batch_close_readiness_preview()
