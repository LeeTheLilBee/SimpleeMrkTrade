...
def test_pack_2458_persistence_contract():
    payload = build_ir_cert_p2458_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Run History'
    assert payload["owner_run_history_ready"] is True
