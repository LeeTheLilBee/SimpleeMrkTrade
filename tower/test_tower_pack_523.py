"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_523_TOWER_CAPITAL_DEPLOYMENT_GATE_SAFETY_MATRIX_TESTS_RECURSION_SAFE
"""
...
def test_pack_523_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_safety_matrix_v523")
    payload = mod.build_tower_capital_deployment_gate_safety_matrix_preview()

    assert payload["pack"] == "523"
    assert payload["pack_number"] == 523
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-safety-matrix-v523.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "522"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "524"
    ...
    assert payload["safe_to_continue_to_pack_524"] is True
    assert payload["prepare_pack_524_tower_capital_deployment_gate_detail_drawer"]["pack"] == "524"


def test_pack_523_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_safety_matrix_v523")
    payload = mod.build_tower_capital_deployment_gate_safety_matrix_preview()
    summary = payload["tower_capital_deployment_gate_safety_matrix_summary"]
    ...
    assert summary["tower_capital_deployment_gate_safety_matrix_ready"] is True
...
def test_pack_523_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_safety_matrix_v523")
    payload = mod.build_tower_capital_deployment_gate_safety_matrix_preview()

    rows = payload["tower_capital_deployment_gate_safety_matrix_rows"]
    checks = payload["tower_capital_deployment_gate_safety_matrix_checks"]
    actions = payload["tower_capital_deployment_gate_safety_matrix_actions"]
    ...
    bridge = mod.build_pack_523_status_bridge()
    assert bridge["pack"] == "523"
    ...
    assert bridge["tower_capital_deployment_gate_safety_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_524"] is True

    prep = mod.prepare_pack_524_tower_capital_deployment_gate_detail_drawer()
    assert prep["ready"] is True
    assert prep["next_pack"] == "524"
    assert prep["source_pack"] == "523"

    first = mod.build_tower_capital_deployment_gate_safety_matrix_preview()
    second = mod.build_tower_capital_deployment_gate_safety_matrix_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_safety_matrix_preview()
