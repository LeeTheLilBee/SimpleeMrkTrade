"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_476_TOWER_KILL_SWITCH_BOARD_NOTE_VERSION_TESTS
"""
...
def test_pack_476_contract_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_note_version_v476")
    payload = mod.build_tower_kill_switch_board_note_version_preview()

    assert payload["pack"] == "476"
    assert payload["pack_number"] == 476
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-kill-switch-board-note-version-v476.json"
    ...
    assert payload["safe_to_continue_to_pack_477"] is True
    assert payload["prepare_pack_477_tower_kill_switch_board_handoff_contract"]["pack"] == "477"


def test_pack_476_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_note_version_v476")
    payload = mod.build_tower_kill_switch_board_note_version_preview()
    summary = payload["tower_kill_switch_board_note_version_summary"]
    ...
    assert summary["tower_kill_switch_board_note_version_ready"] is True
...
def test_pack_476_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_kill_switch_board_note_version_v476")
    payload = mod.build_tower_kill_switch_board_note_version_preview()

    rows = payload["tower_kill_switch_board_note_version_rows"]
    checks = payload["tower_kill_switch_board_note_version_checks"]
    actions = payload["tower_kill_switch_board_note_version_actions"]
    ...
    bridge = mod.build_pack_476_status_bridge()
    assert bridge["pack"] == "476"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_kill_switch_board_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_477"] is True

    prep = mod.prepare_pack_477_tower_kill_switch_board_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "477"
    assert prep["source_pack"] == "476"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-kill-switch-board-note-version-v476.json" in rules

    response = app.test_client().get("/tower/tower-kill-switch-board-note-version-v476.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "476"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_477"] is True

    first = mod.build_tower_kill_switch_board_note_version_preview()
    second = mod.build_tower_kill_switch_board_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_kill_switch_board_note_version_preview()
