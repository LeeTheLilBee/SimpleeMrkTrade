"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_492_TOWER_CAPITAL_SAFETY_ENFORCEMENT_READINESS_REGISTRY_CONTRACT_TESTS
"""
...
def test_pack_492_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_registry_contract_v492")
    payload = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()

    assert payload["pack"] == "492"
    assert payload["pack_number"] == 492
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-safety-enforcement-readiness-registry-contract-v492.json"
    assert payload["tower_layer"] == "Giant Push 2"
    assert payload["tower_sublayer"] == "Capital Safety Enforcement Readiness layer"
    assert payload["source_pack"] == "491"
    assert payload["source_closed_batch"] == "396-450"
    assert payload["giant_push"] == "451-500"
    assert payload["next_batch"] == "501-550"
    assert payload["next_pack"] == "493"
    ...
    assert payload["safe_to_continue_to_pack_493"] is True
    assert payload["prepare_pack_493_tower_capital_safety_enforcement_readiness_policy_matrix"]["pack"] == "493"


def test_pack_492_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_registry_contract_v492")
    payload = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()
    summary = payload["tower_capital_safety_enforcement_readiness_registry_contract_summary"]
    ...
    assert summary["tower_capital_safety_enforcement_readiness_registry_contract_ready"] is True
...
def test_pack_492_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_safety_enforcement_readiness_registry_contract_v492")
    payload = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()

    rows = payload["tower_capital_safety_enforcement_readiness_registry_contract_rows"]
    checks = payload["tower_capital_safety_enforcement_readiness_registry_contract_checks"]
    actions = payload["tower_capital_safety_enforcement_readiness_registry_contract_actions"]
    ...
    bridge = mod.build_pack_492_status_bridge()
    assert bridge["pack"] == "492"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_capital_safety_enforcement_readiness_registry_contract_ready"] is True
    assert bridge["safe_to_continue_to_pack_493"] is True

    prep = mod.prepare_pack_493_tower_capital_safety_enforcement_readiness_policy_matrix()
    assert prep["ready"] is True
    assert prep["next_pack"] == "493"
    assert prep["source_pack"] == "492"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-capital-safety-enforcement-readiness-registry-contract-v492.json" in rules

    response = app.test_client().get("/tower/tower-capital-safety-enforcement-readiness-registry-contract-v492.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "492"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_493"] is True

    first = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()
    second = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_safety_enforcement_readiness_registry_contract_preview()
