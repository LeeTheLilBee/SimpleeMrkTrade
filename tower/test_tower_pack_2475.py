...
def test_pack_2475_hosted_assurance_contract():
    payload = build_ir_cert_p2475_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Backup Cadence Readiness'
    assert payload["backup_cadence_ready"] is True
