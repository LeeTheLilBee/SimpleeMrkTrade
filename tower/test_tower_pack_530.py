"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_530_TOWER_CAPITAL_DEPLOYMENT_GATE_BATCH_CLOSE_READINESS_TESTS_RECURSION_SAFE
"""
...
def test_pack_530_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_batch_close_readiness_v530")
    payload = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()

    assert payload["pack"] == "530"
    assert payload["pack_number"] == 530
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-batch-close-readiness-v530.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "529"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "531"
    ...
    assert payload["safe_to_continue_to_pack_531"] is True
    assert payload["prepare_pack_531_tower_owner_override_decision_trigger_index"]["pack"] == "531"


def test_pack_530_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_batch_close_readiness_v530")
    payload = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()
    summary = payload["tower_capital_deployment_gate_batch_close_readiness_summary"]
    ...
    assert summary["tower_capital_deployment_gate_batch_close_readiness_ready"] is True
...
def test_pack_530_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_batch_close_readiness_v530")
    payload = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()

    rows = payload["tower_capital_deployment_gate_batch_close_readiness_rows"]
    checks = payload["tower_capital_deployment_gate_batch_close_readiness_checks"]
    actions = payload["tower_capital_deployment_gate_batch_close_readiness_actions"]
    ...
    bridge = mod.build_pack_530_status_bridge()
    assert bridge["pack"] == "530"
    ...
    assert bridge["tower_capital_deployment_gate_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_531"] is True

    prep = mod.prepare_pack_531_tower_owner_override_decision_trigger_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "531"
    assert prep["source_pack"] == "530"

    first = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()
    second = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_batch_close_readiness_preview()
