"""
SEARCHABLE LABEL: TOWER_PACK_352_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_RECEIPT_SEAL_RELEASE_TESTS
"""
...
def test_pack_352_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_v352")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()

    assert payload["pack"] == "352"
    assert payload["pack_number"] == 352
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-receipt-seal-release-detail-drawer-v352.json"
    assert payload["tower_sublayer"] == "Handoff Policy Route Enforcement Owner Acceptance Receipt Seal Release layer"
    assert payload["source_pack"] == "351"
    assert payload["source_closed_batch"] == "346-350"
    assert payload["save_batch"] == "351-355"
    assert payload["save_after_pack"] == 355
    assert payload["next_pack"] == "353"
    ...
    assert payload["safe_to_continue_to_pack_353"] is True
    assert payload["prepare_pack_353_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_note_draft"]["pack"] == "353"


def test_pack_352_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_v352")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()
    summary = payload["owner_acceptance_receipt_seal_release_detail_drawer_summary"]
    ...
    assert summary["owner_acceptance_receipt_seal_release_detail_drawer_ready"] is True
...
def test_pack_352_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_v352")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()

    cards = payload["owner_acceptance_receipt_seal_release_detail_drawer_cards"]
    fields = payload["owner_acceptance_receipt_seal_release_detail_drawer_fields"]
    actions = payload["owner_acceptance_receipt_seal_release_detail_drawer_actions"]
    checkpoints = payload["owner_acceptance_receipt_seal_release_detail_drawer_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-receipt-seal-release-detail-drawer-v352.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-receipt-seal-release-detail-drawer-v352.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "352"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["release_preview_only"] is True
        assert data["safe_to_continue_to_pack_353"] is True


def test_pack_352_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_v352")

    bridge = mod.build_pack_352_status_bridge()
    assert bridge["pack"] == "352"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_receipt_seal_release_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_353"] is True

    prep = mod.prepare_pack_353_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "353"
    assert prep["source_pack"] == "352"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_receipt_seal_release_detail_drawer_preview()
