...
def test_pack_2482_hosted_assurance_contract():
    payload = build_ir_cert_p2482_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Persistence Assurance Checkpoint'
    assert payload["hosted_assurance_checkpoint_ready"] is True
