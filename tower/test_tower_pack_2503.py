...
def test_pack_2503_human_login_ob_contract():
    payload = build_ir_cert_p2503_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Human Login Contract'
    assert payload["human_login_contract_ready"] is True
