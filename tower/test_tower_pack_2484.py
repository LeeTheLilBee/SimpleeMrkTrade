...
def test_pack_2484_deployment_boundary():
    payload = build_ir_cert_p2484_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Environment Binding Receipt'
    assert payload["environment_binding_receipt_ready"] is True
