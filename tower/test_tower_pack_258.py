"""
SEARCHABLE LABEL: TOWER_PACK_258_RECEIPT_CHAIN_SAVED_VIEW_OWNER_REVIEW_GOVERNANCE_CONTINUOUS_ASSURANCE_ESCALATION_NOTE_DRAFT_PREVIEW_TESTS
"""
...
def test_pack_258_escalation_note_draft_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()

    assert payload["pack"] == "258"
    assert payload["pack_number"] == 258
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Governance Continuous Assurance Escalation Note Draft Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-draft-v258.json"
    ...
    assert payload["source_pack"] == "257"
    assert payload["source_status"] == "ready"
    assert payload["source_readiness"] == 100
    assert payload["source_safe_to_continue"] is True

    assert payload["safe_to_continue_to_pack_259"] is True
    assert payload["prepare_pack_259_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version"]["pack"] == "259"


def test_pack_258_escalation_note_draft_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()

    summary = payload["governance_continuous_assurance_escalation_note_draft_summary"]

    assert summary["source_detail_drawer_count"] >= 8
    assert summary["draft_card_count"] >= 8
    assert summary["draft_field_count"] >= 80
    assert summary["draft_action_count"] >= 112
    assert summary["draft_checkpoint_count"] >= 5
    assert summary["enabled_action_count"] >= 8
    assert summary["blocked_action_count"] >= 104
    assert summary["editable_field_count"] >= 32
    assert summary["locked_field_count"] >= 48
    ...
    assert summary["escalation_note_draft_ready"] is True
...
def test_pack_258_note_draft_parts_are_preview_safe():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()

    cards = payload["governance_continuous_assurance_escalation_note_draft_cards"]
    fields = payload["governance_continuous_assurance_escalation_note_draft_fields"]
    actions = payload["governance_continuous_assurance_escalation_note_draft_actions"]
    checkpoints = payload["governance_continuous_assurance_escalation_note_draft_checkpoints"]
    ...
    assert len(blocked_actions) >= 104
...
def test_pack_258_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")
    payload = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()
...
def test_pack_258_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")

    first = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_preview()

    assert third["status"] == "ready"


def test_pack_258_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_draft_v258")

    bridge = mod.build_pack_258_status_bridge()
    assert bridge["pack"] == "258"
    assert bridge["pack_number"] == 258
    ...
    assert bridge["source_pack"] == "257"
    assert bridge["source_status"] == "ready"
    assert bridge["draft_card_count"] >= 8
    assert bridge["draft_field_count"] >= 80
    assert bridge["draft_action_count"] >= 112
    assert bridge["escalation_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_259"] is True

    prep = mod.prepare_pack_259_receipt_chain_saved_view_owner_review_governance_continuous_assurance_escalation_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "259"
...
def test_pack_258_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-draft-v258.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-governance-continuous-assurance-escalation-note-draft-v258.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "258"
