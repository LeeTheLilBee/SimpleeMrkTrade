from tower.tower_ir_cert_p2519 import (
    build_ir_cert_p2519_preview,
)


def test_pack_2519_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2519_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Actions Panel'
    assert payload["owner_actions_panel_ready"] is True
