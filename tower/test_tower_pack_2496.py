...
def test_pack_2496_activation_approval_contract():
    payload = build_ir_cert_p2496_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Activation Scope Freeze'
    assert payload["activation_scope_freeze_ready"] is True
