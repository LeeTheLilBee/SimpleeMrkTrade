...
def run_tests():
    ...
    reset_incident_filters = reset_security_incident_filters_escalation_for_test()
    ...
    ack = apply_security_inbox_review_action(
        ...
        note="Pack 129 incident filter UI integration test.",
        reason="Pack 129 seed.",
        metadata={"pack": "129", "token": "SHOULD_NOT_SURVIVE_TOKEN"},
    )
    ...
    if len(target_ids) > 1:
        inv = apply_security_inbox_review_action(
            ...
            actor_user_id="owner_solice",
            note="Pack 129 incident filter seed.",
            reason="Pack 129 seed.",
            metadata={"pack": "129"},
        )
    ...
    incidents = incident_status.get("incident_candidates", [])
    assert isinstance(incidents, list)
    assert len(incidents) >= 1

    target_incident_id = incidents[0].get("incident_id")
    assert target_incident_id

    action = apply_security_incident_action(
        incident_id=target_incident_id,
        incident_status="triaged",
        severity="high",
        actor_user_id="owner_solice",
        assigned_to="owner_solice",
        note="Pack 129 triage seed. Secret should_not_survive.",
        reason="Pack 129 incident filter UI seed.",
        metadata={"pack": "129", "token": "SHOULD_NOT_SURVIVE_TOKEN"},
    )
    assert action.get("ok") is True
    assert action.get("no_secret_leakage") is True
    no_secret(action)

    if len(incidents) > 1:
        action2 = apply_security_incident_action(
            incident_id=incidents[1].get("incident_id"),
            incident_status="investigating",
            severity="high",
            actor_user_id="owner_solice",
            assigned_to="owner_solice",
            note="Pack 129 investigating seed.",
            reason="Pack 129 incident filter UI seed.",
            metadata={"pack": "129"},
        )
        assert action2.get("ok") is True
    ...
    show("PACK 129 INCIDENT FILTERS STATUS", {
    ...
    show("PACK 129 QUICK ACTION STATUS", {
    ...
    assert incident_filters.get("incident_count", 0) >= 1
    assert incident_filters.get("escalation_ready_count", 0) >= 1

    assert quick_status.get("ok") is True
    assert quick_status.get("status") == "passed"
    assert quick_status.get("readiness_score") == 100
    assert quick_status.get("action_count", 0) >= 13
    ...
    show("PACK 129 UNIFIED HTML CHECK", {
        ...
        "has_incident_filters_link": "/tower/security-incident-filters.json" in unified_html,
    })
    ...
    assert "/tower/security-incident-filters.json" in unified_html
    ...
    show("PACK 129 WRITE UNIFIED HTML", write_result)
    assert write_result.get("ok") is True

    from tower.tower_status import pack129_security_incident_filters_escalation_status_bridge
    from tower.security_command_page import pack129_security_incident_filters_escalation_html_section

    filters_bridge = pack129_security_incident_filters_escalation_status_bridge()
    filters_html = pack129_security_incident_filters_escalation_html_section()

    show("PACK 129 BRIDGE CHECKS", {
        "filters_bridge_ok": filters_bridge.get("ok"),
        "filters_bridge_pack": filters_bridge.get("pack"),
        "filters_html_marker": "PACK128_SECURITY_INCIDENT_FILTERS_ESCALATION_SECTION" in filters_html,
    })

    assert filters_bridge.get("ok") is True
    assert filters_bridge.get("pack") == "128"
    assert "PACK128_SECURITY_INCIDENT_FILTERS_ESCALATION_SECTION" in filters_html
    ...
    for path in [
        PROJECT_ROOT / "tower" / "test_tower_pack_129.py",
    ...
    final = {
        ...
        "needs_guard_count": route_report.get("needs_guard_count"),
        "helper_wrapped_count": checkpoint.get("helper_wrapped_count"),
        "human_reason": "Incident filters/escalation readiness are integrated into unified Security Command UI.",
    }
    show("PACK 129 RESULT", final)
    return final
