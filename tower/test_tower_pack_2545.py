from tower.tower_ir_cert_p2545 import (
    build_ir_cert_p2545_preview,
)


def test_pack_2545_tower_ob_handoff_contract():
    payload = build_ir_cert_p2545_preview()

    assert payload["pack"] == "2545"
    assert payload["pack_name"] == 'Tower to OB Launch Continuity Contract'
    ...
    assert payload["tower_to_ob_launch_continuity_contract_ready"] is True
    assert payload[
