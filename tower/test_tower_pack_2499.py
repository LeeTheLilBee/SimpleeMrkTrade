...
def test_pack_2499_activation_approval_contract():
    payload = build_ir_cert_p2499_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Deployment Command Dry Run'
    assert payload["deployment_command_dry_run_ready"] is True
