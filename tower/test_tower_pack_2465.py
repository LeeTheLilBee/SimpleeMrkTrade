...
def test_pack_2465_operations_contract():
    payload = build_ir_cert_p2465_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Ledger Startup Health Check'
    assert payload["startup_health_check_ready"] is True
