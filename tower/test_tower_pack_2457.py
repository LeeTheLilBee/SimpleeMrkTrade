...
def test_pack_2457_persistence_contract():
    payload = build_ir_cert_p2457_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Final Receipt Persistence'
    assert payload["final_receipt_persistence_ready"] is True
