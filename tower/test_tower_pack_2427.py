...
def test_pack_2427_walkthrough_contract():
    payload = build_ir_cert_p2427_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Protected Preview Launch Action"
    assert payload["preview_launch_action_ready"] is True
