...
def test_pack_2506_human_login_ob_contract():
    payload = build_ir_cert_p2506_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Tower Access Home'
    assert payload["tower_access_home_ready"] is True
