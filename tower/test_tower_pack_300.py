"""
SEARCHABLE LABEL: TOWER_PACK_300_HANDOFF_POLICY_ROUTE_ENFORCEMENT_FINAL_AUDIT_BATCH_CLOSE_TESTS
"""
...
def test_pack_300_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_v300")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()

    assert payload["pack"] == "300"
    assert payload["pack_number"] == 300
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-batch-close-readiness-v300.json"
    ...
    assert payload["next_batch"] == "301-305"
    assert payload["next_pack"] == "301"
    ...
    assert payload["safe_to_push_packs_296_to_300"] is True
    assert payload["safe_to_continue_to_pack_301"] is True
    assert payload["prepare_pack_301_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_index"]["pack"] == "301"


def test_pack_300_source_packs_and_summary_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_v300")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()

    for pack in ["296", "297", "298", "299"]:
    ...
    summary = payload["final_audit_batch_close_summary"]

    assert summary["source_closed_batch"] == "291-295"
    assert summary["save_batch"] == "296-300"
    assert summary["save_after_pack"] == 300
    assert summary["next_batch"] == "301-305"
    assert summary["next_pack"] == "301"
    ...
    assert summary["final_audit_batch_ready_to_push"] is True
...
def test_pack_300_manifest_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_v300")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()

    manifest = payload["final_audit_save_manifest_preview"]
    expected = {
    ...
    bridge = mod.build_pack_300_status_bridge()
    assert bridge["pack"] == "300"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["final_audit_batch_ready_to_push"] is True
    assert bridge["safe_to_push_packs_296_to_300"] is True
    assert bridge["safe_to_continue_to_pack_301"] is True

    prep = mod.prepare_pack_301_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "301"
    assert prep["closed_batch"] == "296-300"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-batch-close-readiness-v300.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-audit-batch-close-readiness-v300.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "300"
        assert data["status"] == "ready"
        assert data["safe_to_push_packs_296_to_300"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_audit_batch_close_readiness_preview()
