...
def test_pack_2494_activation_approval_contract():
    payload = build_ir_cert_p2494_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Step-Up Requirement'
    assert payload["owner_step_up_requirement_ready"] is True
