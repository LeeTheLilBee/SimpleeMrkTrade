"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_549_TOWER_BETA_BACKBONE_FINAL_READINESS_OWNER_FOCUS_BRIDGE_TESTS_RECURSION_SAFE
"""
...
def test_pack_549_contract_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_owner_focus_bridge_v549")
    payload = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()

    assert payload["pack"] == "549"
    assert payload["pack_number"] == 549
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-backbone-final-readiness-owner-focus-bridge-v549.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Tower Beta Backbone Final Readiness layer"
    assert payload["source_pack"] == "548"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "550"
    assert payload["cached"] is True
    assert payload["non_recursive"] is True
    assert payload["recursion_safe"] is True
    assert payload["simulation_only"] is True
    assert payload["preview_only"] is True
    assert payload["tower_beta_backbone_final_readiness_preview_only"] is True
    ...
    assert payload["safe_to_continue_to_pack_550"] is True
    assert payload["prepare_pack_550_tower_beta_backbone_final_readiness_batch_close_readiness"]["pack"] == "550"


def test_pack_549_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_owner_focus_bridge_v549")
    payload = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()
    summary = payload["tower_beta_backbone_final_readiness_owner_focus_bridge_summary"]
    ...
    assert summary["tower_beta_backbone_final_readiness_owner_focus_bridge_ready"] is True
...
def test_pack_549_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_beta_backbone_final_readiness_owner_focus_bridge_v549")
    payload = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()

    rows = payload["tower_beta_backbone_final_readiness_owner_focus_bridge_rows"]
    checks = payload["tower_beta_backbone_final_readiness_owner_focus_bridge_checks"]
    actions = payload["tower_beta_backbone_final_readiness_owner_focus_bridge_actions"]
    ...
    bridge = mod.build_pack_549_status_bridge()
    assert bridge["pack"] == "549"
    ...
    assert bridge["tower_beta_backbone_final_readiness_owner_focus_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_550"] is True

    prep = mod.prepare_pack_550_tower_beta_backbone_final_readiness_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "550"
    assert prep["source_pack"] == "549"

    first = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()
    second = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_backbone_final_readiness_owner_focus_bridge_preview()
