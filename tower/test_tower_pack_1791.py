"""
SEARCHABLE LABEL:
TOWER_PACK_1791_OPERATIONS_COMMAND_READINESS_TESTS
"""

import importlib


def test_pack_1791_ready():
    mod = importlib.import_module(
        "tower.tower_tower_beta_operations_command_readiness_owner_summary_command_matrix_v1791"
    )

    payload = mod.build_tower_beta_operations_command_readiness_owner_summary_command_matrix_preview()

    assert payload["pack"] == "1791"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-operations-command-readiness-owner-summary-command-matrix-v1791.json"
    assert payload["source_pack"] == "1790"
    assert payload["current_packs"] == "1759-1808"
    assert payload["save_block"] == "1759-1808"
    assert payload["next_pack"] == "1792"
    ...
    assert payload["safe_to_continue_to_pack_1792"] is True


def test_pack_1791_protocol_authority_and_locks():
    mod = importlib.import_module(
        "tower.tower_tower_beta_operations_command_readiness_owner_summary_command_matrix_v1791"
    )
    ...
    assert summary["tower_beta_operations_command_readiness_owner_summary_command_matrix_ready"] is True
...
def test_pack_1791_bridge_prep_and_copy():
    mod = importlib.import_module(
        "tower.tower_tower_beta_operations_command_readiness_owner_summary_command_matrix_v1791"
    )

    bridge = mod.build_pack_1791_status_bridge()
    prep = mod.prepare_pack_1792_tower_beta_operations_command_readiness_owner_summary_detail_drawer()

    assert bridge["pack"] == "1791"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["tower_beta_operations_command_readiness_owner_summary_command_matrix_ready"] is True
    assert bridge["safe_to_continue_to_pack_1792"] is True

    assert prep["ready"] is True
    assert prep["next_pack"] == "1792"
    assert prep["source_pack"] == "1791"

    first = mod.build_tower_beta_operations_command_readiness_owner_summary_command_matrix_preview()
    second = mod.build_tower_beta_operations_command_readiness_owner_summary_command_matrix_preview()
    ...
    third = mod.build_tower_beta_operations_command_readiness_owner_summary_command_matrix_preview()
