"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_2_PACK_484_TOWER_SAFETY_NOTIFICATION_SNAPSHOT_DETAIL_DRAWER_TESTS
"""
...
def test_pack_484_contract_ready():
    mod = importlib.import_module("tower.tower_tower_safety_notification_snapshot_detail_drawer_v484")
    payload = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()

    assert payload["pack"] == "484"
    assert payload["pack_number"] == 484
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-safety-notification-snapshot-detail-drawer-v484.json"
    assert payload["tower_layer"] == "Giant Push 2"
    assert payload["tower_sublayer"] == "Safety Notification and Status Snapshot layer"
    assert payload["source_pack"] == "483"
    assert payload["source_closed_batch"] == "396-450"
    assert payload["giant_push"] == "451-500"
    assert payload["next_batch"] == "501-550"
    assert payload["next_pack"] == "485"
    ...
    assert payload["safe_to_continue_to_pack_485"] is True
    assert payload["prepare_pack_485_tower_safety_notification_snapshot_note_draft"]["pack"] == "485"


def test_pack_484_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_safety_notification_snapshot_detail_drawer_v484")
    payload = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()
    summary = payload["tower_safety_notification_snapshot_detail_drawer_summary"]
    ...
    assert summary["tower_safety_notification_snapshot_detail_drawer_ready"] is True
...
def test_pack_484_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_safety_notification_snapshot_detail_drawer_v484")
    payload = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()

    rows = payload["tower_safety_notification_snapshot_detail_drawer_rows"]
    checks = payload["tower_safety_notification_snapshot_detail_drawer_checks"]
    actions = payload["tower_safety_notification_snapshot_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_484_status_bridge()
    assert bridge["pack"] == "484"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["mission_account_count"] >= 6
    assert bridge["mode_count"] >= 10
    assert bridge["kill_switch_count"] >= 15
    assert bridge["tower_safety_notification_snapshot_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_485"] is True

    prep = mod.prepare_pack_485_tower_safety_notification_snapshot_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "485"
    assert prep["source_pack"] == "484"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/tower-safety-notification-snapshot-detail-drawer-v484.json" in rules

    response = app.test_client().get("/tower/tower-safety-notification-snapshot-detail-drawer-v484.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "484"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_485"] is True

    first = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()
    second = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_safety_notification_snapshot_detail_drawer_preview()
