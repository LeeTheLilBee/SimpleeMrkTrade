"""
SEARCHABLE LABEL: TOWER_PACK_328_HANDOFF_POLICY_ROUTE_ENFORCEMENT_OWNER_ACCEPTANCE_ARCHIVE_TESTS
"""
...
def test_pack_328_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_v328")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()

    assert payload["pack"] == "328"
    assert payload["pack_number"] == 328
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-archive-note-draft-v328.json"
    ...
    assert payload["safe_to_continue_to_pack_329"] is True
    assert payload["prepare_pack_329_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_version"]["pack"] == "329"


def test_pack_328_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_v328")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()
    summary = payload["owner_acceptance_archive_note_draft_summary"]
    ...
    assert summary["owner_acceptance_archive_note_draft_ready"] is True
...
def test_pack_328_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_v328")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()

    cards = payload["owner_acceptance_archive_note_draft_cards"]
    fields = payload["owner_acceptance_archive_note_draft_fields"]
    actions = payload["owner_acceptance_archive_note_draft_actions"]
    checkpoints = payload["owner_acceptance_archive_note_draft_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-archive-note-draft-v328.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-owner-acceptance-archive-note-draft-v328.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "328"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["archive_preview_only"] is True
        assert data["safe_to_continue_to_pack_329"] is True


def test_pack_328_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_v328")

    bridge = mod.build_pack_328_status_bridge()
    assert bridge["pack"] == "328"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 15
    assert bridge["owner_acceptance_archive_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_329"] is True

    prep = mod.prepare_pack_329_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "329"
    assert prep["source_pack"] == "328"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_owner_acceptance_archive_note_draft_preview()
