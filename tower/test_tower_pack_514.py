"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_514_TOWER_MANUAL_LIVE_CLEARANCE_GATE_DETAIL_DRAWER_TESTS_RECURSION_SAFE
"""
...
def test_pack_514_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_detail_drawer_v514")
    payload = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()

    assert payload["pack"] == "514"
    assert payload["pack_number"] == 514
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-detail-drawer-v514.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "513"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "515"
    ...
    assert payload["safe_to_continue_to_pack_515"] is True
    assert payload["prepare_pack_515_tower_manual_live_clearance_gate_note_draft"]["pack"] == "515"


def test_pack_514_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_detail_drawer_v514")
    payload = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()
    summary = payload["tower_manual_live_clearance_gate_detail_drawer_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_detail_drawer_ready"] is True
...
def test_pack_514_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_detail_drawer_v514")
    payload = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()

    rows = payload["tower_manual_live_clearance_gate_detail_drawer_rows"]
    checks = payload["tower_manual_live_clearance_gate_detail_drawer_checks"]
    actions = payload["tower_manual_live_clearance_gate_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_514_status_bridge()
    assert bridge["pack"] == "514"
    ...
    assert bridge["tower_manual_live_clearance_gate_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_515"] is True

    prep = mod.prepare_pack_515_tower_manual_live_clearance_gate_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "515"
    assert prep["source_pack"] == "514"

    first = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()
    second = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_detail_drawer_preview()
