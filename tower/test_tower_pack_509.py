"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_509_TOWER_BROKER_SAFEGUARD_CHECKLIST_OWNER_FOCUS_BRIDGE_TESTS_RECURSION_SAFE
"""
...
def test_pack_509_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_owner_focus_bridge_v509")
    payload = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()

    assert payload["pack"] == "509"
    assert payload["pack_number"] == 509
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-owner-focus-bridge-v509.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "508"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "510"
    ...
    assert payload["safe_to_continue_to_pack_510"] is True
    assert payload["prepare_pack_510_tower_broker_safeguard_checklist_batch_close_readiness"]["pack"] == "510"


def test_pack_509_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_owner_focus_bridge_v509")
    payload = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()
    summary = payload["tower_broker_safeguard_checklist_owner_focus_bridge_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_owner_focus_bridge_ready"] is True
...
def test_pack_509_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_owner_focus_bridge_v509")
    payload = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()

    rows = payload["tower_broker_safeguard_checklist_owner_focus_bridge_rows"]
    checks = payload["tower_broker_safeguard_checklist_owner_focus_bridge_checks"]
    actions = payload["tower_broker_safeguard_checklist_owner_focus_bridge_actions"]
    ...
    bridge = mod.build_pack_509_status_bridge()
    assert bridge["pack"] == "509"
    ...
    assert bridge["tower_broker_safeguard_checklist_owner_focus_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_510"] is True

    prep = mod.prepare_pack_510_tower_broker_safeguard_checklist_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "510"
    assert prep["source_pack"] == "509"

    first = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()
    second = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_owner_focus_bridge_preview()
