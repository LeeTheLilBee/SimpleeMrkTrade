"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_1_PACK_424_SHARED_CONNECTIVE_TISSUE_DETAIL_DRAWER_TESTS
"""
...
def test_pack_424_contract_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_detail_drawer_v424")
    payload = mod.build_shared_connective_tissue_detail_drawer_preview()

    assert payload["pack"] == "424"
    assert payload["pack_number"] == 424
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/shared-connective-tissue-detail-drawer-v424.json"
    assert payload["tower_layer"] == "Giant Push 1"
    assert payload["tower_sublayer"] == "Shared Connective Tissue layer"
    assert payload["source_pack"] == "423"
    assert payload["source_closed_batch"] == "391-395"
    assert payload["giant_push"] == "396-450"
    assert payload["next_batch"] == "451-500"
    assert payload["next_pack"] == "425"
    ...
    assert payload["safe_to_continue_to_pack_425"] is True
    assert payload["prepare_pack_425_shared_connective_tissue_note_draft"]["pack"] == "425"


def test_pack_424_summary_safety_ready():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_detail_drawer_v424")
    payload = mod.build_shared_connective_tissue_detail_drawer_preview()
    summary = payload["shared_connective_tissue_detail_drawer_summary"]
    ...
    assert summary["shared_connective_tissue_detail_drawer_ready"] is True
...
def test_pack_424_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_shared_connective_tissue_detail_drawer_v424")
    payload = mod.build_shared_connective_tissue_detail_drawer_preview()

    rows = payload["shared_connective_tissue_detail_drawer_rows"]
    checks = payload["shared_connective_tissue_detail_drawer_checks"]
    actions = payload["shared_connective_tissue_detail_drawer_actions"]
    ...
    bridge = mod.build_pack_424_status_bridge()
    assert bridge["pack"] == "424"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["shared_connective_tissue_detail_drawer_ready"] is True
    assert bridge["safe_to_continue_to_pack_425"] is True

    prep = mod.prepare_pack_425_shared_connective_tissue_note_draft()
    assert prep["ready"] is True
    assert prep["next_pack"] == "425"
    assert prep["source_pack"] == "424"

    import web.app as web_app
    app = getattr(web_app, "app", None)
    assert app is not None
    rules = {rule.rule for rule in app.url_map.iter_rules()}
    assert "/tower/shared-connective-tissue-detail-drawer-v424.json" in rules

    response = app.test_client().get("/tower/shared-connective-tissue-detail-drawer-v424.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "424"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_425"] is True

    first = mod.build_shared_connective_tissue_detail_drawer_preview()
    second = mod.build_shared_connective_tissue_detail_drawer_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_shared_connective_tissue_detail_drawer_preview()
