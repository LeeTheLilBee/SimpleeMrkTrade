from tower.tower_ir_cert_p2518 import (
    build_ir_cert_p2518_preview,
)


def test_pack_2518_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2518_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Return Receipt Status Panel'
    assert payload["return_receipt_status_panel_ready"] is True
