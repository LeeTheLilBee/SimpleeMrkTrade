"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_506_TOWER_BROKER_SAFEGUARD_CHECKLIST_NOTE_VERSION_TESTS_RECURSION_SAFE
"""
...
def test_pack_506_contract_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_version_v506")
    payload = mod.build_tower_broker_safeguard_checklist_note_version_preview()

    assert payload["pack"] == "506"
    assert payload["pack_number"] == 506
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-broker-safeguard-checklist-note-version-v506.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Broker Safeguard Checklist layer"
    assert payload["source_pack"] == "505"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "507"
    ...
    assert payload["safe_to_continue_to_pack_507"] is True
    assert payload["prepare_pack_507_tower_broker_safeguard_checklist_handoff_contract"]["pack"] == "507"


def test_pack_506_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_version_v506")
    payload = mod.build_tower_broker_safeguard_checklist_note_version_preview()
    summary = payload["tower_broker_safeguard_checklist_note_version_summary"]
    ...
    assert summary["tower_broker_safeguard_checklist_note_version_ready"] is True
...
def test_pack_506_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_broker_safeguard_checklist_note_version_v506")
    payload = mod.build_tower_broker_safeguard_checklist_note_version_preview()

    rows = payload["tower_broker_safeguard_checklist_note_version_rows"]
    checks = payload["tower_broker_safeguard_checklist_note_version_checks"]
    actions = payload["tower_broker_safeguard_checklist_note_version_actions"]
    ...
    bridge = mod.build_pack_506_status_bridge()
    assert bridge["pack"] == "506"
    ...
    assert bridge["tower_broker_safeguard_checklist_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_507"] is True

    prep = mod.prepare_pack_507_tower_broker_safeguard_checklist_handoff_contract()
    assert prep["ready"] is True
    assert prep["next_pack"] == "507"
    assert prep["source_pack"] == "506"

    first = mod.build_tower_broker_safeguard_checklist_note_version_preview()
    second = mod.build_tower_broker_safeguard_checklist_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_broker_safeguard_checklist_note_version_preview()
