from tower.tower_ir_cert_p2517 import (
    build_ir_cert_p2517_preview,
)


def test_pack_2517_tower_access_home_ui_v2_contract():
    payload = build_ir_cert_p2517_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'OB Return Session Continuity'
    assert payload["ob_return_session_continuity_ready"] is True
