...
def run_tests():
    ...
    show("RESET SECURITY WATCH CHECKPOINT", reset_watch_checkpoint)
    ...
    ack = apply_security_inbox_review_action(
        ...
        note="Pack 134 Security Watch checkpoint UI integration test.",
        reason="Pack 134 seed.",
        metadata={"pack": "134", "token": "SHOULD_NOT_SURVIVE_TOKEN"},
    )
    ...
    if len(target_ids) > 1:
        inv = apply_security_inbox_review_action(
            ...
            actor_user_id="owner_solice",
            note="Pack 134 investigation seed.",
            reason="Pack 134 seed.",
            metadata={"pack": "134"},
        )
    ...
    triaged = apply_security_incident_action(
        ...
        note="Pack 134 triage seed. Secret should_not_survive.",
        reason="Pack 134 Security Watch checkpoint UI seed.",
        metadata={"pack": "134", "token": "SHOULD_NOT_SURVIVE_TOKEN"},
    )
    ...
    show("PACK 134 WATCH CHECKPOINT STATUS", {
        "ok": watch_checkpoint.get("ok"),
        "status": watch_checkpoint.get("status"),
        "readiness_score": watch_checkpoint.get("readiness_score"),
        "security_watch": watch_checkpoint.get("security_watch"),
        "failed_checks": watch_checkpoint.get("failed_checks"),
    })

    show("PACK 134 QUICK ACTION STATUS", {
    ...
    assert watch_checkpoint.get("security_watch", {}).get("recommended_first_action") in {
    ...
    assert quick_status.get("action_count", 0) >= 16
    ...
    show("PACK 134 UNIFIED HTML CHECK", {
        ...
        "has_watch_checkpoint_link": "/tower/security-watch-checkpoint.json" in unified_html,
    ...
    assert "/tower/security-watch-checkpoint.json" in unified_html
    ...
    show("PACK 134 WRITE UNIFIED HTML", write_result)
    assert write_result.get("ok") is True

    from tower.tower_status import pack134_security_watch_checkpoint_status_bridge
    from tower.security_command_page import pack134_security_watch_checkpoint_html_section

    checkpoint_bridge = pack134_security_watch_checkpoint_status_bridge()
    checkpoint_html = pack134_security_watch_checkpoint_html_section()

    show("PACK 134 BRIDGE CHECKS", {
        "checkpoint_bridge_ok": checkpoint_bridge.get("ok"),
        "checkpoint_bridge_pack": checkpoint_bridge.get("pack"),
        "checkpoint_html_marker": "PACK133_SECURITY_WATCH_CHECKPOINT_SECTION" in checkpoint_html,
    })

    assert checkpoint_bridge.get("ok") is True
    assert checkpoint_bridge.get("pack") == "133"
    assert "PACK133_SECURITY_WATCH_CHECKPOINT_SECTION" in checkpoint_html
    assert "SHOULD_NOT_SURVIVE" not in checkpoint_html
    ...
    for path in [
        PROJECT_ROOT / "tower" / "test_tower_pack_134.py",
    ...
    final = {
        ...
        "watch_checkpoint_readiness": watch_checkpoint.get("readiness_score"),
        ...
        "needs_guard_count": route_report.get("needs_guard_count"),
        "helper_wrapped_count": checkpoint.get("helper_wrapped_count"),
        "human_reason": "Security Watch checkpoint is integrated into unified Security Command UI.",
    }
    show("PACK 134 RESULT", final)
    return final
