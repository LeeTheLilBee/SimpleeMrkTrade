...
def test_pack_2456_persistence_contract():
    payload = build_ir_cert_p2456_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Room Receipt Persistence'
    assert payload["room_receipt_persistence_ready"] is True
