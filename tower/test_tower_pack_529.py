"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_529_TOWER_CAPITAL_DEPLOYMENT_GATE_OWNER_FOCUS_BRIDGE_TESTS_RECURSION_SAFE
"""
...
def test_pack_529_contract_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_owner_focus_bridge_v529")
    payload = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()

    assert payload["pack"] == "529"
    assert payload["pack_number"] == 529
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-capital-deployment-gate-owner-focus-bridge-v529.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Capital Deployment Gate layer"
    assert payload["source_pack"] == "528"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "530"
    ...
    assert payload["safe_to_continue_to_pack_530"] is True
    assert payload["prepare_pack_530_tower_capital_deployment_gate_batch_close_readiness"]["pack"] == "530"


def test_pack_529_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_owner_focus_bridge_v529")
    payload = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()
    summary = payload["tower_capital_deployment_gate_owner_focus_bridge_summary"]
    ...
    assert summary["tower_capital_deployment_gate_owner_focus_bridge_ready"] is True
...
def test_pack_529_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_capital_deployment_gate_owner_focus_bridge_v529")
    payload = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()

    rows = payload["tower_capital_deployment_gate_owner_focus_bridge_rows"]
    checks = payload["tower_capital_deployment_gate_owner_focus_bridge_checks"]
    actions = payload["tower_capital_deployment_gate_owner_focus_bridge_actions"]
    ...
    bridge = mod.build_pack_529_status_bridge()
    assert bridge["pack"] == "529"
    ...
    assert bridge["tower_capital_deployment_gate_owner_focus_bridge_ready"] is True
    assert bridge["safe_to_continue_to_pack_530"] is True

    prep = mod.prepare_pack_530_tower_capital_deployment_gate_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "530"
    assert prep["source_pack"] == "529"

    first = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()
    second = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_capital_deployment_gate_owner_focus_bridge_preview()
