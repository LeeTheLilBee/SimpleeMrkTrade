...
def test_pack_2504_human_login_ob_contract():
    payload = build_ir_cert_p2504_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Owner Credential Verification'
    assert payload["owner_credential_verification_ready"] is True
