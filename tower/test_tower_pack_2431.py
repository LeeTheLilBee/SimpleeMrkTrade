...
def test_pack_2431_walkthrough_contract():
    payload = build_ir_cert_p2431_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Six-Room Click-Path Rehearsal"
    assert payload["click_path_rehearsal_ready"] is True
