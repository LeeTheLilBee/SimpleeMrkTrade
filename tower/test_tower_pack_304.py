"""
SEARCHABLE LABEL: TOWER_PACK_304_HANDOFF_POLICY_ROUTE_ENFORCEMENT_FINAL_HANDOFF_TESTS
"""
...
def test_pack_304_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_v304")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()

    assert payload["pack"] == "304"
    assert payload["pack_number"] == 304
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-note-version-v304.json"
    ...
    assert payload["safe_to_continue_to_pack_305"] is True
    assert payload["prepare_pack_305_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_batch_close_readiness"]["pack"] == "305"


def test_pack_304_summary_safety_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_v304")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()
    summary = payload["final_handoff_note_version_summary"]
    ...
    assert summary["final_handoff_note_version_ready"] is True
...
def test_pack_304_payload_shapes_and_endpoint():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_v304")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()

    cards = payload["final_handoff_note_version_cards"]
    fields = payload["final_handoff_note_version_fields"]
    actions = payload["final_handoff_note_version_actions"]
    checkpoints = payload["final_handoff_note_version_checkpoints"]
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-note-version-v304.json" in rules

    response = app.test_client().get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-final-handoff-note-version-v304.json")
    assert response.status_code in {200, 302, 401, 403}
    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "304"
        assert data["status"] == "ready"
        assert data["readiness"] == 100
        assert data["preview_only"] is True
        assert data["safe_to_continue_to_pack_305"] is True


def test_pack_304_bridge_prep_and_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_v304")

    bridge = mod.build_pack_304_status_bridge()
    assert bridge["pack"] == "304"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["card_count"] >= 14
    assert bridge["final_handoff_note_version_ready"] is True
    assert bridge["safe_to_continue_to_pack_305"] is True

    prep = mod.prepare_pack_305_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_batch_close_readiness()
    assert prep["ready"] is True
    assert prep["next_pack"] == "305"
    assert prep["source_pack"] == "304"
    assert prep["safe_to_continue"] is True

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_final_handoff_note_version_preview()
