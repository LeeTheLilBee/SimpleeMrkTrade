...
def test_pack_2483_deployment_boundary():
    payload = build_ir_cert_p2483_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Hosted Deployment Manifest'
    assert payload["deployment_manifest_ready"] is True
