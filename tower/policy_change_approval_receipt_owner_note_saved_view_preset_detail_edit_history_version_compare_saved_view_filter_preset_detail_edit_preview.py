...
"""
PACK 185 - Owner Note Saved View Preset Detail Edit History Version Compare Saved View / Filter Preset Detail Edit Preview.

This module sits on top of Pack 184.

Pack 184 creates preview-only saved view/filter presets for the version compare navigation layer.
Pack 185 creates preview-only detail/edit scaffolding:
...
PACK_ID = "PACK_185"
DETAIL_EDIT_ENDPOINT = "/tower/policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-preview.json"
...
FILTER_PRESET_EDIT_FIELDS = [
    ...
    {"field_id": "default_selected_version_detail_drawer_id", "field_label": "Default Selected Drawer", "field_type": "readonly_text"},
]
...
def _load_pack_184_payload(force_refresh: bool = False) -> Dict[str, object]:
    try:
        mod = importlib.import_module(
            "tower.policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_navigation_saved_view_filter_preset"
        )
        fn = getattr(
            mod,
...
def _normalize_value(value: Any) -> Dict[str, object]:
    ...
    if isinstance(value, list):
        return {"kind": "list", "value": [_safe_text(item) for item in value], "item_count": len(value), "safe_preview_only": True}
...
def _preview_edit_value(field_id: str, current_value: Any, field_type: str) -> Any:
...
def _build_editable_row(source_kind: str, source_id: str, drawer_id: str, source: Dict[str, object], field_def: Dict[str, object], sequence: int) -> Dict[str, object]:
    ...
    preview_value = _preview_edit_value(field_id, current_value, field_type)
    ...
    row = {
        "editable_field_row_id": f"version_compare_saved_view_filter_editable_field_row_{_stable_hash(identity, 18)}",
        "source_kind": source_kind,
        "source_id": source_id,
        "drawer_id": drawer_id,
        "field_id": field_id,
        ...
        "editable_in_preview": not field_type.startswith("readonly"),
        "row_status": "version_compare_saved_view_filter_editable_field_row_preview_ready",
        "row_result_type": "owner_note_version_compare_saved_view_filter_editable_field_row_preview",
        "safe_preview_only": True,
    }
...
def _build_saved_view_detail_edit_drawer(preset: Dict[str, object], sequence: int) -> Dict[str, object]:
    preset = dict(preset or {})
    source_id = _safe_text(preset.get("saved_view_preset_id"))
    preview_id = _safe_text(preset.get("saved_view_preset_preview_id"))
    drawer_id = f"version_compare_saved_view_preset_detail_edit_drawer_{_stable_hash((PACK_ID, source_id, preview_id, sequence), 18)}"
    ...
    original_snapshot = {
        ...
        "saved_view_preset_preview_id": preview_id,
        ...
        "matched_navigation_item_count": preset.get("matched_navigation_item_count"),
        "comparison_row_count": preset.get("comparison_row_count"),
    }

    preview_edit_snapshot = {row["field_id"]: row["preview_value"].get("value") for row in rows}
    preview_edit_snapshot["saved_view_preset_id"] = source_id
    preview_edit_snapshot["saved_view_preset_preview_id"] = preview_id
    ...
    drawer = {
        ...
        "sequence": int(sequence),
        "drawer_status": "version_compare_saved_view_preset_detail_edit_preview_ready",
        "drawer_result_type": "owner_note_version_compare_saved_view_preset_detail_edit_drawer_preview",
...
def _build_filter_preset_detail_edit_drawer(preset: Dict[str, object], sequence: int) -> Dict[str, object]:
    preset = dict(preset or {})
    source_id = _safe_text(preset.get("filter_preset_id"))
    lane_id = _safe_text(preset.get("filter_lane_id"))
    drawer_id = f"version_compare_filter_preset_detail_edit_drawer_{_stable_hash((PACK_ID, source_id, lane_id, sequence), 18)}"
    ...
    original_snapshot = {
        ...
        "default_selected_version_detail_drawer_id": preset.get("default_selected_version_detail_drawer_id"),
    }
    ...
    drawer = {
        ...
        "sequence": int(sequence),
        "drawer_status": "version_compare_filter_preset_detail_edit_preview_ready",
        "drawer_result_type": "owner_note_version_compare_filter_preset_detail_edit_drawer_preview",
        "editable_field_rows": rows,
        "editable_field_row_count": len(rows),
        "changed_field_count": len([row for row in rows if row.get("changed") is True]),
        "unchanged_field_count": len([row for row in rows if row.get("changed") is False]),
...
def _build_indexes(saved_view_drawers: List[Dict[str, object]], filter_preset_drawers: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    for drawer in saved_view_drawers:
        ...
        priority = _safe_text(drawer.get("view_priority"))
        ...
        indexes["saved_view_drawers_by_priority"].setdefault(priority, []).append(drawer)
...
@lru_cache(maxsize=1)
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload_cached() -> Dict[str, object]:
    pack_184 = _load_pack_184_payload(force_refresh=False)

    saved_view_presets = pack_184.get("saved_view_presets", [])
    if not isinstance(saved_view_presets, list):
        saved_view_presets = []

    filter_presets = pack_184.get("filter_presets", [])
    ...
    readiness_checks = {
        "pack_184_ready": pack_184.get("status") == "ready",
        ...
        "all_saved_view_drawers_ready": all(drawer.get("drawer_status") == "version_compare_saved_view_preset_detail_edit_preview_ready" for drawer in saved_view_drawers),
        "all_filter_preset_drawers_ready": all(drawer.get("drawer_status") == "version_compare_filter_preset_detail_edit_preview_ready" for drawer in filter_preset_drawers),
        ...
        "all_version_detail_preview_only": all(item.get("version_detail_preview_only") is True for item in all_drawers + all_rows),
        "all_compare_view_preview_only": all(item.get("compare_view_preview_only") is True for item in all_drawers + all_rows),
    ...
    return {
        ...
        **_base_flags(),
        "source_pack_184": {
            "status": pack_184.get("status"),
            "readiness_score": pack_184.get("summary", {}).get("readiness_score"),
            "saved_view_preset_count": pack_184.get("summary", {}).get("saved_view_preset_count"),
            "filter_preset_count": pack_184.get("summary", {}).get("filter_preset_count"),
            "source_navigation_item_count": pack_184.get("summary", {}).get("source_navigation_item_count"),
            "comparison_row_count": pack_184.get("summary", {}).get("comparison_row_count"),
        },
        "summary": {
            ...
            "readiness_label": "Owner note version compare saved view/filter preset detail edit preview ready" if readiness_score == 100 else "Owner note version compare saved view/filter preset detail edit preview needs review",
        },
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload(force_refresh: bool = False) -> Dict[str, object]:
    if force_refresh:
        build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload_cached.cache_clear()
    return copy.deepcopy(build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload_cached())


def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload(force_refresh: bool = False) -> Dict[str, object]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge(force_refresh: bool = False) -> Dict[str, object]:
    payload = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload(force_refresh=force_refresh)
...
def get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge(force_refresh: bool = False) -> Dict[str, object]:
    return build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge(force_refresh=force_refresh)


def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_quick_action() -> Dict[str, object]:
    bridge = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge()
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_unified_owner_section() -> Dict[str, object]:
    ...
    section = {
        ...
        "subtitle": "Preview detail edit drawers, editable fields, and blocked persistence for version compare saved view/filter presets.",
        "status": payload.get("status"),
        "href": DETAIL_EDIT_ENDPOINT,
        "cards": [
            {"id": "readiness", "title": "Detail edit readiness", "value": summary.get("readiness_score"), "label": summary.get("readiness_label")},
            {"id": "saved_view_drawers", "title": "Saved view drawers", "value": summary.get("saved_view_detail_drawer_count"), "label": "Saved view detail drawers"},
            {"id": "filter_preset_drawers", "title": "Filter preset drawers", "value": summary.get("filter_preset_detail_drawer_count"), "label": "Filter preset detail drawers"},
            {"id": "editable_rows", "title": "Editable rows", "value": summary.get("total_editable_row_count"), "label": "Preview editable rows"},
            {"id": "changed_fields", "title": "Changed fields", "value": summary.get("changed_field_count"), "label": "Preview-only changes"},
            {"id": "persistence", "title": "Persistence", "value": "Blocked" if checks.get("all_saves_blocked") and checks.get("all_persists_blocked") else "Review", "label": "No real save/persist"},
        ],
    }
...
def build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_html_section() -> str:
    section = build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_unified_owner_section()
    ...
    for card in cards:
        card_html.append(
            f"""
            <article class="tower-card policy-change-approval-receipt-owner-note-version-compare-detail-edit-card">
    ...
    return f"""
    <section class="tower-section policy-change-approval-receipt-owner-note-version-compare-detail-edit-section" id="policy-change-approval-receipt-owner-note-saved-view-preset-detail-edit-history-version-compare-saved-view-filter-preset-detail-edit-preview">
        <div class="tower-section-heading">
            <p class="tower-kicker">Pack 185</p>
            <h2>{section.get('title', 'Owner Note Version Compare Saved View Detail Edit')}</h2>
            <p>{section.get('subtitle', '')}</p>
            <a class="tower-link-pill" href="{DETAIL_EDIT_ENDPOINT}">Open version compare saved view/filter detail edit JSON</a>
        </div>
...
__all__ = [
    ...
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_payload",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge",
    "get_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_status_bridge",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_quick_action",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_unified_owner_section",
    "build_policy_change_approval_receipt_owner_note_saved_view_preset_detail_edit_history_version_compare_saved_view_filter_preset_detail_edit_preview_html_section",
]
