"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_520_TOWER_MANUAL_LIVE_CLEARANCE_GATE_BATCH_CLOSE_READINESS_TESTS_RECURSION_SAFE
"""
...
def test_pack_520_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_batch_close_readiness_v520")
    payload = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()

    assert payload["pack"] == "520"
    assert payload["pack_number"] == 520
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-batch-close-readiness-v520.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "519"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "521"
    ...
    assert payload["safe_to_continue_to_pack_521"] is True
    assert payload["prepare_pack_521_tower_capital_deployment_gate_index"]["pack"] == "521"


def test_pack_520_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_batch_close_readiness_v520")
    payload = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()
    summary = payload["tower_manual_live_clearance_gate_batch_close_readiness_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_batch_close_readiness_ready"] is True
...
def test_pack_520_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_batch_close_readiness_v520")
    payload = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()

    rows = payload["tower_manual_live_clearance_gate_batch_close_readiness_rows"]
    checks = payload["tower_manual_live_clearance_gate_batch_close_readiness_checks"]
    actions = payload["tower_manual_live_clearance_gate_batch_close_readiness_actions"]
    ...
    bridge = mod.build_pack_520_status_bridge()
    assert bridge["pack"] == "520"
    ...
    assert bridge["tower_manual_live_clearance_gate_batch_close_readiness_ready"] is True
    assert bridge["safe_to_continue_to_pack_521"] is True

    prep = mod.prepare_pack_521_tower_capital_deployment_gate_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "521"
    assert prep["source_pack"] == "520"

    first = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()
    second = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_batch_close_readiness_preview()
