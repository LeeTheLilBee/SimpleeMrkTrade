"""
SEARCHABLE LABEL: TOWER_PACK_285_HANDOFF_POLICY_ROUTE_ENFORCEMENT_AUDIT_BATCH_CLOSE_READINESS_PREVIEW_TESTS
"""
...
def test_pack_285_audit_batch_close_contract_ready():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()

    assert payload["pack"] == "285"
    assert payload["pack_number"] == 285
    assert payload["pack_name"] == "Receipt Chain Saved View Owner Review Handoff Policy Route Enforcement Audit Batch Close Readiness Preview"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-batch-close-readiness-v285.json"
    ...
    assert payload["safe_to_push_packs_281_to_285"] is True
    assert payload["safe_to_continue_to_pack_286"] is True
    assert payload["prepare_pack_286_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_index"]["pack"] == "286"


def test_pack_285_audit_batch_close_summary_ready_to_push():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()

    summary = payload["handoff_policy_route_audit_batch_close_summary"]

    assert summary["source_closed_batch"] == "276-280"
    assert summary["save_batch"] == "281-285"
    assert summary["save_after_pack"] == 285
    assert summary["next_batch"] == "286-290"
    assert summary["next_pack"] == "286"
    assert summary["pack_card_count"] >= 5
    assert summary["close_check_count"] >= 20
    ...
    assert summary["handoff_policy_route_audit_batch_ready_to_push"] is True
...
def test_pack_285_cards_checks_manifest_and_transitions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()

    cards = payload["handoff_policy_route_audit_batch_pack_cards"]
    checks = payload["handoff_policy_route_audit_batch_close_checks"]
    manifest = payload["handoff_policy_route_audit_save_manifest_preview"]
    transitions = payload["handoff_policy_route_audit_transition_previews"]

    packs = {card["pack"] for card in cards}
    assert {"281", "282", "283", "284", "285"}.issubset(packs)
    ...
    expected_paths = {
        ...
        "tower/receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285.py",
        "tower/test_tower_pack_285.py",
        "web/app.py",
    }
...
def test_pack_285_safety_invariants_and_blocked_actions():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")
    payload = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()
...
def test_pack_285_public_builder_returns_defensive_copy():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")

    first = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()
    second = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()

    assert first == second
    assert first is not second

    first["status"] = "mutated_by_test"
    third = mod.build_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_preview()

    assert third["status"] == "ready"


def test_pack_285_status_bridge_and_next_prep():
    mod = importlib.import_module("tower.receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_batch_close_readiness_v285")

    bridge = mod.build_pack_285_status_bridge()
    assert bridge["pack"] == "285"
    assert bridge["pack_number"] == 285
    ...
    assert bridge["next_batch"] == "286-290"
    assert bridge["next_pack"] == "286"
    assert bridge["pack_card_count"] >= 5
    assert bridge["close_check_count"] >= 20
    assert bridge["save_manifest_preview_count"] >= 11
    assert bridge["handoff_policy_route_audit_batch_ready_to_push"] is True
    assert bridge["safe_to_push_packs_281_to_285"] is True
    assert bridge["safe_to_continue_to_pack_286"] is True

    prep = mod.prepare_pack_286_receipt_chain_saved_view_owner_review_handoff_policy_route_enforcement_audit_evidence_index()
    assert prep["ready"] is True
    assert prep["next_pack"] == "286"
    ...
    assert prep["closed_batch"] == "281-285"
    assert prep["next_batch"] == "286-290"
    assert prep["save_after_pack"] == 290
    assert prep["safe_to_continue"] is True


def test_pack_285_endpoint_is_registered_if_app_imports():
    ...
    assert "/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-batch-close-readiness-v285.json" in rules

    client = app.test_client()
    response = client.get("/tower/receipt-chain-saved-view-owner-review-handoff-policy-route-enforcement-audit-batch-close-readiness-v285.json")

    assert response.status_code in {200, 302, 401, 403}

    if response.status_code == 200:
        data = response.get_json()
        assert data["pack"] == "285"
        ...
        assert data["next_batch"] == "286-290"
        assert data["next_pack"] == "286"
        assert data["safe_to_push_packs_281_to_285"] is True
        assert data["safe_to_continue_to_pack_286"] is True
