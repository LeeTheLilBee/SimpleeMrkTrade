...
def test_pack_2480_hosted_assurance_contract():
    payload = build_ir_cert_p2480_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == 'Storage Operations Incident Receipt'
    assert payload["storage_incident_receipt_ready"] is True
