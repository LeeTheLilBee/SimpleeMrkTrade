"""
SEARCHABLE LABEL: TOWER_GIANT_PUSH_3_PACK_515_TOWER_MANUAL_LIVE_CLEARANCE_GATE_NOTE_DRAFT_TESTS_RECURSION_SAFE
"""
...
def test_pack_515_contract_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_draft_v515")
    payload = mod.build_tower_manual_live_clearance_gate_note_draft_preview()

    assert payload["pack"] == "515"
    assert payload["pack_number"] == 515
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-manual-live-clearance-gate-note-draft-v515.json"
    assert payload["tower_layer"] == "Giant Push 3"
    assert payload["tower_sublayer"] == "Manual Live Clearance Gate layer"
    assert payload["source_pack"] == "514"
    assert payload["source_closed_batch"] == "451-500"
    assert payload["giant_push"] == "501-550"
    assert payload["next_batch"] == "551-575"
    assert payload["next_pack"] == "516"
    ...
    assert payload["safe_to_continue_to_pack_516"] is True
    assert payload["prepare_pack_516_tower_manual_live_clearance_gate_note_version"]["pack"] == "516"


def test_pack_515_summary_safety_ready():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_draft_v515")
    payload = mod.build_tower_manual_live_clearance_gate_note_draft_preview()
    summary = payload["tower_manual_live_clearance_gate_note_draft_summary"]
    ...
    assert summary["tower_manual_live_clearance_gate_note_draft_ready"] is True
...
def test_pack_515_payload_shapes_endpoint_bridge_and_copy():
    mod = importlib.import_module("tower.tower_tower_manual_live_clearance_gate_note_draft_v515")
    payload = mod.build_tower_manual_live_clearance_gate_note_draft_preview()

    rows = payload["tower_manual_live_clearance_gate_note_draft_rows"]
    checks = payload["tower_manual_live_clearance_gate_note_draft_checks"]
    actions = payload["tower_manual_live_clearance_gate_note_draft_actions"]
    ...
    bridge = mod.build_pack_515_status_bridge()
    assert bridge["pack"] == "515"
    ...
    assert bridge["tower_manual_live_clearance_gate_note_draft_ready"] is True
    assert bridge["safe_to_continue_to_pack_516"] is True

    prep = mod.prepare_pack_516_tower_manual_live_clearance_gate_note_version()
    assert prep["ready"] is True
    assert prep["next_pack"] == "516"
    assert prep["source_pack"] == "515"

    first = mod.build_tower_manual_live_clearance_gate_note_draft_preview()
    second = mod.build_tower_manual_live_clearance_gate_note_draft_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_manual_live_clearance_gate_note_draft_preview()
