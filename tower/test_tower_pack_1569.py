"""
SEARCHABLE LABEL: TOWER_ONE_CELL_PACK_1554_1604_PACK_1569_TESTS
"""
...
def test_pack_1569_ready():
    mod = importlib.import_module("tower.tower_tower_beta_owner_go_no_go_decision_route_review_owner_summary_v1569")
    payload = mod.build_tower_beta_owner_go_no_go_decision_route_review_owner_summary_preview()

    assert payload["pack"] == "1569"
    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["endpoint"] == "/tower/tower-beta-owner-go-no-go-decision-route-review-owner-summary-v1569.json"
    assert payload["source_pack"] == "1568"
    assert payload["current_packs"] == "1555-1604"
    assert payload["save_block"] == "1554-1604"
    assert payload["next_pack"] == "1570"
    ...
    assert payload["safe_to_continue_to_pack_1570"] is True


def test_pack_1569_summary_safety_no_pivot():
    mod = importlib.import_module("tower.tower_tower_beta_owner_go_no_go_decision_route_review_owner_summary_v1569")
    payload = mod.build_tower_beta_owner_go_no_go_decision_route_review_owner_summary_preview()
    ...
    assert summary["tower_beta_owner_go_no_go_decision_route_review_owner_summary_ready"] is True
...
def test_pack_1569_bridge_prep_copy():
    mod = importlib.import_module("tower.tower_tower_beta_owner_go_no_go_decision_route_review_owner_summary_v1569")
    bridge = mod.build_pack_1569_status_bridge()
    prep = mod.prepare_pack_1570_tower_beta_owner_go_no_go_decision_route_review_note_draft()

    assert bridge["pack"] == "1569"
    assert bridge["status"] == "ready"
    assert bridge["readiness"] == 100
    assert bridge["tower_beta_owner_go_no_go_decision_route_review_owner_summary_ready"] is True
    assert bridge["safe_to_continue_to_pack_1570"] is True

    assert prep["ready"] is True
    assert prep["next_pack"] == "1570"
    assert prep["source_pack"] == "1569"

    first = mod.build_tower_beta_owner_go_no_go_decision_route_review_owner_summary_preview()
    second = mod.build_tower_beta_owner_go_no_go_decision_route_review_owner_summary_preview()
    assert first == second
    assert first is not second
    first["status"] = "mutated"
    third = mod.build_tower_beta_owner_go_no_go_decision_route_review_owner_summary_preview()
