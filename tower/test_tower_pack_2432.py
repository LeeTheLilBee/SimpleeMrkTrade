...
def test_pack_2432_walkthrough_contract():
    payload = build_ir_cert_p2432_preview()

    assert payload["status"] == "ready"
    assert payload["readiness"] == 100
    assert payload["pack_name"] == "Owner Walkthrough Readiness Checkpoint"
    assert payload["owner_walkthrough_ready"] is True
