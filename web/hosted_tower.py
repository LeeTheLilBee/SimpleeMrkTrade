"""
Hosted Tower WSGI entrypoint for Render.

This module intentionally imports the canonical Flask app from web.app.
It exists so managed hosts configured for web.hosted_tower:app can
...
def hosted_tower_runtime_manifest() -> dict[str, object]:
...
if (
    "simplee_hosted_tower_runtime_manifest"
...
def hosted_tower_status() -> dict[str, object]:
...
if "simplee_hosted_tower_healthz" not in app.view_functions:
    app.add_url_rule(
        "/tower/healthz",
        endpoint="simplee_hosted_tower_healthz",
        view_func=_simplee_hosted_tower_health_view,
        methods=["GET"],
    )


class _SimpleeHostedTowerHealthMiddleware:
...
if not getattr(app, "_simplee_health_middleware_v1", False):
    app.wsgi_app = _SimpleeHostedTowerHealthMiddleware(app.wsgi_app)
...
# hosted Tower health middleware and sees every response, including the
