...
# =========================================================
# Mode resolution + policy helpers
# =========================================================
...
def execute_trades(
    ...
    for trade in queue:
        ...
        # ---------------------------------------------
        # Mandatory execution-ready state
        # Paper must block here exactly like live.
        # Only Survey is allowed to soften soft blocks.
        # ---------------------------------------------
        lifecycle_reason_code = _safe_str(
            lifecycle.get("final_reason_code"),
            "execution_not_ready",
        )
        lifecycle_reason_text = _safe_str(
            lifecycle.get("final_reason"),
        ...
        if not _safe_bool(lifecycle.get("execution_ready"), False):
            ...
            if should_reject:
                ...
                lifecycle["warnings"] = _dedupe_keep_order(
                    lifecycle.get("warnings", []) + [lifecycle_reason_code]
                )

        # ---------------------------------------------
        # Existing open position duplication guard
        # ---------------------------------------------
        ...
        # ---------------------------------------------
        # Open position cap
        # This remains hard for paper/live.
        # ---------------------------------------------
        ...
        if current_open_positions >= max_open_positions:
            ...
            if should_reject:
                ...
                lifecycle.setdefault("warnings", [])
                lifecycle["warnings"] = _dedupe_keep_order(
                    lifecycle.get("warnings", []) + [reason_code]
                )

        # ---------------------------------------------
        # Canonical handoff execution
        # ---------------------------------------------
        ...
        # ---------------------------------------------
        # Rejection from handoff / guard layer
        # Survey can soften soft blocks.
        # Paper/live cannot.
        # ---------------------------------------------
        if not packet_success:
            ...
            lifecycle_after["execution_ready"] = False if should_reject else _safe_bool(
                lifecycle_after.get("execution_ready"), False
            )
