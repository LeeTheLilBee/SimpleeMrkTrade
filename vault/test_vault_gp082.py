"""
Tests for VAULT GP082 — Real Storage Provider Restore Eligibility Lock Contract
"""
...
from vault.real_storage_provider_restore_eligibility_lock_contract_service import (
    ...
    ensure_storage_provider_restore_eligibility_lock_contract_schema,
    get_gp082_status,
    get_real_storage_provider_restore_eligibility_lock_contract_home,
    get_storage_provider_restore_eligibility_blockers,
    get_storage_provider_restore_eligibility_events,
    get_storage_provider_restore_eligibility_lock_contract_record,
    get_storage_provider_restore_eligibility_next_step,
    get_storage_provider_restore_eligibility_policies,
    get_storage_provider_restore_eligibility_requirements,
    initialize_real_storage_provider_restore_eligibility_lock_contract,
    record_storage_provider_restore_eligibility_event,
    render_real_storage_provider_restore_eligibility_lock_contract_page,
    validate_storage_provider_restore_eligibility_lock_contract,
)

PROJECT_ROOT = Path(__file__).resolve().parents[1]
EXPECTED_SOURCE_REQUIREMENTS = 54
EXPECTED_REQUIREMENTS = EXPECTED_SOURCE_REQUIREMENTS * len(RESTORE_ELIGIBILITY_REQUIREMENT_SPECS)
EXPECTED_POLICIES = len(RESTORE_ELIGIBILITY_POLICIES)
EXPECTED_BLOCKERS = 18

@pytest.fixture()
def gp082_db(tmp_path, monkeypatch):
    envs = {
    ...
    return str(tmp_path / "restore_eligibility_lock_contract.sqlite")

def test_gp082_schema_is_real_sqlite_backed(gp082_db):
    result = ensure_storage_provider_restore_eligibility_lock_contract_schema(gp082_db)

    assert result["schema_ready"] is True
    assert result["real_sqlite_backed"] is True
    assert Path(result["db_path"]).exists()
    assert "vault_storage_provider_restore_eligibility_lock_contracts" in result["tables"]
    assert "vault_storage_provider_restore_eligibility_requirements" in result["tables"]
    assert "vault_storage_provider_restore_eligibility_policies" in result["tables"]
    assert "vault_storage_provider_restore_eligibility_blockers" in result["tables"]
    assert "vault_storage_provider_restore_eligibility_events" in result["tables"]

def test_gp082_initialize_creates_real_contract_requirements_policies_blockers_events(gp082_db):
    result = initialize_real_storage_provider_restore_eligibility_lock_contract(gp082_db)
    ...
    second = initialize_real_storage_provider_restore_eligibility_lock_contract(gp082_db)
...
def test_gp082_contract_sourced_from_gp081_and_locked(gp082_db):
    contract = get_storage_provider_restore_eligibility_lock_contract_record(gp082_db)["restore_eligibility_lock_contract"]

    assert contract["restore_eligibility_lock_contract_id"] == DEFAULT_RESTORE_ELIGIBILITY_LOCK_CONTRACT_ID
    assert contract["pack_id"] == "VAULT_GP082"
    ...
    assert contract["source_restore_request_lock_contract_id"] == "VSPRQLC-GP081-001"
    assert contract["source_restore_request_pack_id"] == "VAULT_GP081"
    assert contract["contract_status"] == "REAL_RESTORE_ELIGIBILITY_LOCK_CONTRACT_OPEN_TEMPLATE_ONLY_TOWER_LOCKED"

    assert contract["restore_eligibility_lock_contract_ready"] is True
    assert contract["restore_eligibility_requirements_ready"] is True
    assert contract["restore_eligibility_policies_ready"] is True
    assert contract["restore_eligibility_blockers_ready"] is True
    assert contract["restore_eligibility_validation_ready"] is True
    assert contract["restore_eligibility_locked"] is True
    assert contract["restore_eligibility_template_only"] is True
    assert contract["source_restore_request_lock_contract_attached"] is True
    assert contract["safe_to_continue_to_gp083"] is True
    ...
    assert contract["contract_data"]["next_pack"] == "VAULT_GP083_REAL_STORAGE_PROVIDER_RESTORE_AUTHORITY_LOCK_CONTRACT"
    assert contract["contract_data"]["safe_to_continue_to_gp083"] is True

def test_gp082_requirements_are_real_template_only_and_locked(gp082_db):
    payload = get_storage_provider_restore_eligibility_requirements(gp082_db)
    ...
    assert payload["requirement_code_count"] == len(RESTORE_ELIGIBILITY_REQUIREMENT_SPECS)
    assert payload["requirement_required_count"] == EXPECTED_REQUIREMENTS
    assert payload["requirement_verified_count"] == 0
    assert payload["restore_eligibility_locked_count"] == EXPECTED_REQUIREMENTS
    assert payload["template_only_count"] == EXPECTED_REQUIREMENTS
    assert payload["eligibility_redaction_required_count"] == EXPECTED_REQUIREMENTS
    assert payload["tower_review_granted_count"] == 0

    assert payload["restore_eligibility_checked_count"] == 0
    assert payload["restore_eligibility_passed_count"] == 0
    assert payload["restore_eligibility_failed_count"] == 0
    assert payload["restore_eligibility_result_recorded_count"] == 0
    ...
    for item in payload["requirements"]:
        assert item["restore_eligibility_requirement_id"].startswith("VSPRER-")
        assert item["restore_eligibility_lock_contract_id"] == DEFAULT_RESTORE_ELIGIBILITY_LOCK_CONTRACT_ID
        assert item["source_requirement_id"].startswith("VSPRQR-")
        assert item["restore_eligibility_locked"] is True
        assert item["template_only"] is True
        assert item["restore_eligibility_checked"] is False
        assert item["restore_eligibility_passed"] is False
        assert item["restore_eligibility_failed"] is False
...
def test_gp082_policies_are_real_template_only_and_locked(gp082_db):
    payload = get_storage_provider_restore_eligibility_policies(gp082_db)
    ...
    for item in payload["policies"]:
        assert item["restore_eligibility_policy_id"].startswith("VSPREP-")
        assert item["restore_eligibility_lock_contract_id"] == DEFAULT_RESTORE_ELIGIBILITY_LOCK_CONTRACT_ID
        assert item["restore_eligibility_checked"] is False
        assert item["restore_eligibility_passed"] is False
        assert item["restore_eligibility_failed"] is False
...
def test_gp082_blockers_are_real_and_carried_from_gp081(gp082_db):
    payload = get_storage_provider_restore_eligibility_blockers(gp082_db)
    ...
    assert payload["blocks_restore_eligibility_result_count"] == EXPECTED_BLOCKERS
    ...
    for item in payload["blockers"]:
        assert item["restore_eligibility_blocker_id"].startswith("VSPREB-")
        assert item["restore_eligibility_lock_contract_id"] == DEFAULT_RESTORE_ELIGIBILITY_LOCK_CONTRACT_ID
        assert item["source_restore_request_blocker_id"].startswith("VSPRQB-")
        assert item["blocker_active"] is True
        assert item["blocks_restore_eligibility_check"] is True
        assert item["blocks_restore_eligibility_result"] is True
...
def test_gp082_event_log_and_manual_event_write_do_not_unlock(gp082_db):
    events = get_storage_provider_restore_eligibility_events(gp082_db)
    assert events["event_count"] >= 6

    event_types = {event["event_type"] for event in events["events"]}
    assert "REAL_STORAGE_PROVIDER_RESTORE_ELIGIBILITY_LOCK_CONTRACT_CREATED" in event_types
    assert "SOURCE_GP081_RESTORE_REQUEST_LOCK_CONTRACT_ATTACHED" in event_types
    assert "REAL_RESTORE_ELIGIBILITY_REQUIREMENTS_CREATED_TEMPLATE_ONLY" in event_types
    assert "REAL_RESTORE_ELIGIBILITY_POLICIES_CREATED_TEMPLATE_ONLY" in event_types
    assert "REAL_RESTORE_ELIGIBILITY_BLOCKERS_CARRIED_FORWARD" in event_types
    assert "RESTORE_ELIGIBILITY_LOCKS_CONFIRMED" in event_types

    before = events["event_count"]
    written = record_storage_provider_restore_eligibility_event(
        "OWNER_GP082_RESTORE_ELIGIBILITY_REVIEWED",
        {"reviewer": "owner"},
        gp082_db,
    )
    after = get_storage_provider_restore_eligibility_events(gp082_db)

    assert after["event_count"] == before + 1
    assert written["event_written"] is True
    assert written["restore_eligibility_lock_contract_ready"] is True
    assert written["restore_eligibility_checked"] is False
    assert written["restore_eligibility_passed"] is False
    assert written["restore_eligibility_failed"] is False
...
def test_gp082_validation_home_status_and_next_step(gp082_db):
    validation = validate_storage_provider_restore_eligibility_lock_contract(gp082_db)

    assert validation["valid"] is True
    assert validation["failed_count"] == 0
    assert validation["safe_to_continue_to_gp083"] is True
    assert validation["vault_done"] is False

    codes = {item["code"] for item in validation["checks"]}
    assert "REAL_SQLITE_RESTORE_ELIGIBILITY_LOCK_CONTRACT_EXISTS" in codes
    assert "SOURCE_GP081_RESTORE_REQUEST_LOCK_CONTRACT_ATTACHED" in codes
    assert "SECTION_GP081_GP090" in codes
    assert "RESTORE_ELIGIBILITY_LOCK_CONTRACT_READY" in codes
    assert "RESTORE_ELIGIBILITY_REQUIREMENTS_EXIST" in codes
    assert "RESTORE_ELIGIBILITY_POLICIES_EXIST" in codes
    assert "RESTORE_REQUEST_BLOCKERS_CARRIED_FORWARD" in codes
    assert "NO_CONTRACT_RESTORE_ELIGIBILITY_CHECKED" in codes
    assert "NO_CONTRACT_RESTORE_ELIGIBILITY_PASSED" in codes
    assert "NO_CONTRACT_RESTORE_ELIGIBILITY_FAILED" in codes
    ...
    home = get_real_storage_provider_restore_eligibility_lock_contract_home(gp082_db)
    truth = home["restore_eligibility_truth"]
    assert truth["real_storage_provider_restore_eligibility_lock_contract_ready"] is True
    assert truth["validation_passed"] is True
    assert truth["restore_eligibility_locked"] is True
    assert truth["restore_eligibility_template_only"] is True
    ...
    assert truth["restore_eligibility_passed"] is False
    assert truth["restore_eligibility_failed"] is False
    ...
    status = get_gp082_status(gp082_db)
    gp082 = status["gp082_status"]
    assert gp082["ready"] is True
    assert gp082["validation_passed"] is True
    assert gp082["safe_to_continue_to_gp083"] is True
    assert gp082["source_requirement_count"] == EXPECTED_SOURCE_REQUIREMENTS
    assert gp082["requirement_code_count"] == len(RESTORE_ELIGIBILITY_REQUIREMENT_SPECS)
    assert gp082["policy_code_count"] == EXPECTED_POLICIES
    assert gp082["blocker_count"] == EXPECTED_BLOCKERS
    assert gp082["restore_eligibility_checked_count"] == 0
    assert gp082["restore_eligibility_passed_count"] == 0
    assert gp082["restore_eligibility_failed_count"] == 0
    assert gp082["restore_authority_verified_count"] == 0
    assert gp082["restore_scope_selected_count"] == 0
    assert gp082["restore_target_selected_count"] == 0
    assert gp082["restore_object_selected_count"] == 0
    assert gp082["provider_restore_api_called_count"] == 0
    assert gp082["object_body_read_count"] == 0
    assert gp082["direct_upload_enabled_count"] == 0
    assert gp082["export_enabled_count"] == 0
    assert gp082["execution_enabled_count"] == 0
    assert gp082["vault_done"] is False
    assert gp082["clouds_status"] == "parked_do_not_continue_from_vault_gp082"
    assert gp082["next_pack"] == "VAULT_GP083_REAL_STORAGE_PROVIDER_RESTORE_AUTHORITY_LOCK_CONTRACT"

    next_step = get_storage_provider_restore_eligibility_next_step()["next_step"]
    assert next_step["current_section_range"] == "GP081-GP090"
    assert next_step["next_pack"] == "VAULT_GP083_REAL_STORAGE_PROVIDER_RESTORE_AUTHORITY_LOCK_CONTRACT"
    assert next_step["safe_to_continue_to_gp083"] is True
    assert next_step["vault_done"] is False
    assert next_step["clouds_should_continue"] is False

def test_gp082_html_is_dark_and_mentions_restore_eligibility(monkeypatch, tmp_path):
    envs = {
    ...
    html = render_real_storage_provider_restore_eligibility_lock_contract_page()
    lowered = html.lower()

    assert "Vault Real Storage Provider Restore Eligibility Lock Contract" in html
    assert "Real Provider Restore and Export Governance Layer" in html
    assert "GP082" in html
    assert "Eligibility locked" in html
    assert "Template-only" in html
    assert "No eligibility check" in html
    assert "No authority verify" in html
    assert "No object selection" in html
    assert "No export" in html
    assert "No execution" in html
    assert "/vault/real-storage-provider-restore-eligibility-lock-contract.json" in html
    assert "/vault/gp082-status.json" in html
...
def test_gp082_routes_registered_in_web_app_text():
    text = (PROJECT_ROOT / "web" / "app.py").read_text(encoding="utf-8", errors="ignore")
    required_routes = [
        "/vault/real-storage-provider-restore-eligibility-lock-contract",
        "/vault/real-storage-provider-restore-eligibility-lock-contract.json",
        "/vault/storage-provider-restore-eligibility-lock-contract-record.json",
        "/vault/storage-provider-restore-eligibility-requirements.json",
        "/vault/storage-provider-restore-eligibility-policies.json",
        "/vault/storage-provider-restore-eligibility-blockers.json",
        "/vault/storage-provider-restore-eligibility-events.json",
        "/vault/storage-provider-restore-eligibility-validation.json",
        "/vault/storage-provider-restore-eligibility-next-step.json",
        "/vault/gp082-status.json",
    ]
    for route in required_routes:
        assert route in text

def test_gp082_flask_routes_when_app_importable_accepts_tower_guard(monkeypatch, tmp_path):
    envs = {
    ...
    for route in routes:
        ...
        if response.status_code == 200:
            if route.endswith(".json"):
                ...
                assert b"Vault Real Storage Provider Restore Eligibility Lock Contract" in response.data
