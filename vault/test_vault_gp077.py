"""
Tests for VAULT GP077 — Real Storage Provider Owner Review Decision Lock Contract
"""
...
from vault.real_storage_provider_owner_review_decision_lock_contract_service import (
    ...
    ensure_storage_provider_owner_review_decision_lock_contract_schema,
    get_gp077_status,
    get_real_storage_provider_owner_review_decision_lock_contract_home,
    get_storage_provider_owner_review_decision_blockers,
    get_storage_provider_owner_review_decision_events,
    get_storage_provider_owner_review_decision_lock_contract_record,
    get_storage_provider_owner_review_decision_next_step,
    get_storage_provider_owner_review_decision_policies,
    get_storage_provider_owner_review_decision_requirements,
    initialize_real_storage_provider_owner_review_decision_lock_contract,
    record_storage_provider_owner_review_decision_event,
    render_real_storage_provider_owner_review_decision_lock_contract_page,
    validate_storage_provider_owner_review_decision_lock_contract,
)

PROJECT_ROOT = Path(__file__).resolve().parents[1]
EXPECTED_SOURCE_REQUIREMENTS = 63
EXPECTED_REQUIREMENTS = EXPECTED_SOURCE_REQUIREMENTS * len(DECISION_REQUIREMENT_SPECS)
EXPECTED_POLICIES = len(DECISION_POLICIES)
EXPECTED_BLOCKERS = 14

@pytest.fixture()
def gp077_db(tmp_path, monkeypatch):
    envs = {
    ...
    return str(tmp_path / "owner_review_decision_lock_contract.sqlite")

def test_gp077_schema_is_real_sqlite_backed(gp077_db):
    result = ensure_storage_provider_owner_review_decision_lock_contract_schema(gp077_db)

    assert result["schema_ready"] is True
    assert result["real_sqlite_backed"] is True
    assert Path(result["db_path"]).exists()
    assert "vault_storage_provider_owner_review_decision_lock_contracts" in result["tables"]
    assert "vault_storage_provider_owner_review_decision_requirements" in result["tables"]
    assert "vault_storage_provider_owner_review_decision_policies" in result["tables"]
    assert "vault_storage_provider_owner_review_decision_blockers" in result["tables"]
    assert "vault_storage_provider_owner_review_decision_events" in result["tables"]

def test_gp077_initialize_creates_real_contract_requirements_policies_blockers_events(gp077_db):
    result = initialize_real_storage_provider_owner_review_decision_lock_contract(gp077_db)
    ...
    second = initialize_real_storage_provider_owner_review_decision_lock_contract(gp077_db)
...
def test_gp077_contract_sourced_from_gp076_and_locked(gp077_db):
    contract = get_storage_provider_owner_review_decision_lock_contract_record(gp077_db)["owner_review_decision_lock_contract"]

    assert contract["owner_review_decision_lock_contract_id"] == DEFAULT_OWNER_REVIEW_DECISION_LOCK_CONTRACT_ID
    assert contract["pack_id"] == "VAULT_GP077"
    ...
    assert contract["source_owner_review_queue_lock_contract_id"] == "VSPORQLC-GP076-001"
    assert contract["source_owner_review_queue_pack_id"] == "VAULT_GP076"
    assert contract["contract_status"] == "REAL_OWNER_REVIEW_DECISION_LOCK_CONTRACT_OPEN_TEMPLATE_ONLY_TOWER_LOCKED"

    assert contract["owner_review_decision_lock_contract_ready"] is True
    assert contract["owner_review_decision_requirements_ready"] is True
    assert contract["owner_review_decision_policies_ready"] is True
    assert contract["owner_review_decision_blockers_ready"] is True
    assert contract["owner_review_decision_validation_ready"] is True
    assert contract["owner_review_decision_locked"] is True
    assert contract["decision_template_only"] is True
    assert contract["decision_redaction_required"] is True
    assert contract["source_owner_review_queue_lock_contract_attached"] is True
    assert contract["safe_to_continue_to_gp078"] is True
    ...
    assert contract["contract_data"]["safe_to_continue_to_gp078"] is True
    assert contract["contract_data"]["next_pack"] == "VAULT_GP078_REAL_STORAGE_PROVIDER_OWNER_REVIEW_DECISION_RECEIPT_LOCK_CONTRACT"

def test_gp077_requirements_are_real_template_only_and_locked(gp077_db):
    payload = get_storage_provider_owner_review_decision_requirements(gp077_db)
    ...
    assert payload["requirement_code_count"] == len(DECISION_REQUIREMENT_SPECS)
    assert payload["requirement_required_count"] == EXPECTED_REQUIREMENTS
    assert payload["requirement_verified_count"] == 0
    assert payload["decision_locked_count"] == EXPECTED_REQUIREMENTS
    assert payload["template_only_count"] == EXPECTED_REQUIREMENTS
    assert payload["decision_redaction_required_count"] == EXPECTED_REQUIREMENTS
    assert payload["tower_review_granted_count"] == 0

    assert payload["owner_review_decision_created_count"] == 0
    assert payload["provider_backed_decision_record_created_count"] == 0
    assert payload["provider_backed_decision_record_attached_count"] == 0
    ...
    assert payload["owner_decision_deferred_count"] == 0
    assert payload["decision_reason_recorded_count"] == 0
    assert payload["decision_receipt_created_count"] == 0
    ...
    assert payload["decision_download_enabled_count"] == 0
    ...
    for item in payload["requirements"]:
        assert item["owner_review_decision_requirement_id"].startswith("VSPORDR-")
        assert item["owner_review_decision_lock_contract_id"] == DEFAULT_OWNER_REVIEW_DECISION_LOCK_CONTRACT_ID
        assert item["source_requirement_id"].startswith("VSPORQR-")
        assert item["decision_locked"] is True
        assert item["template_only"] is True
        assert item["decision_redaction_required"] is True
        assert item["owner_review_decision_created"] is False
        assert item["owner_decision_recorded"] is False
        assert item["owner_decision_approved"] is False
        assert item["owner_decision_denied"] is False
        assert item["decision_reason_recorded"] is False
        assert item["export_enabled"] is False
        assert item["execution_enabled"] is False

def test_gp077_policies_are_real_template_only_and_locked(gp077_db):
    payload = get_storage_provider_owner_review_decision_policies(gp077_db)
    ...
    assert payload["decision_reason_recorded_count"] == 0
    assert payload["decision_receipt_created_count"] == 0
    assert payload["decision_receipt_finalized_count"] == 0
    assert payload["owner_approval_granted_count"] == 0
    assert payload["owner_denial_recorded_count"] == 0
    assert payload["tower_unlock_granted_count"] == 0
    assert payload["provider_packet_attached_count"] == 0
    ...
    for item in payload["policies"]:
        assert item["owner_review_decision_policy_id"].startswith("VSPORDP-")
        assert item["owner_review_decision_lock_contract_id"] == DEFAULT_OWNER_REVIEW_DECISION_LOCK_CONTRACT_ID
        assert item["owner_review_decision_created"] is False
        assert item["owner_decision_recorded"] is False
        assert item["owner_decision_approved"] is False
...
def test_gp077_blockers_are_real_and_carried_from_gp076(gp077_db):
    payload = get_storage_provider_owner_review_decision_blockers(gp077_db)

    assert payload["blocker_count"] == EXPECTED_BLOCKERS
    assert payload["blocker_active_count"] == EXPECTED_BLOCKERS
    assert payload["blocks_owner_review_decision_count"] == EXPECTED_BLOCKERS
    assert payload["blocks_provider_backed_decision_record_count"] == EXPECTED_BLOCKERS
    assert payload["blocks_owner_approval_count"] == EXPECTED_BLOCKERS
    assert payload["blocks_owner_denial_count"] == EXPECTED_BLOCKERS
    assert payload["blocks_decision_reason_count"] == EXPECTED_BLOCKERS
    ...
    for item in payload["blockers"]:
        assert item["owner_review_decision_blocker_id"].startswith("VSPORDB-")
        assert item["owner_review_decision_lock_contract_id"] == DEFAULT_OWNER_REVIEW_DECISION_LOCK_CONTRACT_ID
        assert item["source_owner_review_queue_blocker_id"].startswith("VSPORQB-")
        assert item["blocker_active"] is True
        assert item["blocks_owner_review_decision"] is True
        assert item["blocks_provider_backed_decision_record"] is True
        assert item["blocks_owner_approval"] is True
        assert item["blocks_owner_denial"] is True
        assert item["blocks_tower_unlock"] is True
        assert item["blocks_export"] is True
        assert item["blocks_execution"] is True
        assert item["resolved"] is False

def test_gp077_event_log_and_manual_event_write_do_not_unlock(gp077_db):
    events = get_storage_provider_owner_review_decision_events(gp077_db)
    assert events["event_count"] >= 6

    event_types = {event["event_type"] for event in events["events"]}
    assert "REAL_STORAGE_PROVIDER_OWNER_REVIEW_DECISION_LOCK_CONTRACT_CREATED" in event_types
    assert "SOURCE_GP076_OWNER_REVIEW_QUEUE_LOCK_CONTRACT_ATTACHED" in event_types
    assert "REAL_OWNER_REVIEW_DECISION_REQUIREMENTS_CREATED_TEMPLATE_ONLY" in event_types
    assert "REAL_OWNER_REVIEW_DECISION_POLICIES_CREATED_TEMPLATE_ONLY" in event_types
    assert "REAL_OWNER_REVIEW_DECISION_BLOCKERS_CARRIED_FORWARD" in event_types
    assert "OWNER_REVIEW_DECISION_LOCKS_CONFIRMED" in event_types

    before = events["event_count"]
    written = record_storage_provider_owner_review_decision_event(
        "OWNER_GP077_DECISION_REVIEWED",
        {"reviewer": "owner"},
        gp077_db,
    )
    after = get_storage_provider_owner_review_decision_events(gp077_db)

    assert after["event_count"] == before + 1
    assert written["event_written"] is True
    assert written["owner_review_decision_locked"] is True
    assert written["decision_template_only"] is True
    assert written["owner_review_decision_created"] is False
    assert written["provider_backed_decision_record_created"] is False
    assert written["owner_decision_requested"] is False
    assert written["owner_decision_recorded"] is False
    assert written["owner_decision_approved"] is False
    assert written["owner_decision_denied"] is False
    assert written["decision_reason_recorded"] is False
    ...
    assert written["decision_download_enabled"] is False
    assert written["export_enabled"] is False
    assert written["execution_enabled"] is False
    assert written["vault_done"] is False

def test_gp077_validation_home_status_and_next_step(gp077_db):
    validation = validate_storage_provider_owner_review_decision_lock_contract(gp077_db)

    assert validation["valid"] is True
    assert validation["failed_count"] == 0
    assert validation["safe_to_continue_to_gp078"] is True
    ...
    assert "SOURCE_GP076_OWNER_REVIEW_QUEUE_LOCK_CONTRACT_ATTACHED" in codes
    assert "OWNER_REVIEW_DECISION_LOCK_CONTRACT_READY" in codes
    assert "OWNER_REVIEW_DECISION_LOCKED" in codes
    assert "DECISION_TEMPLATE_ONLY" in codes
    assert "REAL_OWNER_REVIEW_DECISION_REQUIREMENTS_EXIST" in codes
    assert "REAL_OWNER_REVIEW_DECISION_POLICIES_EXIST" in codes
    assert "REAL_OWNER_REVIEW_DECISION_BLOCKERS_CARRIED_FORWARD" in codes
    assert "NO_CONTRACT_OWNER_REVIEW_DECISION_CREATED" in codes
    assert "NO_CONTRACT_OWNER_DECISION_RECORDED" in codes
    assert "NO_CONTRACT_OWNER_DECISION_APPROVED" in codes
    assert "NO_CONTRACT_OWNER_DECISION_DENIED" in codes
    assert "NO_CONTRACT_DECISION_REASON_RECORDED" in codes
    ...
    home = get_real_storage_provider_owner_review_decision_lock_contract_home(gp077_db)
    truth = home["owner_review_decision_truth"]
    assert truth["real_storage_provider_owner_review_decision_lock_contract_ready"] is True
    assert truth["validation_passed"] is True
    assert truth["owner_review_decision_locked"] is True
    assert truth["decision_template_only"] is True
    ...
    assert truth["owner_review_decision_created"] is False
    assert truth["owner_decision_requested"] is False
    ...
    assert truth["decision_reason_recorded"] is False
    ...
    assert truth["decision_download_enabled"] is False
    ...
    status = get_gp077_status(gp077_db)
    gp077 = status["gp077_status"]
    assert gp077["ready"] is True
    assert gp077["validation_passed"] is True
    assert gp077["safe_to_continue_to_gp078"] is True
    assert gp077["source_requirement_count"] == EXPECTED_SOURCE_REQUIREMENTS
    assert gp077["requirement_code_count"] == len(DECISION_REQUIREMENT_SPECS)
    assert gp077["policy_code_count"] == EXPECTED_POLICIES
    assert gp077["blocker_count"] == EXPECTED_BLOCKERS
    assert gp077["owner_review_decision_created_count"] == 0
    assert gp077["provider_backed_decision_record_created_count"] == 0
    assert gp077["owner_decision_requested_count"] == 0
    assert gp077["owner_decision_recorded_count"] == 0
    assert gp077["owner_decision_approved_count"] == 0
    assert gp077["owner_decision_denied_count"] == 0
    assert gp077["decision_reason_recorded_count"] == 0
    assert gp077["decision_receipt_created_count"] == 0
    assert gp077["owner_approval_granted_count"] == 0
    assert gp077["tower_unlock_granted_count"] == 0
    assert gp077["provider_packet_attached_count"] == 0
    assert gp077["object_identifier_attached_count"] == 0
    assert gp077["object_body_attached_count"] == 0
    assert gp077["decision_download_enabled_count"] == 0
    assert gp077["direct_upload_enabled_count"] == 0
    assert gp077["export_enabled_count"] == 0
    assert gp077["execution_enabled_count"] == 0
    assert gp077["vault_done"] is False
    assert gp077["clouds_status"] == "parked_do_not_continue_from_vault_gp077"
    assert gp077["next_pack"] == "VAULT_GP078_REAL_STORAGE_PROVIDER_OWNER_REVIEW_DECISION_RECEIPT_LOCK_CONTRACT"
    ...
    assert next_step["next_pack"] == "VAULT_GP078_REAL_STORAGE_PROVIDER_OWNER_REVIEW_DECISION_RECEIPT_LOCK_CONTRACT"
    assert next_step["safe_to_continue_to_gp078"] is True
    assert next_step["vault_done"] is False
    assert next_step["clouds_should_continue"] is False

def test_gp077_html_is_dark_and_mentions_owner_review_decision(monkeypatch, tmp_path):
    envs = {
    ...
    html = render_real_storage_provider_owner_review_decision_lock_contract_page()
    lowered = html.lower()

    assert "Vault Real Storage Provider Owner Review Decision Lock Contract" in html
    assert "Real Provider Receipt and Redacted Access Layer" in html
    assert "GP077" in html
    assert "Owner decision contract ready" in html
    assert "Template-only" in html
    assert "No decision" in html
    assert "No approval" in html
    assert "No denial" in html
    assert "No export" in html
    assert "No execution" in html
    assert "/vault/real-storage-provider-owner-review-decision-lock-contract.json" in html
    assert "/vault/gp077-status.json" in html
...
def test_gp077_routes_registered_in_web_app_text():
    text = (PROJECT_ROOT / "web" / "app.py").read_text(encoding="utf-8", errors="ignore")
    required_routes = [
        "/vault/real-storage-provider-owner-review-decision-lock-contract",
        "/vault/real-storage-provider-owner-review-decision-lock-contract.json",
        "/vault/storage-provider-owner-review-decision-lock-contract-record.json",
        "/vault/storage-provider-owner-review-decision-requirements.json",
        "/vault/storage-provider-owner-review-decision-policies.json",
        "/vault/storage-provider-owner-review-decision-blockers.json",
        "/vault/storage-provider-owner-review-decision-events.json",
        "/vault/storage-provider-owner-review-decision-validation.json",
        "/vault/storage-provider-owner-review-decision-next-step.json",
        "/vault/gp077-status.json",
    ]
    for route in required_routes:
        assert route in text

def test_gp077_flask_routes_when_app_importable_accepts_tower_guard(monkeypatch, tmp_path):
    envs = {
    ...
    for route in routes:
        ...
        if response.status_code == 200:
            if route.endswith(".json"):
                ...
                assert b"Vault Real Storage Provider Owner Review Decision Lock Contract" in response.data
